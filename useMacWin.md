...
3. 由於 shell 狀態不一定會在你之後每次呼叫工具時持續生效，之後所有 Python 相關指令（pytest、ruff、pip 等）請直接使用 `.venv/bin/python`（或先確認 `which python` 真的指向這個路徑）執行，不要誤用系統 Python。

4. 確認你已讀過根目錄 CLAUDE.md（monorepo 導覽），並在動手前先讀清楚本次工作屬於哪個子專案（AutoClaude 或 AISDLC_SDD），進去該子專案前務必先讀它自己的 CLAUDE.md ——兩份子 CLAUDE.md 都是 override 級規範，其目錄/命名/測試規則以子專案內部路徑為準，不要套用錯誤的相對路徑假設。
...
1. 在 repo 根目錄的 PowerShell 環境中執行（dot-source，前面的點與空格不可省略）：
   . .\tools\dev_start.ps1
...
3. 之後所有 Python 相關指令（pytest、ruff、pip 等）請直接使用 `.venv\Scripts\python.exe`，不要誤用系統 Python。
...
需要時可以在對應提示詞的第 1 步指令後面追加，或另外請 Claude Code 加上：
