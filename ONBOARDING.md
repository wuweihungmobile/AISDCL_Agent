...
| **凍結版 v0.01~v0.29 的 7 支 `tools/fsm_runtime/` 檔案路徑穿越已補丁，但 `_sanitize_component()` 本身仍是較弱版本**（只剝除 `/`／`\`，未比照 v0.30 做 Windows 保留裝置名／控制字元／長度上限強化） | 本輪（R44）例外回補足以阻斷 `../` 相對路徑逃逸攻擊面，但不具備 v0.30 現有的完整防護縱深；與過去「凍結版較弱防護」判例不同的是，這是真的在擋一個已證實可利用的攻擊面，優先度基準可能需重新評估 | v0.30 已有強化版可參考；backport 與否留待下一輪評估（DEF-101-358） |
