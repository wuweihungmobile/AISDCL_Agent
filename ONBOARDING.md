...
① **環境偵測**（三行：當前 Now／本機上次平台（gitignored 狀態檔 `.dev_env_state.json`，只記這台機器）／**最近 commit 開發平台**（讀 git：每個 commit 由 `tools/git-hooks/prepare-commit-msg`／`commit-msg` 補的 `Dev-Platform` trailer，舊 commit 退到 ONBOARDING 平台錨／perf baseline `environment` 的內容啟發式；本體 `tools/lib/dev_platform_provenance.py`）；「專案上次在哪開發」以第三行為準；非 `--no-sync` 時 ① 會先做一次 `git fetch origin --prune`（最長 120 秒，開始前先印一行進度；只更新遠端追蹤分支、不動工作樹），本機落後 origin 時改讀 `origin/<branch>` 最新 commit，② 沿用同一次 fetch 不重複打網路；與 origin 分叉時另多印一行「遠端側 origin/<branch> 最近 commit 開發平台：…」、兩側各自宣告跨機切換；`--no-sync`／離線／無 origin 時第三行標「未 fetch，只反映本機 HEAD」，fetch 成功卻無法與 origin 比對（detached HEAD／origin/<branch> 不存在／計數失敗）則改標「未與 origin 比對，只反映本機 HEAD」——兩種警語出現時第三行與摘要皆不可拿來回答「上次在哪開發」）→ ② **GitHub 同步**（fetch + `--ff-only` pull；髒工作樹／分叉／離線一律明示不硬做，**絕不自動 stash／rebase／push**；未追蹤檔不擋同步）→ ③ **平台切換**（Developing≠Now 時清除含絕對路徑的 `.pytest_cache`/`.ruff_cache`）→ ④ **venv／依賴整備**（另一平台形狀的 `.venv` **換手保留**至 `.venv-cache-<flavor>/`，本平台快取存在則**秒級換回**；缺 `.venv` 或依賴檔（`pyproject.toml`/`requirements-ci.txt`）hash 變動 → 自動重跑 §2 bootstrap）→ ⑤ **git hooks 檢核**（`core.hooksPath` 未設／漂移 → 自動重跑安裝腳本，治 §6「搬移後 hooks 靜默全滅」）→ ⑥ **平台健檢**（Windows 自動設 `core.longpaths=true`）→ ⑦ **狀態寫回＋摘要**。

適用兩種拓撲：**共用工作目錄**（外接碟／同步資料夾，macOS ⇄ Windows 輪開同一份）由 ③④ 吸收全部切換成本；**雙機各自 clone** 則本機狀態檔恆等於 Now（③ 不會清快取，正確——本機快取本來就是本機的），跨機切換由 ① **先 fetch 再**從 git 判定並宣告（本機落後時讀的是 origin 上對面機器剛 push 的 commit；另一台機器有 commit 但尚未 push 時本機 git 天生看不到，此時第三行印本機平台屬正確、不是 bug），由 ②④ 把那些變更 ff-only 同步進來並保持依賴新鮮。旗標：`--no-sync`（離線跳過 ②）、`--force-bootstrap`（強制重裝依賴）。
...
| AutoClaude 加了 `-n auto` 之後 `--pdb`／單檔除錯卡住或行為怪異（DEF-200-274 D5） | `pyproject.toml` 的 `addopts` 全域帶 `-n auto --dist worksteal`；pytest-xdist 平行下 `--pdb` 與 xdist 互斥（xdist 官方文件明載），`-k` 篩單一測試時多開 8~9 個 worker 也是浪費 | 除錯／`--pdb`／單檔小範圍一律加 `-n 0`（停用平行，退回序列）：`python -m pytest tests/test_foo.py -k test_bar -n 0 --pdb`；`AutoClaude/tools/local_ci_gate.py::gate_pytest()` 對非預設參數呼叫已自動附加等效旗標，免手動 |
| 在 `AutoClaude/` 下跑 `uv sync`／`uv run …` 後多出 `AutoClaude/.venv`（或測試跑在錯的環境） | uv 專案指令預設用 **cwd 專案自己的 `.venv`**、無視已啟用的根層 venv（`uv sync --help`／`uv run --help` 皆有 `--active`「改用已啟用 venv」旗標，反證預設值正是專案自己的 venv；本 repo 只用 `uv venv`＋`uv pip install -e`，從不用這兩個專案指令） | 不要在子專案目錄跑 `uv sync`／`uv run`；`. tools\dev_start.ps1`／`source tools/dev_start.sh` 啟用 venv 時已同步 export `UV_PROJECT_ENVIRONMENT=<repo 根>/.venv` 兜底（DEF-200-323；uv 0.8.22 以 TEMP 沙盒 `uv sync --dry-run` 實測印出 `Would create project environment at: <該變數值>`，`uv help` 不列此變數是因它不綁旗標）；已長出的以 dev_start／pre-commit 印出的刪除指令移除 |
...
python -m pytest tests/ -q            # 全套測試（pyproject.toml addopts 自動套用 -n auto --dist worksteal；
                                       # 除錯／--pdb／單檔小範圍請額外加 -n 0，見 §5；PG 在場時見 §7.1）
...
AUTOSDD_PARALLEL_TESTS=1 python tools/run_root_unittests.py  # 強制平行（與未設同義；三支 CI 顯式帶它）；windows-compat-ci 已在
                                                              # windows-latest runner 真的跑過（成功）；掌舵者本人 Windows 11 機器上手動驗證仍待進行
python tools/refresh_parallel_timing_seed.py                 # 把本機活體快取灌進 git 追蹤的種子檔（CI 全新 runner 靠它排序）

# AISDLC_SDD（在 AISDLC_SDD/ 下）
bash scripts/ci-gate.sh               # 本機 CI 閘門（pytest + arch_fitness；兩份 pytest.ini 不含 xdist
                                       # addopts，ci-gate.sh 內部呼叫端已自動加 -n auto --dist worksteal，
                                       # 凍結基線 v0.01 除外——見該腳本內註記，DEF-200-274 D6）
```
...
```powershell
# AutoClaude（在 AutoClaude\ 下，需已啟用 venv）
python -m pytest tests/ -q            # 全套測試（與 bash 形態同；addopts 自動套用 -n auto --dist worksteal，見 §5/§7.1）
$env:PYTHONUTF8=1; lint-imports       # 架構約束（契約條數 SSOT＝AutoClaude\.importlinter；rc=0 即全 kept）

# 根層（在 repo 根目錄下）
python tools\run_root_unittests.py
python tools\run_root_unittests.py                                 # 預設自動多核心（DEF-200-274 第九輪；三態語意同上方 mac 區塊）；windows-compat-ci 已在 windows-latest runner 真的跑過（成功）；掌舵者本人 Windows 11 機器上手動驗證仍待進行
$env:AUTOSDD_PARALLEL_TESTS=0; python tools\run_root_unittests.py   # 強制序列

# AISDLC_SDD（在 AISDLC_SDD\ 下）
powershell -ExecutionPolicy Bypass -File scripts\ci-gate.ps1   # 偵測到 Git Bash 即薄委派 ci-gate.sh＝完整對等，見 §6
                                                                # （ci-gate.sh 內部已接 xdist，同左欄註記）
```
...
> <!-- snapshot-fingerprints-darwin: v001=8ffe3c3dabbd v030=38c50de2f6c3 scripts=e31c750901b4 autoclaude=b468881fecf3 measured-at=2026-09-22 host=Darwin-25.6.0-arm64 docker=down pgextras=absent interpreter=autoclaude_cleanvenv_20260922T014945Z/bin@3.11.15 sdk-extra=present baseline-origin=self-recorded ／ 由 `python tools/sync_onboarding_baselines.py --write --with-slow` 在 macOS 上維護，勿手改；刪除本標記會讓 --check-snapshot fail-loud -->
> <!-- snapshot-fingerprints-win32: v001=8ffe3c3dabbd v030=38c50de2f6c3 scripts=e31c750901b4 autoclaude=b468881fecf3 measured-at=2026-09-22 host=Windows-10-AMD64 docker=up pgextras=absent interpreter=autoclaude_cleanvenv_20260922T033152Z/Scripts@3.11.9 sdk-extra=present baseline-origin=self-recorded ／ 同上，由 Windows 側維護。🔴 該 origin 值的語意以 `tools/lib/baseline_origin.py::ORIGIN_SELF` 為準（本行不另寫一份定義）＝**本欄四格是在同一台 Windows 真機上一次量完、env 欄位在當時的定義下齊全**。🔴 兩條錨的後兩欄現值是 `tools/lib/baseline_origin.py::PRE_FIELD`＝**本錨早於那一欄**（不是「不可考」，也**不得**手填一個猜的值——那會把今天的環境寫在昨天的數字旁邊）；下一次在該平台跑 `--write --with-slow` 就會自動被真值取代 -->
>
...
> |---|---|---|---|
> | `windows-compat-ci.yml` | ✅ success | `2206a3a` | 2026-09-21 現查：push run `35487889144`，headSha 即 HEAD（`2206a3a0`）本身，conclusion=success。🔴 **本表存在的理由（史料保留）**：曾有一輪六道本機閘門全綠、而雲端這支是紅的，沒有任何本機機械物報得出來；此後也曾因「判準拿會隨 push 前進的參照當比較對象」再紅過一次。現值已回綠，史料保留供下次再紅時對照判讀路徑 |
> | `macos-compat-ci.yml` | ✅ success | `2206a3a` | 2026-09-21 現查：push run `35487889288`，headSha 即 HEAD 本身，conclusion=success |
> | `root-infra-ci.yml` | ✅ success | `2206a3a` | 2026-09-21 現查：push run `35487889186`，headSha 即 HEAD 本身，conclusion=success |
> | `autoclaude-ci.yml` | ✅ success | `2206a3a` | 2026-09-21 現查：push run `35487889188`（GitHub UI 顯示名 `AutoClaude CI`），本次 `paths:` 過濾有觸發（headSha 即 HEAD），conclusion=success |
> | `aisdlc-sdd-ci.yml` | ✅ success | `2206a3a` | 2026-09-21 現查：push run `35487889168`，本次有觸發（headSha 即 HEAD），conclusion=success |
> | `autoclaude-mutation-on-change.yml` | ✅ success | `be53ff0` | 2026-09-21 現查：本次 HEAD 因 `paths:` 過濾未觸發（源碼未變動，見 `ADR-SD09-011`）；最近一次觸發的 push run `31840901824`（commit `be53ff0e`）conclusion=success，commit 欄照實記該次 run 的 sha、不得填 HEAD |
> | `shellcheck-ci.yml` | ✅ success | `2206a3a` | 2026-09-21 現查：push run `35487889179`，headSha 即 HEAD 本身，conclusion=success。**本機仍未接線**：唯一執行者＝`python tools/run_shellcheck.py`（載具缺席時 fail-loud），尚未接進 pre-push（見 `DEF-101-938`，接線層級待拍板） |
>
...
> |---|---|---|---|---|
> | `windows-nightly-full` | windows-compat-ci.yml | 🔴 **failure** | `34844265895` ／ `33b9470f` | 2026-09-21 現查（2026-09-14 排程 run）：job 層結論「Windows nightly full suite」為 failure，失敗 step＝「執行 local_ci_gate.ps1」；pytest 本身零 failed（`[pytest] FAIL` 但無 failed 測試），紅因＝`local_ci_gate.check_skip_census` 對**未登記剖面** `win32+nopg+solo` 判定失敗（與 DEF-200-183 同型；掌舵者裁決修好剖面軸前不登記）⇒ 結構性紅，修 DEF-200-183 前每週都會紅 |
> | `macos-nightly-full` | macos-compat-ci.yml | 🔴 **failure** | `34852978108` ／ `33b9470f` | 2026-09-21 現查（2026-09-14 排程 run）：job 層結論「macOS nightly full suite」為 failure，失敗 step＝「執行 local_ci_gate.sh」；pytest 本身零 failed（4596 passed, 222 skipped）；紅因同上列（剖面 `darwin+nopg+solo` 未登記，DEF-200-183） |
>
...
> **確認本錨沒有 `pending` 欄**。
>
> <!-- cloud-ci-status: checked-at=2026-09-21T00:44:53+08:00 head-sha=2206a3a0dcd599770ab54770e79c0fd38ed2b42b red=none nightly-red=windows-compat-ci.yml:windows-nightly-full,macos-compat-ci.yml:macos-nightly-full nightly-run=34844265895 nightly-checked-at=2026-09-21T00:44:53+08:00 ／ 由上方 gh 指令現查後手動回填，回填步驟見上方六步 SOP；`tools/tests/test_doc_loc_baseline_freshness_r60.py` 的 TestR74CloudCiStatusIsRecorded 機械守。🔴 **判準清單一律以該檔的 `cloud_status_problems` 為準，本行不再列舉**（先前這裡逐項抄了一份判準，其中「新鮮度」那一項在拿掉整層判準之後就成了假話，而抄本沒有任何東西在守——同一份知識住兩個家、只有一個家被鎖）。🔴 本錨現值＝**2026-09-21 重查一次的結果**：現查 HEAD（`2206a3a0`）對應的 push run，七支帶 `push:` 觸發的 workflow 裡有六支這次觸發、conclusion 皆為 success，故 `red` 欄填 `none`；`autoclaude-mutation-on-change.yml` 因 `paths:` 過濾本次未觸發，commit 欄照實記其最近一次觸發 push run（commit `be53ff0e`）的 sha，見表格判讀欄。🔴 提醒：日後若再出現 push 軌 `failure`，本欄記的是 run 層 conclusion 本身——即使查出成因是 GitHub Actions 帳務停擺型未啟動而非程式碼紅，也**不得**因此把 `red` 欄填成綠，這正是本表存在的理由。`pending` 欄不填：本次查的就是 HEAD 自己，沒有任何「已 push 而結論未進表」的較新 commit。`granularity` 欄亦已刪除——`checked-at` 現在帶完整時間與時區，再自陳「只到日」就是一句與同一行資料矛盾的假話，該矛盾另有判準守。🔴 **表③-b（機器面）取樣的是排程軌 job 層、不是 push 軌 run 層**，故它的新鮮度與本行其餘欄位**刻意不同步**（週頻 vs 每次 push），逐列 provenance 寫在表③-b；現值指向的是 2026-09-21 重查的結果：對 `windows-compat-ci.yml`／`macos-compat-ci.yml` 各自最近一次**排程軌** run（2026-09-14，皆對 commit `33b9470f`）現查 job 層，`windows-nightly-full`／`macos-nightly-full` 兩列同為 failure——pytest 本身零 failed，紅因＝`local_ci_gate.check_skip_census` 對未登記剖面 `win32+nopg+solo`／`darwin+nopg+solo` 判定失敗（與 `DEF-200-183` 同型；掌舵者裁決修好剖面軸前不登記）⇒ 結構性紅，那筆紅在 run 層看不見（`continue-on-error` 吸收），正是它存在的理由。🔴 **這一欄自己也帶時點與出處**（`nightly-run` 是那一次 schedule run 的 databaseId、必須逐字對得上表③-b；`nightly-checked-at` 是查核時點，逾 14 天即紅）——沒有這兩欄的話，本欄只保證「有人查過一次」而不是「現在是什麼狀態」，於是「修好之後沒人回來清」與「新的紅沒人補進來」兩個方向都不會有任何東西出聲。🔴 **本行的說明文字刻意不寫出任何 `欄位＝值` 形態的字樣**：機器欄位與人讀散文同住這一行，散文裡只要出現可被解析的那種寫法就會覆蓋真欄位值（此前已實測踩到，其中一次是由該筆的根治判準自己抓出來的）。🔴 下一次 push 之後本表即成為 dated snapshot，收輪檢查清單第一項＝重跑上方 gh 指令、確認本表每一列與錨都對得上現況（該項無 rc 級機械物，且刻意不做成 CI 判準——會不可滿足，見上方分工表與 TestR75CloudCriteriaAreSatisfiableAtAnyCommit）。刪除本標記會讓該鎖 fail-loud -->
>
...
# 🔴 DEF-200-274 X1：PG 在場時必須加 --dist loadgroup——pyproject.toml 的 addopts 預設
# 帶 -n auto --dist worksteal，多個 xdist worker 若同時觸碰這顆 PG 會造成競態；
# conftest.py 已有 fail-loud 的 UsageError 兜底（漏加會直接報錯並教你怎麼修），
# 這裡直接示範正確用法。除錯／單檔小範圍可改 -n 0（見 §5）。
...
# ④ 照常跑（PG 在場需 --dist loadgroup，理由同左欄 bash 版註記，DEF-200-274 X1）
python -m pytest tests/ -q --dist loadgroup
Pop-Location
```
...
python -m pytest tests/ -q --dist loadgroup | tail -1      # 第二次：容器已 healthy（PG 在場須 --dist loadgroup，DEF-200-274 X1）
```
...
Remove-Item Env:\AUTOCLAUDE_NO_PG_AUTODETECT; python -m pytest tests/ -q --dist loadgroup | Select-Object -Last 1
```
