...
>
> | 量測項 | macOS（既有，見下方 R57 註） | **Windows 11（R59 實測）** | 差異歸因（實測，非推算） |
> |---|---|---|---|
> | AutoClaude `pytest tests/ -q` | 3738 passed / 210 skipped | **3740 passed / 208 skipped** | 兩平台的 **passed+skipped 結果總數相同**（3948）〔**二審 SA 訂正用詞**：`--collect-only` 實測為 **3945**，差額來自兩個模組級 `pytest.importorskip("sqlalchemy")`——本機 sqlalchemy ABSENT 故整檔跳過、collect 少算但跑起來各記 1 skip。原寫「收集總數」會讓照本節驗證的人跑出 3945 而誤以為少了 3 支〕。Windows 上 `[WINDOWS-NATIVE-ONLY]` 標籤的 3 支（`test_perception.py::TestCloseKillsCmdShimGrandchild` 1 支、`test_run_local_nightly_static.py::TestConcurrencyGuardBehavior` 2 支）由 skip 轉 pass；反向新增 2 支 Windows 專屬 skip（`test_perception.py` POSIX process-group、`test_sdd_to_playbook_adapter.py` 無 symlink 權限 `[WinError 1314]`）。**殘差 ±1 未歸因**——需一台 macOS 同時量測才能對帳，本節依既定紀律**不做加減推算**、只填實測值 |
> | 根層 `tools/run_root_unittests.py` | 616 tests OK / skipped=4（R57 macOS 量測，本輪未在 macOS 重測） | **661 tests OK / skipped=10** | 收集總數本輪由 616 增至 661（本輪新增回歸鎖，含四方複審條件補的鎖），兩平台收集總數相同、差異只在 skip。Windows 的 10 支全為 POSIX-only 或 macOS-only 語意：8 支 pgid/killpg/SIGINT（`test_dev_start.py`）、1 支 `install_mac_nightly.sh` BSD `stat -f` 心跳等價鎖、1 支無 symlink 權限（`[WinError 1314]`）。**R59 動工時為 11 支**——其中 1 支是 DEF-101-509（語法閘門因無 pwsh 7 而 skip），本輪修掉後轉為實跑 |
> | AISDLC_SDD `ci-gate` v0.01 | 1478 | **1478**（相同） | v0.01 的 3 支 docker 測試無 Windows 排除；本機 docker 執行中故全跑 |
> | AISDLC_SDD `ci-gate` v0.30 | 1729 | **1725** | −4 ＝ 2 支 `test_phase_h.py` `requires_docker_success`（**`sys.platform.startswith("win")` 硬排除，與 docker 是否可用無關**，見 DEF-101-062）＋ 2 支 `test_post_commit_drift_worktree.py`（POSIX shebang hook chain，`skipif(win)`） |
> | AISDLC_SDD `ci-gate` scripts/tests | 244（R57 記載） | **248** | R59 動工時實測 245（比 R57 記載 +1，未追查是平台差異或 R57 收尾後的回填落差）；收尾為 **248**＝再 +3，即 DEF-101-512 的兩道降級 fallback 鎖 + QA-R59-10 的 `-rs` 鎖 |
> | `check_loc_budget` / `lint-imports` | total=20356 cap=20438 violations=0 ／ 8 kept 0 broken | **完全相同** | 純靜態分析，無平台差異 |
>
...
>
> **本表的量測時點宣告（R59 SA 複審 SA-R59-01 訂正）**：上表四組數字皆為 **R59 收尾工作樹**（所有並行 agent 與四方複審 agent 停工後）由主控一次性重測所得，非中途快照。**訂正紀實**：本表初版誤填了「根層 616 / scripts/tests 245」兩個**動工時**的中途值，與同輪缺陷帳本 DEF-101-519 自己記的 653 互相矛盾——這正是本節 R56 註記警告過、且本輪 DEF-101-515 要根治的同一形態在修復內部原地復發（且 `MIN_TESTS` 的 ratchet 警戒線 616×1.10=677.6 > 653，**沒有任何機械物會抓到它**，是靠 SA 人工對帳抓出）。
>
