...
> | `shellcheck-ci.yml` | ✅ success | `59e8bf9` | 🔴 **本格原記「雲端尚未執行過／零次 run」，該宣稱已過期**：那是 R80 落地當下（工作樹還沒 push）的瞬間快照，寫死後沒人回頭訂正。現查 `gh run list --workflow shellcheck-ci.yml --limit 10 --json headSha,status,conclusion,createdAt` → 自 2026-08-08 起已跑 ≥10 次，**其中 8 次 failure**，最近兩次（`b0d2528`／`59e8bf9`）才轉綠 ⇒ 本格填最近一次的 sha 與結論。訂正輪＝R128（`DEF-101-938` 勘查途中發現）。**本機仍未接線**：唯一執行者＝`python tools/run_shellcheck.py`（載具缺席 rc=2 fail-loud），尚未接進 pre-push（見 DEF-101-938，該筆已有裁決方向但接線層級待拍板） |
>
...
> |---|---|---|---|---|
> | `windows-nightly-full` | windows-compat-ci.yml | 🔴 **failure** | `33397708097` ／ `39d2a4a6` | 2026-09-05 現查：job 層 `steps=11`、真跑到「執行 local_ci_gate.ps1」那一步才紅 ⇒ 不再是先前的帳務停擺型未啟動。pytest 本身零 failed；紅因＝`local_ci_gate.check_skip_census` 對**未登記剖面** `win32+nopg+solo` 判 rc=1（與 DEF-200-183 同型；掌舵者裁決修好剖面軸前不登記）⇒ 結構性紅，修 DEF-200-183 前每週都會紅 |
> | `macos-nightly-full` | macos-compat-ci.yml | 🔴 **failure** | `33406416207` ／ `39d2a4a6` | 2026-09-05 現查：job 層 `steps=9`、同樣紅在「執行 local_ci_gate.sh」；pytest 零 failed；紅因同上列（剖面 `darwin+nopg+solo` 未登記，DEF-200-183）。本機 mac nightly 的 autoclaude_gate 同日同因 |
>
