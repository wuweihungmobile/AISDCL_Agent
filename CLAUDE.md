...
#### 並行派工防互踩檢查表（掌舵者 2026-09-01 D3 裁決明文化＝DEF-101-886 的檢查表形態解）

派工前逐格勾，任一格不成立＝改單人串行或換形態派工（機械物＝`tools/tests/test_doc_loc_baseline_freshness_r60.py` 的 `TestParallelDispatchChecklistIsPinnedInClaudeMd`）：

1. **結案單線**：三本帳（缺陷／外部阻塞／結構性長債）的結案編修只准單一窗口做；並行包一律禁寫帳本。
2. **修復棒串行**：動同一鎖持有面的修復包不得並行——前一棒收工且主控親驗後才派下一棒。
3. **複審唯讀**：複審鏡全程唯讀＋禁自起背景長任務；審查期間主控不動工作樹（審查 agent 的背景子任務未確認全部結束前，視同審查窗口未關——同 session 實測：窗口誤判一次換來流程性 REJECT）。
4. **收尾單人窗口**：淨減法（刪檔／合併鎖檔／`git rm`）與守衛線重釘儀式只准收尾單人窗口做。
