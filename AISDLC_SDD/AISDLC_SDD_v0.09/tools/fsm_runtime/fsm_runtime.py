...
class FSMRuntime:
    ...
    def rule_fire_telemetry_stats(self) -> dict:
        ...
        原則（純讀、不渲染大圖、不碰 meta-oracle）。**誠實揭露 catch_side_wired=False**：本輪
        只接 fire 側（on-watch 計數），catch 側語意未定義（DEF-18-001）故 ROI 為單側信號，
        生產啟用退役前須先定義 catch 契約。度量穩健：零規則不除零（回空清單 / 0）。
        """
        from . import rule_loader as _rl  # local import：避 top-level cycle
        ...
        return {
            ...
            "safety_certificate": {
                "auto_retire": False,
                "catch_side_wired": False,
                "human_gate": "rule_loader.set_maturity(reviewed_by=...)",
                "well_founded": (
                    "fire 遙測只增計數、永不 set_maturity（R-9.20 #11）；catch 側語意未定義"
                    "（DEF-18-001）故 ROI 為單側信號，退役提議仍 🔴 人工 review，生產啟用退役"
                    "前須先定義 catch 契約 — 遙測在環上守界、不自走退役。"
                ),
            },
        }
