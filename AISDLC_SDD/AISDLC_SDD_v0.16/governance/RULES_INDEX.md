...
>
> **遷移狀態（2026-06-12 v0.02 更新，DEF-01-001 修正）**：**Phase 1 抽取完成** —
> Rule 9.x 已全數抽成結構化 `R-*.yaml`（R-9.1~9.38 共 38 條 + R-SELF-STRIDE =
> **39 檔**，以 `ls governance/rules/` 磁碟實數為準），由
...
| R-9.38 | Phase Z Playbook 翻譯保真（SDD AT↔AutoClaude step 100% 雙向映射；缺漏/多餘 → SPEC_AUDIT；evaluator 白名單模板 + weak_regex audit 留痕） | IMPLEMENTATION / PR_REVIEW | high | active | R-9.38-playbook-translation-fidelity.yaml |
...
- 分配新號前必查：`python -m tools.fsm_runtime.id_registry next-act`（現為 **ACT-172**）／`next-rule`（現為 **R-9.39**）。
