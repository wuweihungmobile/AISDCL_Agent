...
class ChaosScenarioTests(unittest.TestCase):
    ...
    def test_infinite_scg_failure_terminates_in_3_retries(self) -> None:
        ...
        """
        # v0.22 預設 AUTO_RECOVERY=ON（improving_57 B 軌常態化）；本測驗純 Rule 9.1
        # retry budget→escalation（與 auto-recovery 正交），opt-out 隔離。預設 ON 之
        # 有界停機由 test_100_rounds_are_all_bounded 等 bounded 場景在 ON 下覆蓋。
        os.environ["SDD_ENABLE_AUTO_RECOVERY"] = "0"
        self.addCleanup(os.environ.pop, "SDD_ENABLE_AUTO_RECOVERY", None)
    ...
    def test_retry_tamper_cannot_bypass_escalation(self) -> None:
        ...
        # v0.22 預設 AUTO_RECOVERY=ON；本測驗 tamper 後 retry budget 仍→escalation
        # （與 auto-recovery 正交），opt-out 隔離（improving_57 B 軌）。
        os.environ["SDD_ENABLE_AUTO_RECOVERY"] = "0"
        self.addCleanup(os.environ.pop, "SDD_ENABLE_AUTO_RECOVERY", None)
