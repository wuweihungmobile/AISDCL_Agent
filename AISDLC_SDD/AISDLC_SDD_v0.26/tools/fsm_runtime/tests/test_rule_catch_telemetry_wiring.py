...
flag-gated、fail-closed（缺證據不記、不污染 ROI）、紅線守界（只增 catch_count，退役仍 🔴 人工
set_maturity）。

improving_62（v0.24，B L5 telemetry 活體化）：flag 預設由 OFF 翻為 **ON**（unset → escalation
歸因即記 on-attribution catch 活體；顯式 falsy=0/false/no/off → opt-out 還原 v0.09 不記 catch）。
Case 1 改寫為 default-ON 活體驗收、Case 2 為顯式 opt-out 零退化守。
（測試套 conftest autouse 預設把兩 flag 設 "0" 保護凍結 governance；default-ON 案以 delenv 覆寫。）

每個 case 編碼「為何此行為重要」（Rule 9）：default-ON 活體記 catch、顯式 opt-out 零退化、
helper 真記（非半接）、monitor violation 呼叫點真接（R-9.21）、fail-closed 不阻塞、R-9.20 #11
紅線、要件①無 failure_mode 不歸因、寧缺勿濫空歸因不記、helper 子集語意、**持久化陷阱回歸鎖
（fire round-trip 不抹 failure_mode）**、stats 覆蓋率誠實揭露。
"""
...
# ---------- Case 1：default ON（unset）= L5 活體記 catch（improving_62 翻環）----------

def test_default_on_records_catch(tmp_path, monkeypatch, _catch_rules):
    """改寫自 v0.23 Case 1（原斷言 delenv→catch 0）：v0.24 翻環後 unset = 預設 ON。

    為何重要（Rule 9 / R-62-2）：B L5 catch 側遙測活體化的**定義性驗收**——env 完全未設
    （production 常態）時，escalation 歸因即對 failure_mode 已定義的規則記 on-attribution catch，
    無需手動 opt-in。（conftest autouse 預設把 flag 設 "0"；此處 delenv 覆寫回 unset 以驗預設 ON。）
    """
    monkeypatch.delenv("SDD_ENABLE_RULE_CATCH_TELEMETRY", raising=False)  # 覆寫 conftest，unset = 預設 ON
    rt = _rt_in_init(tmp_path)
    rt._record_escalation_catches(["R-9.1"])
    assert _catch_count(_catch_rules, "R-9.1") == 1, "v0.24 預設 ON：unset 即記 catch（活體化）"


# ---------- Case 2：顯式 opt-out（=0）= 還原 v0.09 不記 catch（零退化逃生閥）----------

def test_explicit_opt_out_records_no_catch(tmp_path, monkeypatch, _catch_rules):
    """顯式 opt-out（flag='0'）= 還原 v0.09 不記 catch 行為（零退化逃生閥）。
    ...
    rt = _rt_in_init(tmp_path)
    rt._record_escalation_catches(["R-9.1"])
    assert _catch_count(_catch_rules, "R-9.1") == 0, "顯式 opt-out 不記 catch（零退化）"
