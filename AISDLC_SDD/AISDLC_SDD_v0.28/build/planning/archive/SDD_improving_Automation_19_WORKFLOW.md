# SDD_improving_Automation_19_WORKFLOW — Phase S 動態自我修正執行工作流

**用途**：詳細執行 [`SDD_improving_Automation_19.md`](SDD_improving_Automation_19.md)（Phase S：生成文法詞彙自我擴充 meta⁴ + 多維度批次退役聯動）的 **dynamic workflow**——一套「生成 → 評估 → 合約談判 → 修復 → 停機」的自我修正 Agentic 閉環，把藍圖 §6 檢核清單逐項落地並以**客觀守門**（pytest / 五軌 TLC / chaos / QA 抓漏）驗收。
**建立日期**：2026-06-04
**前置**：藍圖 §0~§6 已撰；基線確認 pytest 1168 passed / 4 skipped / 14 subtests、META_FSM TLC 13 distinct No error、next_free=ACT-135 / R-9.31。
...
| `EVALUATE` | 跑客觀守門：`pytest`（相關子集）+（ACT-137/140）五軌 TLC + chaos | 全綠 → PASS；否則 FAIL | §3.1 驗收 |
...
| `VERIFIED` | 全 ACT + QA 全綠、1168→新基線不回歸 | → TAG_PUSH_MERGE | §6 |
...
> **Self-Verification 對齊（藍圖 §5）**：若某 ACT 的「驗收契約」本身寫錯（例如要求「自指自利字應被採納」這種矛盾標準），EVALUATE 會持續 FAIL；FIX 重試觸頂後**不**降級成「註解掉測試」，而是 `HALT_ESCALATION` 導人類修正驗收標準——人類維持「設計環境舵手」高度，而非被降級成修碼員。

---

## 3. 漸進式揭露與上下文管理（Context Degradation 防護）

- **每 ACT 隔離**：CONTRACT_NEGOTIATION 只載入該 ACT 需要的範本（Phase R 對應元件）+ 藍圖該 ACT 段落；不一次載入全部。
...
1. **Pillar A**：ACT-135（vocabulary_genesis + 有界詞彙生成文法 + 詞彙自指守門）→ CONTRACT_NEGOTIATION（寫 test_phase_s ACT-135 段）→ GENERATE → EVALUATE（pytest 子集綠）
2. **Pillar B**：ACT-136（feature-grounded oracle evaluate_genesis_feature + VOC 語料）→ 同上循環
3. **Pillar C**：ACT-137（meta_ledger vocab-genesis stock / batch-cadence + guard_vocab_genesis / guard_batch_swap + batch_swap_dimensions + META_FSM.tla/.cfg）→ EVALUATE（pytest + **META_FSM TLC 13 distinct 重證**）
4. **Pillar D**：ACT-138（steersman 詞彙外發明 diff + 批次重構 diff）→ EVALUATE
5. **收官**：ACT-139（治理 + ID 翻牌）→ ACT-140（**五軌 TLC + chaos 100 輪 + 全量 pytest 不回歸**）
...
| 守門 | 客觀通過條件 |
|------|-------------|
| pytest | `1168 passed`（基線）→ 新增 ~50~65 全綠、4 skip 不變、0 回歸 |
| 五軌 TLC | `SDD_FSM`/`META_FSM`(13 distinct)/`FLEET_FSM`/`COMPOSITION_FSM`/`OPTIMIZATION_FSM` 全 `No error`（不增第六軌） |
| chaos | 100 輪 `bounded_ratio==1.0`、`avg_tokens < 20000`，含 `VOCAB_GENESIS_GOODHART_FLAP` + `BATCH_SWAP_THRASH_FLAP` |
| ID 一致性 | `python -m tools.fsm_runtime.id_registry validate` → `[OK]`；next_free 翻 ACT-141 / R-9.32 |
