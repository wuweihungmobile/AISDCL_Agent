...
# ---------- Case 1：v0.25 預設 ON = 鷹架代謝活體（delenv 覆寫 conftest opt-out）----------

def test_default_on_enter_auto_runs_gc(tmp_path, monkeypatch, _redirect_gc_report):
    """unset（delenv 覆寫 conftest 測試套 opt-out）→ 預設 ON：進態自動跑 GC = v0.25 活體常態。

    WHY（Rule 9）：v0.25 把 SDD_ENABLE_SCAFFOLD_GC_AUTO_PROPOSE 預設翻 ON（B L5 鷹架代謝活體化）。
    本測試 delenv 還原「使用者未設環境變數」的真實 production 預設，驗證 enter_scaffold_gc 在預設
    下即自動跑 run_gc、填 origin=auto / roi_report_ref（鏡像 W-15/16/18/19 翻環的 default-ON 活體
    驗收）。報告以 _redirect_gc_report 導向 tmp，不污染框架 build/reports/gc。
    """
    ...
    assert result["auto_gc"]["proposed"] is True, "unset → 預設 ON 須自動跑 GC（v0.25 活體化）"
    tracking = rt.state.root["scaffold_gc_tracking"]
    assert tracking["origin"] == "auto"
    assert tracking["roi_report_ref"], "預設 ON 須填 roi_report_ref（活體信號）"


# ---------- Case 2：顯式 opt-out（=0）零退化逃生閥（還原 v0.07 只記 tracking）----------

def test_explicit_opt_out_pure_tracking_no_auto_gc(tmp_path, monkeypatch):
    """flag 顯式 '0' → 還原 v0.07：只記 tracking、無 auto_gc 鍵、不產報告（零退化逃生閥）。

    WHY（Rule 9）：翻環後仍保留顯式 opt-out，供保守/除錯場景關閉自動 GC。設 "0" 須與翻環前的
    純記 tracking 行為逐字一致（無 auto_gc 鍵、tracking 無 roi_report_ref），證明翻環只改「預設」、
    不剝奪「關閉」能力。
    """
    monkeypatch.setenv("SDD_ENABLE_SCAFFOLD_GC_AUTO_PROPOSE", "0")
    rt = _rt_in_release(tmp_path)
    result = rt.enter_scaffold_gc()
    assert "auto_gc" not in result, "顯式 opt-out 不得自動跑 GC（零退化）"
    tracking = rt.state.root["scaffold_gc_tracking"]
    assert "roi_report_ref" not in tracking, "顯式 opt-out 不得產報告"
