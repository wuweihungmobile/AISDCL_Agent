...
# nfr_id is optional in REQUIRED_FIELDS (auto-derived when absent via
# map_metric_to_nfr()) but is used to compose the PBS-DRIFT report filename
# (see _report_path()). If it were excluded from the signed-fields set, an
# attacker could tamper with nfr_id on an otherwise-validly-signed event
# without invalidating the signature. Include it here so it always falls
# inside the HMAC coverage.
