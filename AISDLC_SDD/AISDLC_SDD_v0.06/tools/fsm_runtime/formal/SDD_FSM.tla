...
ObservationStates == {
    ...
    "AUTOCLAUDE_DELEGATED"      \* Phase Z / ACT-172 ← SDD→AutoClaude 委派執行觀測態
}
...
(* IMPLEMENTATION 期間把實作子任務委派給 AutoClaude playbook 引擎執行。入口僅從
   IMPLEMENTATION（enter_autoclaude_delegated；IMPLEMENTATION happy-path 出邊集不含本態，
   遵循觀測態入口慣例）；出口 done→IMPLEMENTATION（委派完成帶回 resume）/
   failed→ESCALATION（AutoClaude 側演化亦失敗/越閘 → 升級）。
   IMPLEMENTATION↔AUTOCLAUDE_DELEGATED 2-cycle 比照 SPEC_PATCH↔EXPERIMENT_REPLAY，
   由 SF_vars(T_AutoDelegToImpl) 破環（見 Fairness），不引入新非 terminal cycle。 *)
AutoDelegSources == {"IMPLEMENTATION"}
T_EnterAutoclaudeDelegated == \E src \in AutoDelegSources :
    /\ state = src
    /\ state' = "AUTOCLAUDE_DELEGATED"
    /\ retry' = retry
    /\ recovery' = recovery
    /\ compact' = compact
    /\ hub' = hub
T_AutoDelegToImpl == JumpKeep("AUTOCLAUDE_DELEGATED", "IMPLEMENTATION")
T_AutoDelegToEsc  == JumpKeep("AUTOCLAUDE_DELEGATED", "ESCALATION")
...
Next ==
    ...
    (* Phase Z — SDD→AutoClaude 委派執行觀測態 *)
    \/ T_EnterAutoclaudeDelegated \/ T_AutoDelegToImpl \/ T_AutoDelegToEsc
    (* Emergency / recovery *)
...
Fairness ==
    ...
    (* Phase Z / ACT-172：破 IMPLEMENTATION↔AUTOCLAUDE_DELEGATED 2-cycle。委派反覆
       進出時，SF 強制 AUTOCLAUDE_DELEGATED 終究回 IMPLEMENTATION（done 出口），
       不被委派 2-cycle 永久困住，保 ObservationsTransient / EventuallyTerminal。*)
    /\ SF_vars(T_AutoDelegToImpl)
