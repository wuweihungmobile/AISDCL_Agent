...
# W-16-1 / B-axis L4→L5：規則自演化「自動提議」開關（同 _AUTO_RECOVERY_ENV env-flag 慣例；
# FSMRuntime.__init__ 無 config 物件）。
# v0.23（improving_59，B L4→L5 活體化）：預設由 OFF 翻為 **ON**——規則自演化「自動提議」由
#   「失敗即停等人手動跑 CLI」(opt-in 鷹架) 升為「learn 出口自動 draft proposed 草案」(活體常態)。
#   鏡像 W-15 AUTO_RECOVERY（B L3→L4）的翻環先例：轉態邊與 _HAPPY_PATH/*.tla **零變更**
#   （improving_15/16 已模型化、五軌 TLC 已證有界），本輪僅翻轉「誰觸發」的預設。仍保留**顯式
#   opt-out**（SDD_ENABLE_SLV_AUTO_PROPOSE=0/false/no/off → 還原 v0.06 純轉態行為），供除錯/保守
#   場景關閉。**紅線零弱化**：草案恆 trust_level=proposed（R-9.11），trust_level→verified 仍
#   🔴 人工（exit_learning_commit 既有強制檢查不動）；fail-closed 不偽造；gate 仍需 `fpl_id`
#   （無 fpl_id → 純轉態，與翻環前逐字一致）。
_SLV_AUTO_PROPOSE_ENV = "SDD_ENABLE_SLV_AUTO_PROPOSE"


def _slv_auto_propose_enabled() -> bool:
    """True 為預設（unset → ON，v0.23 活體化）；僅顯式 falsy（0/false/no/off）→ OFF opt-out。"""
    val = os.environ.get(_SLV_AUTO_PROPOSE_ENV, "").strip().lower()
    if val == "":
        return True  # v0.23 預設 ON（B-axis L4→L5 自演化活體化）
    return val in {"1", "true", "yes", "on"}


# W-17-1 / B-axis L5（鷹架代謝「自動提議退役」開關，鏡像 _SLV_AUTO_PROPOSE_ENV）。
# v0.25（improving_63，B L5 鷹架代謝活體化）：預設由 OFF 翻為 **ON**——進 SCAFFOLD_GC 態自動跑
#   既有 run_gc，依 scaffold_roi（fire_count/catch_count，v0.24 fire/catch 遙測活體化後真實累積）
#   算 ROI 並落 build/reports/gc/SCAFFOLD-ROI-{date}.md 的 proposed 退役提議，由「失敗即停等人手動
#   跑 GC」(opt-in 鷹架) 升為「進代謝態自動 draft proposed 草案」(活體常態)。閉合 arch_fitness
#   FF-16 GAP-X2（「鷹架代謝 GC 從未產退役 ROI 提案」）。鏡像 W-15 AUTO_RECOVERY（B L3→L4）/
#   W-16 SLV_AUTO_PROPOSE（B L4→L5）/ W-18/19 telemetry（v0.24）翻環先例：進態後純 side-effect、
#   零新增 reachable 邊、不碰 *.tla（_HAPPY_PATH 零變更，免五軌 TLC），本輪僅翻轉「預設」。仍保留
#   **顯式 opt-out**（SDD_ENABLE_SCAFFOLD_GC_AUTO_PROPOSE=0/false/no/off → 還原 v0.07 只記 tracking、
#   不算 GC 行為），供保守/除錯場景。**紅線零弱化**：run_gc 只產 RetirementProposal + Markdown 報告、
#   **永不呼叫 set_maturity**（active 規則退役仍 🔴 人工，R-9.20 #11）；fail-closed（run_gc 失敗不阻塞
#   已落定的進態、不偽造報告）。
_SCAFFOLD_GC_AUTO_PROPOSE_ENV = "SDD_ENABLE_SCAFFOLD_GC_AUTO_PROPOSE"


def _scaffold_gc_auto_propose_enabled() -> bool:
    """True 為預設（unset → ON，v0.25 活體化）；僅顯式 falsy（0/false/no/off）→ OFF opt-out。"""
    val = os.environ.get(_SCAFFOLD_GC_AUTO_PROPOSE_ENV, "").strip().lower()
    if val == "":
        return True  # v0.25 預設 ON（B-axis L5 鷹架代謝活體化）
    return val in {"1", "true", "yes", "on"}


# W-18-1 / B-axis L5（規則命中遙測「on-watch fire 記帳」開關，鏡像 _SLV_AUTO_PROPOSE_ENV 翻環）。
# v0.24（improving_62，B L5 telemetry 活體化）：預設由 OFF 翻為 **ON**——規則命中遙測由
#   「opt-in 鷹架（需手動設 env 才記 fire）」升為「每次 transition 進態後對 load_for_state(dst)
#   命中規則各記一次 on-watch fire」的活體常態，使 scaffold_roi.fire_count 真實累積（閉合
#   DEF-17-001 點名的「fire_count=0」根因，GC 才有非零資料驅動退役提議）。鏡像 W-15 AUTO_RECOVERY
#   / W-16 SLV_AUTO_PROPOSE 的翻環先例：純 transition 後 side-effect、零新增 reachable 邊、不碰
#   *.tla（_HAPPY_PATH 零變更，免五軌 TLC），本輪僅翻轉「預設」。仍保留**顯式 opt-out**
#   （SDD_ENABLE_RULE_FIRE_TELEMETRY=0/false/no/off → 還原 v0.08 不記 fire 行為），供保守/除錯場景。
#   **紅線零弱化**：只增 fire_count 計數、永不 set_maturity（active 規則退役仍 🔴 人工，R-9.20 #11）；
#   fail-closed（記帳失敗絕不阻塞已落定的轉態）。
...
# v0.24（improving_62，B L5 telemetry 活體化）：預設由 OFF 翻為 **ON**——escalation 真實發生且
#   呼叫端明確歸因（attributed_rule_ids）時，對 failure_mode 已定義的規則記 on-attribution catch
#   （閉合 DEF-18-001 catch 側語意未定義）由 opt-in 鷹架升為活體常態。鏡像 fire 側 / AUTO_RECOVERY
#   / SLV 翻環先例：純 escalation 後 side-effect、零新增 reachable 邊、不碰 *.tla（免五軌 TLC），
#   本輪僅翻轉「預設」。仍保留**顯式 opt-out**（SDD_ENABLE_RULE_CATCH_TELEMETRY=0/false/no/off →
#   還原 v0.09 不記 catch 行為）。**紅線零弱化**：只增 catch_count、永不 set_maturity（R-9.20 #11）；
#   fail-closed（缺證據不記、不污染 ROI；記帳失敗絕不阻塞已落定的 escalation）。
...
class FSMRuntime:
    ...
    def _record_escalation_catches(self, attributed_rule_ids: list) -> None:
        ...
        flag-gated（SDD_ENABLE_RULE_CATCH_TELEMETRY，v0.24 預設 ON 活體；顯式 opt-out=0/false/no/off
        → 還原 v0.09 不記行為）；fail-closed（記帳任何失敗絕不阻塞已落定的 escalation）。**無 FSM
        狀態副作用**＝純記帳、零新增
    ...
    def record_gate_result(self, gate: str, result: str, reason: str = "") -> dict:
        ...
        if escalate:
            ...
            # W-15-1 / B-axis L4：flag-gated 有界自動恢復接入。v0.22 起預設 ON（顯式 opt-out=0
    ...
    def exit_production_behavioral_signal(
        ...
        W-16-1 / B-axis L4→L5（規則自演化「自動提議」接入主迴圈）：當 decision=="learn"
        ∧ `_slv_auto_propose_enabled()`（flag `SDD_ENABLE_SLV_AUTO_PROPOSE`，**v0.23 預設 ON**、
        顯式 falsy 才 opt-out）∧ 提供 `fpl_id` 時，轉態到 LEARNING_COMMIT **之後**自動 draft 一份
        proposed 草案並填 `learning_commit_tracking`（使迴圈不再需人手動跑 CLI ＝ L5 自走信號）。
        ...
        LEARNING_COMMIT 不偽造、不轉錯態。flag 顯式 OFF 或無 fpl_id → 行為逐字同 v0.06（純轉態）。
