...
class RuntimeBootstrapTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        # v0.22 預設 AUTO_RECOVERY=ON（improving_57 B 軌常態化）；本類驗純 retry-limit→
        # escalation 轉態（與 auto-recovery 正交），顯式 opt-out 隔離該關注點。
        os.environ["SDD_ENABLE_AUTO_RECOVERY"] = "0"
        self.state_path = Path(self._tmp.name) / "FSM-STATE-test.yaml"

    def tearDown(self) -> None:
        os.environ.pop("SDD_ENABLE_AUTO_RECOVERY", None)
        self._tmp.cleanup()
...
class TamperedRetryCountTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        self._tmp = tempfile.TemporaryDirectory()
        # v0.22 預設 AUTO_RECOVERY=ON；tamper 防禦驗 retry-count 完整性→escalation（與
        # auto-recovery 正交），顯式 opt-out 隔離（improving_57 B 軌）。
        os.environ["SDD_ENABLE_AUTO_RECOVERY"] = "0"
