...
# DEF-19-002 通則化（improving_20 W-20-2）：原寫死 `AISDLC_SDD_v0\.0\*` 只認 v0.0* 單 glob，
# 迫使 ci-gate.sh 必須保留 v0.0* 子串才不誤報——結構耦合異味：ci-gate 想清掉舊 glob 改純通則化
# （v0.[0-9]* / v0.*）即被 FF-17 假紅擋下（improving_19 雙 glob 修復正是被此耦合逼著保留子串）。
# 改為接受任何 `ls -d AISDLC_SDD_v0.<glob>` 動態偵測慣用語：v0.0* / v0.[1-9]* / v0.[0-9]* / v0.*
# 皆命中（glob 首字元 ∈ 數字 / `[` / `*`），解除子串耦合、守住「動態偵測」真不變量本身。
_CI_GATE_LATEST_GLOB_RE = re.compile(r"ls\s+-d\s+AISDLC_SDD_v0\.[0-9\[*]")
...
def _latest_version_dir() -> Optional[str]:
    """回傳磁碟上語意版本最高的 AISDLC_SDD_v0.* 目錄名（無則 None）。

    DEF-19-002：glob 由 `v0.0*` 放寬為 `v0.*`，使版本跨越十位數（v0.10+）仍正確偵測為最新版
    （否則 detail 會誤報 v0.09 為最新、與 ci-gate 實測版本不符）。排序鍵已是數值切分，v0.10>v0.09。
    """
    dirs = sorted((p.name for p in PROJECT_ROOT.glob("AISDLC_SDD_v0.*") if p.is_dir()),
...
def check_ff17_evolution_version_gate_coverage(report: FitnessReport) -> None:
    ...
    if latest is None:
        report.add(Finding(
            ...
            title="略過演化版閘門涵蓋檢查", detail="磁碟上無 AISDLC_SDD_v0.* 版本目錄。"))
        return
