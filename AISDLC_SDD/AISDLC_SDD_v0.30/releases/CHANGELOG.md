...
## [v0.30] - 2026-06-30（Copy-on-Evolve 自 v0.29；v0.29 凍結唯讀）

> 使用者請求：推進整合迭代（improving_98）。掌舵者已於 2026-06-30 AskUserQuestion 裁定本輪 W 項＝P3 清理批次、並裁 DEF-62-001 真修＝Copy-on-Evolve v0.30 校正註解。B 軌＝P3 doc-lag 校正；A/C 軌不動，`L_合體=min(A,B,C)` 本輪維持不變（純文件債清理，不新增自治能力）。

### auto_recovery call-site 滯後註解極性校正（DEF-62-001 真修，純註解零行為變更）
- **`tools/fsm_runtime/fsm_runtime.py:420-421`**（唯一改處）：「預設 OFF＝行為同 v0.05（停在 ESCALATION 等人）」→「v0.22 起預設 ON（顯式 opt-out=0 才還原 v0.05 停在 ESCALATION 等人）」，與同檔 L39 檔頭、L51-55 `_auto_recovery_enabled()` 極性一致。
- **根因**：auto_recovery 自 v0.22（improving_57）翻預設 ON，L420 call-site 註解未隨翻環同步、逐版繼承至 v0.06~v0.29（improving_62 Architect 鏡揪出之跨輪文件債）。純註解時間差，零執行語意（`_auto_recovery_enabled()` unset→True 本就正確）。
- **同時 dogfood improving_97**：本版以 `copy_on_evolve.sh` 建立，三個建版後自動同步 block（戳記/鏡像、.gitignore、**FRAMEWORK_STATUS.md 重生**）全自動跑＝DEF-96-001 修復跨版實證。

### 驗證
- 見 improving_98.md §5（ci-gate exit 0 含 v0.01 + v0.30 雙軌；v0.30 not-chaos ＝ floor v0.29:1665；AutoClaude 端零碰、≥3607 passed；三鏡 zero-trust）。
- **FSM/`*.tla` 對 v0.29 逐位元零差異 → Rule 9.18.1 無重跑義務**（純改一行 Python 註解，未動 `_HAPPY_PATH`/狀態/形式化模型；不取新 ACT/Rule）。

## [v0.29] - 2026-06-29（Copy-on-Evolve 自 v0.28；v0.28 凍結唯讀）

> 使用者請求：推進整合迭代（improving_96）。掌舵者已於 2026-06-29 AskUserQuestion 裁定本輪 W 項＝候選 (a) DEF-95-002 修。A 軌＝PRD→playbook 橋接 backend-robust 化；B/C 軌不動，`L_合體=min(A,B,C)` 本輪維持不變（橋接把關方式加固，不新增自治能力）。

### Archy doc/spec 步 evaluator 政策：keyword 回顯 → artifact-existence（DEF-95-002 修復）
- **`agent/specialized/sdd-prd-to-playbook-zh.yaml`**（唯一改檔）：core_principles 補「evaluator backend-robust 化」原則；outputs 契約改寫——doc/spec 產檔步改用 artifact-existence evaluator `python -m autoclaude.artifact_check <相對產出檔> --min-bytes N`（檢查檔案真的產生、後端無關），`expected_output_regex` 留空、不靠 keyword 回顯；version 欄 v0.28→v0.29。
- **根因**：improving_95 端到端真跑揭露 pty 後端 `--output-format json` 對寫檔步擷取脆弱（DEF-81-001 族）會吞掉完成 keyword，致 SPEC.md 明明寫對（4151 bytes）卻 regex 不過 → escalated（pty 0/5、sdk 5/5）。
- **配合 AutoClaude 端（C 軌，非框架本體）**：新增 `autoclaude/artifact_check.py`（pip 套件內葉 CLI，任意 cwd 可解析）+ bridge fixture E-SPEC-1 改 artifact evaluator + 連帶測試更新 + sanitize 白名單回歸鎖。

### 驗證
- 見 improving_96.md §5（ci-gate exit 0 含 v0.01 + v0.29 雙軌；v0.29 not-chaos ≥ floor v0.28；AutoClaude 端 artifact_check 6 單測 + 橋接 harness 測試更新 + bridge e2e pty 後端真跑證 DEF-95-002 closed；三鏡 zero-trust）。
- **FSM/`*.tla` 對 v0.28 逐位元零差異 → Rule 9.18.1 無重跑義務**（純改 agent YAML evaluator 指引 + version 欄，未動 `_HAPPY_PATH`/狀態/形式化模型；不取新 ACT/Rule）。

## [v0.28] - 2026-06-27（Copy-on-Evolve 自 v0.27；v0.27 凍結唯讀）

> 使用者請求：在 AISDLC_SDD 建立專職 Agent，把 PRD 轉成「專案→目標→任務」playbook.yaml，搭配 AutoClaude + AISDLC_SDD 進行 PRD 完整產品開發（improving_94）。掌舵者已於 2026-06-27 AskUserQuestion 三問 signoff：①playbook 形態＝SDD 全流程驅動器；②三層編碼＝重用既有 three_tier_schema + 薄橋接；③流程＝直接完整入框架（Copy-on-Evolve）。A 軌＝PRD→playbook 雙向協作橋接（前端）。

### 新增 PRD→Playbook 前端規劃 agent（Archy）
- **`agent/specialized/sdd-prd-to-playbook-zh.yaml`**：讀 PRD → 產出 three_tier 結構 YAML（Project→GoalTask→ExecutionItem；goal_task=SDD 階段/產品目標、execution_item=驅動一道 SDD skill 的步驟 + evaluator 檢查該關 SCG 閘門產物），覆蓋 PRD→FRD→SRD→Contract→實作→RTM 全流程。
- **與 sdd-playbook-compiler（Compily）分界**：Archy＝前端（原始 PRD→整條開發路徑規劃）；Compily＝末端（凍結 TEST-CONTRACT-SPEC→測試 playbook）。兩者上下游互補、不重疊。
- **下游攤平**：產物交 AutoClaude `tools/three_tier_to_playbook.py`（C 軌，非框架本體）確定性攤平為可執行 `playbook.yaml`（workflow_type=aisdlc_sdd），每 task 帶 goal_task_id 供跨 run 進度彙總。
- **三處註冊**：`agent/README.md`、`AISDLC_SDD_INIT.md`（Specialized 19→20 + 2 橋接 agent 敘述 + Specialized Agents 清單表列）、EVOLUTION_LOG + 本 CHANGELOG。（cross-scenario bridge agent 不綁場景，故不進 auto_load_config 場景載入區。）

### 驗證
- 見 improving_94.md §5（ci-gate exit 0 含 v0.01 + v0.28 雙軌；v0.28 not-chaos ≥ floor v0.27；AutoClaude 端 model additive + compiler 26 單測 + 端到端攤平驗證；三鏡 zero-trust）。
- **FSM/`*.tla` 對 v0.27 逐位元零差異 → Rule 9.18.1 無重跑義務**（純新增 agent YAML + 註冊文件，未動 `_HAPPY_PATH`/狀態/形式化模型；不取新 ACT/Rule）。

## [v0.27] - 2026-06-27（Copy-on-Evolve 自 v0.26；v0.26 凍結唯讀）

> 使用者請求：推進整合迭代（improving_85）。掌舵者已於 2026-06-27 AskUserQuestion signoff「Copy-on-Evolve v0.27（閃閉端到端）」。A 軌＝spec-format-version「生產端」閃閉（雙向協作橋接加固，連動 B 柱 Copy-on-Evolve）；B/C 軌不動，`L_合體=min(A=L5,B=L5,C=L5)=L5` 本輪維持不變（橋接契約閉合，不新增自治能力）。

### spec-format-version 生產端宣告（TCS 模板，閉合 improving_67 防漂移閘的生產↔消費契約）
- **4 份 spec 生產模板各加宣告行 `**spec-format-version**: 1.0`**（皆為 AutoClaude `SddToPlaybookAdapter` glob `*CONTRACT*SPEC*` / `TCS-*` 命中、會被 `load_spec` 讀取的來源）：`docs_template/sdd/testing/` 下 `TEST-CONTRACT-SPEC-TEMPLATE.md`（主 TCS）、`CONTRACT-TEST-SPEC-INTEGRATION-TEMPLATE.md`、`CONTRACT-TEST-SPEC-MIGRATION-TEMPLATE.md`、`ENV-CONTRACT-SPEC-TEMPLATE.md`。
- **缺口**：improving_67 只建消費端 validator，框架模板從不宣告版本 → 真實 spec 永走靜默預設 1.0（`declared=False`），防漂移閘形同死碼。本輪補生產端使真實框架 spec 首次帶 `declared=True`，防漂移閘端到端可驗證。
- **格式選擇**：宣告行格式經 adapter `_SPEC_VERSION_RE`（`spec[-_ ]format[-_ ]version\D{0,4}(\d+\.\d+)`）實測匹配為 "1.0"；table-cell / backtick 格式經實測不匹配故避開。

### 驗證
- 見 improving_85.md §5（ci-gate exit 0 含 v0.01 + v0.27 雙軌；v0.27 not-chaos ≥ floor v0.26；4 模板宣告經真實 adapter regex 解析 "1.0"；AutoClaude 消費端 round-trip + MUT-85-1/2 突變非空殼；三鏡 zero-trust）。
- **FSM/`*.tla` 對 v0.26 逐位元零差異 → Rule 9.18.1 無重跑義務**（純 docs_template metadata 行，未動 `_HAPPY_PATH`/狀態/形式化模型；不取新 ACT/Rule）。

---

## [v0.26] - 2026-06-25（Copy-on-Evolve 自 v0.25；v0.25 凍結唯讀）

> 使用者請求：推進整合迭代（improving_66）。掌舵者已於 improving_65 §9（2026-06-25）signoff 批准 §8 CLI closure 方案。B 軌＝meta⁸ 可審批儀表板「最後一哩」runtime 觸達（可解釋性轉向實作輪，GAP-Y2 closure）；A/C 軌不動，`L_合體=min(A=L5,B=L5,C=L5)=L5` 本輪維持不變（可審批性加固，不新增自治能力）。

### meta⁸ 可審批儀表板 runtime 觸達（沿用 Phase Y / ACT-161 / Rule 9.37，不取新 ID；read-only CLI）
- **新增 `tools/fsm_runtime/render_topology_dashboard.py`（唯讀薄殼 CLI）**：v0.25（含）以前 `render_recursion_topology_dashboard`（Phase Y / ACT-160/161）**只在測試裡被呼叫**、runtime 零 wire、無 CLI 入口——舵手 K=1 signoff proposed 互遞迴算子時，打開 `value-dimension-ledger.yaml` 仍只看到原始 `recursion_inventions` 鄰接+rank dict（盲簽）。本 CLI 讀 ledger proposed 算子 → `extract_topology` →**必過** `guard_visualization_bounded` → 印拓樸/終止/接地三視圖儀表板，斷盲簽鏈。用法：`python -m tools.fsm_runtime.render_topology_dashboard [--ledger PATH] [--rule-id RCR-xxx] [--page N] [--fold] [--json]`。
- **紅線零弱化（Rule 9.37.4）**：①**read-only 純觀察者**——只讀 ledger、零寫 FSM-STATE / 不碰 churn / meta-loop（測試證 ledger 跑後 byte-identical）；②**對抗分離乾淨**——零 import 任何 generator（AST 測試機械守）；③**無繞過 guard 開關**——呈現前必過 guard，違反即非零 exit + 不印儀表板（fail-closed，絕不 false-green）。
- **回歸鎖 `tests/test_phase_y_dashboard_cli.py`（9 測試）**：happy path / rule-id / `--json`；**真實 guard 觸發**（char_budget 逃逸 → exit 3 + stdout 空，證 guard 非空殼）+ guard wired（monkeypatch raise → fail-closed）；fail-loud（缺檔/缺段/rule-id 不存在 → 非零 exit + 明確訊息）；read-only + 對抗分離守。

### 驗證
- 見 improving_66.md §階段四（ci-gate exit 0、v0.26 not-chaos ≥ floor v0.25:1656、CLI 9 測試 PASS、三鏡 zero-trust）。**FSM/`*.tla` 對 v0.25 逐位元零差異 → Rule 9.18.1 無重跑義務**（CLI 為既有 guarded 渲染管線的新呼叫者，未改 `_HAPPY_PATH`/狀態/meta-loop；完全複用既有 `VisualizationBounded`，免新 TLA+ 不變量、免五軌 TLC）。

---

## [v0.25] - 2026-06-25（Copy-on-Evolve 自 v0.24；v0.24 凍結唯讀）

> 使用者請求：推進整合迭代（improving_63）。掌舵者 AskUserQuestion 裁定「**B 軌 scaffold_gc 翻 ON**」，明確 signoff 翻轉鷹架代謝「自動提議退役」預設。B 軌＝鷹架代謝活體化（流程自治 L5 機制加固，非升級）；A/C 軌不動，`L_合體=min(A=L5,B=L5,C=L5)=L5` 本輪維持不變。

### 鷹架代謝「自動提議退役」活體化（B-axis L5 加固，scaffold_gc 預設 ON）
- **`fsm_runtime._scaffold_gc_auto_propose_enabled()` 預設翻轉 OFF→ON**：improving_17/W-17-1 已把 `scaffold_gc.run_gc`（產 RetirementProposal proposed 提議 + Markdown 報告）接入 `enter_scaffold_gc` 主迴圈但**預設 opt-in 鷹架**＝代謝機具齊備卻未活體（arch_fitness FF-16 GAP-X2「鷹架代謝 GC 從未產退役 ROI 提案／代謝肌肉從未收縮」）。承接 v0.24 fire/catch 遙測活體化使 `scaffold_roi.fire_count/catch_count` 真實累積（資料就位），本輪翻為 **default 活體常態**＝L5：未設環境變數即 ON，進 SCAFFOLD_GC 態自動跑 run_gc 算 ROI 落 proposed 退役提議。**保留顯式 opt-out**（flag=0/false/no/off → 還原 v0.07 只記 tracking、不算 GC）。鏡像 v0.22 AUTO_RECOVERY / v0.23 SLV_AUTO_PROPOSE / v0.24 telemetry 翻環先例。
- **紅線零弱化**：run_gc 只產 RetirementProposal + Markdown 報告（落 build/reports/gc/SCAFFOLD-ROI-{date}.md）、**永不 set_maturity**（active 規則退役仍 🔴 人工，R-9.20 #11）；fail-closed（run_gc 失敗不阻塞已落定的進態、不偽造報告）；入口紀律不被 flag 弱化（非 RELEASE 源仍 raise）。**無 FSM 狀態/規則/`*.tla` 變更**（對 v0.24 逐位元零差異），僅翻轉「既有 side-effect 記帳的 runtime 預設」。
- **零退化護欄（conftest 隔離）**：翻 ON 後進 SCAFFOLD_GC 態會落盤 SCAFFOLD-ROI 報告至 GC_REPORT_DIR，新增 session autouse fixture `_isolate_scaffold_gc_default`（鏡像既有 telemetry 隔離），測試套預設把 flag 設 "0" → 既有裸 enter_scaffold_gc 測試 byte-identical v0.24、零污染工作區；production 出貨仍 ON。
- **測試對齊**：`test_scaffold_gc_auto_propose_wiring.py` Case 1 改寫為 default-ON 活體驗收（delenv 覆寫→自動跑 GC + 報告導向 tmp）、Case 2 顯式 opt-out 零退化守（setenv "0"→純記 tracking）。

### 驗證
- 見 improving_63.md §階段四（ci-gate exit 0、v0.25 not-chaos 1656＝floor v0.24:1656、三鏡 zero-trust）。**FSM/`*.tla` 對 v0.24 逐位元零差異 → Rule 9.18.1 無重跑義務**（run_gc 為進態後純 side-effect，零新增 reachable 邊；只產 proposed 報告不碰 churn/meta-loop/set_maturity 不變量）。

---

## [v0.24] - 2026-06-25（Copy-on-Evolve 自 v0.23；v0.23 凍結唯讀）

> 使用者請求：推進整合迭代（improving_62）。掌舵者 AskUserQuestion 裁定「**B 軸：telemetry 翻 ON**」，明確 signoff 翻轉規則命中遙測 fire/catch 兩 flag 預設。B 軌＝規則命中遙測活體化（流程自治 L5 機制加固，非升級）；A/C 軌不動，`L_合體=min(A=L5,B=L5,C=L5)=L5` 本輪維持不變。

### 規則命中遙測活體化（B-axis L5 加固，fire/catch 預設 ON）
- **`fsm_runtime._rule_fire_telemetry_enabled()`/`_rule_catch_telemetry_enabled()` 預設翻轉 OFF→ON**：improving_18/19 已把 on-watch fire（接入 `transition()`）與 on-attribution catch（接入 escalation 落點）記帳接好但**預設 opt-in 鷹架**＝遙測機具齊備卻未活體（DEF-17-001「fire_count=0」/ DEF-18-001「catch 側」因 opt-in 未開而懸置）。本輪翻為 **default 活體常態**＝L5：未設環境變數即 ON，進態即記 fire（fire_count 真實累積）、escalation 歸因即記 catch。**保留顯式 opt-out**（兩 flag =0/false/no/off → 還原 v0.08/v0.09 不記行為）。鏡像 v0.22 AUTO_RECOVERY / v0.23 SLV_AUTO_PROPOSE 翻環先例。
- **紅線零弱化**：只增 fire_count/catch_count，**永不 set_maturity**（active 規則退役仍 🔴 人工，R-9.20 #11）；fail-closed（記帳失敗不阻塞已落定的轉態/escalation；catch 缺證據不記、不污染 ROI）。**無 FSM 狀態/規則/`*.tla` 變更**（對 v0.23 逐位元零差異），僅翻轉「既有 side-effect 記帳的 runtime 預設」。
- **零退化護欄（conftest 隔離）**：fire 遙測每次 transition 寫凍結 governance/rules/，新增 session autouse fixture `_isolate_rule_telemetry_default`（鏡像既有 meta-ledger 隔離），測試套預設把兩 flag 設 "0" → 既有測試 byte-identical v0.23、零污染凍結本體；production 出貨仍 ON。
- **測試對齊**：`test_rule_fire/catch_telemetry_wiring.py` Case 1 改寫為 default-ON 活體驗收（delenv 覆寫→記 fire/catch）+ Case 2 顯式 opt-out 零退化守；`test_w20/w37/w38_catch_wiring.py` 5 個 flag_off_zero_regression 測試由 delenv 改顯式 `setenv("0")`。

### 驗證
- 見 improving_62.md §階段四（ci-gate exit 0、v0.24 not-chaos ≥floor v0.23:1656、三鏡 zero-trust）。**FSM/`*.tla` 對 v0.23 逐位元零差異 → Rule 9.18.1 無重跑義務**（fire/catch 記帳為 transition/escalation 後純 side-effect，零新增 reachable 邊；只增計數不碰 churn/meta-loop 不變量）。

---

## [v0.23] - 2026-06-24（Copy-on-Evolve 自 v0.22；v0.22 凍結唯讀）

> 使用者請求：推進整合迭代（improving_59）。掌舵者 AskUserQuestion 裁定「**B→L5 自演化活體化**」，明確 signoff 翻轉規則自演化「自動提議」預設。B 軌＝SLV 自動提議活體化（流程自治 L4→L5）；A 軌不動，故 `L_合體=min(A=L4,B=L5,C=L5)=L4` 本輪不變（綁定約束由 {A,B} 縮為僅 A）。

### 規則自演化活體化（B-axis L4→L5，SLV 自動提議預設 ON）
- **`fsm_runtime._slv_auto_propose_enabled()` 預設翻轉 OFF→ON**：improving_16 已把 `propose_slv_from_fpl`（proposed 草案）接入 `exit_production_behavioral_signal("learn")` 主迴圈但**預設 opt-in 鷹架**＝自演化機具齊備卻未活體（B 軸卡 L4）。本輪把規則自演化翻轉為 **default 活體常態**＝L5：未設環境變數即 ON，learn 出口自動 draft proposed 草案（無需人手動跑 CLI）。**保留顯式 opt-out**（`SDD_ENABLE_SLV_AUTO_PROPOSE=0/false/no/off` → 還原 v0.06 純轉態）。鏡像 v0.22 AUTO_RECOVERY（B L3→L4）翻環先例。
- **紅線零弱化**：草案恆 trust_level=proposed（R-9.11）；trust_level→verified 仍 🔴 人工（`exit_learning_commit` 強制檢查不動）；meta-halt（R-9.24 ChurnBounded/GraduationRatchet）守門不動；fail-closed 不偽造；gate 仍需 `fpl_id`。**無 FSM 狀態/規則/`*.tla` 變更**（對 v0.22 逐位元零差異），僅翻轉「誰觸發既有邊後的非轉態 side-effect」之 runtime 預設。
- **測試對齊**：`test_slv_auto_propose_wiring.py` 9→10 case：v0.22 Case 1 改寫為 default-ON 活體驗收（unset→自動 draft proposed）+ 紅線恆守（預設活體下 proposed 仍不自動升 verified）+ 顯式 opt-out 純轉態零退化守。Rule 9 突變 M-W592 實證非空殼。

### 驗證
- 見 improving_59.md §階段四（ci-gate exit 0、v0.23 not-chaos 1656≥floor v0.22:1655、三鏡 zero-trust）。**FSM/`*.tla` 對 v0.22 逐位元零差異 → Rule 9.18.1 無重跑義務**（side-effect 預設翻轉不改狀態圖；META_FSM ChurnBounded/GraduationRatchet 為靜態不變量，與 runtime 預設無關）。

---

## [v0.22] - 2026-06-24（Copy-on-Evolve 自 v0.21；v0.21 凍結唯讀）

> 使用者請求：推進整合迭代（improving_57）。掌舵者 AskUserQuestion 裁定「**A①+B② 並進（真正推合體 L4）**」，明確 signoff 兩個治理/安全政策 flip。B 軌＝AUTO_RECOVERY 常態化（流程自治 L3→L4）；A 軌（整合層自動凍結 signoff）落 AutoClaude repo，非 v0.22 本體。

### 流程自治常態化（B-axis L3→L4，AUTO_RECOVERY 預設 ON）
- **`fsm_runtime._auto_recovery_enabled()` 預設翻轉 OFF→ON**：improving_15 已把 `enter_auto_recovery`（ESCALATION→AUTO_RECOVERY_ATTEMPT，Rule 9.14 有界、TLA 已模型化、五軌 TLC 已證）接入主迴圈但**預設 opt-in**＝B 軸卡 L3。本輪把「有界自動恢復」翻轉為 **default 運作模式**＝L4：未設環境變數即 ON。**保留顯式 opt-out**（`SDD_ENABLE_AUTO_RECOVERY=0/false/no/off` → 還原 v0.05 停機行為）。
- **fail-closed 紅線零弱化**：`enter_auto_recovery` 內 Rule 9.14 全部守界不變（structural 禁、≤3/session、≤1/同因、失敗→ESCALATION_FINAL）。**無 FSM 狀態/規則/`*.tla` 變更**（對 v0.21 逐位元零差異），僅翻轉「誰觸發既有邊」的 runtime 預設。
- **測試對齊**：`DefaultOnNormalizationTests`（unset→ON 進 recovery + 顯式 opt-out 仍停 ESCALATION）；6 個驗純 retry-budget→escalation/tamper 抵抗之既有測試（與 auto-recovery 正交）顯式 opt-out 隔離。

### 驗證
- 見 improving_57.md §階段四（ci-gate exit 0、v0.22 not-chaos 1655≥floor v0.21:1654、**chaos 34 passed**＝bounded_ratio==1.0 在預設 ON 下成立、三鏡 zero-trust）。**FSM/`*.tla` 對 v0.21 逐位元零差異 → Rule 9.18.1 無重跑義務；仍實跑五軌 TLC 確認 0 violation（safety-default 翻轉之審慎舉證）。**
