# SDD_improving_Automation_18_WORKFLOW — Phase R 動態自我修正執行工作流

**用途**：詳細執行 [`SDD_improving_Automation_18.md`](SDD_improving_Automation_18.md)（Phase R：維度語意自我發明 + 退役聯動 meta-meta-meta 層）的 **dynamic workflow**——一套「生成 → 評估 → 合約談判 → 修復 → 停機」的自我修正 Agentic 閉環，把藍圖 §6 檢核清單逐項落地並以**客觀守門**（pytest / 五軌 TLC / chaos / QA 抓漏）驗收。
**建立日期**：2026-06-04
**前置**：藍圖 §0~§6 已撰；基線確認 pytest 1102 passed / 4 skipped、META_FSM TLC 13 distinct No error、next_free=ACT-129 / R-9.30。
...
| `EVALUATE` | 跑客觀守門：`pytest`（相關子集）+（ACT-131/134）五軌 TLC + chaos | 全綠 → PASS；否則 FAIL | §3.1 驗收 |
...
| `VERIFIED` | 全 ACT + QA 全綠、1102→新基線不回歸 | → TAG_PUSH_MERGE | §6 |
...
> **Self-Verification 對齊（藍圖 §5）**：若某 ACT 的「驗收契約」本身寫錯（例如要求「自指自利軸應被採納」這種矛盾標準），EVALUATE 會持續 FAIL；FIX 重試觸頂後**不**降級成「註解掉測試」，而是 `HALT_ESCALATION` 導人類修正驗收標準——人類維持「設計環境舵手」高度，而非被降級成修碼員。

---

## 3. 漸進式揭露與上下文管理（Context Degradation 防護）

- **每 ACT 隔離**：CONTRACT_NEGOTIATION 只載入該 ACT 需要的範本（Phase Q 對應元件）+ 藍圖該 ACT 段落；不一次載入全部。
...
1. **Pillar A**：ACT-129（synthesizer + 有界生成文法 + 自指守門）→ CONTRACT_NEGOTIATION（寫 test_phase_r ACT-129 段）→ GENERATE → EVALUATE（pytest 子集綠）
2. **Pillar B**：ACT-130（feature-keyed oracle + INV 語料）→ 同上循環
3. **Pillar C**：ACT-131（meta_ledger swap-cadence / guard_dimension_swap + META_FSM.tla/.cfg）→ EVALUATE（pytest + **META_FSM TLC 13 distinct 重證**）
4. **Pillar D**：ACT-132（steersman 候選池外發明 diff + 退役聯動 diff）→ EVALUATE
5. **收官**：ACT-133（治理 + ID 翻牌）→ ACT-134（**五軌 TLC + chaos 100 輪 + 全量 pytest 不回歸**）
...
| 守門 | 客觀通過條件 |
|------|-------------|
| pytest | `1102 passed`（基線）→ 新增 ~30~45 全綠、4 skip 不變、0 回歸 |
| 五軌 TLC | `SDD_FSM`/`META_FSM`(13 distinct)/`FLEET_FSM`/`COMPOSITION_FSM`/`OPTIMIZATION_FSM` 全 `No error`（不增第六軌） |
| chaos | 100 輪 `bounded_ratio==1.0`、`avg_tokens < 20000`，含 `DIMENSION_INVENTION_GOODHART_FLAP` + `DIMENSION_SWAP_THRASH_FLAP` |
| ID 一致性 | `python -m tools.fsm_runtime.id_registry validate` → `[OK]`；next_free 翻 ACT-135 / R-9.31 |
