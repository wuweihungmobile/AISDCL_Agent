...
class FlagOffZeroRegressionTests(_Base):
    def test_flag_off_stays_escalation_no_auto_recovery(self):
        """預設（未設環境變數）→ escalate 停在 ESCALATION、payload 無 auto_recovery 鍵。"""
        import os
        os.environ.pop(_AUTO_RECOVERY_ENV, None)
        self.assertFalse(_auto_recovery_enabled())
        rt = self._rt()
