...
- [ ] 6.1.1 確認 Phase 01 所有基礎設施文件已建立
- [ ] 6.1.2 確認 Phase 02 Greenfield + Documentation SOP 更新完成
- [ ] 6.1.3 確認 Phase 03 Brownfield + Refactoring SOP 更新完成
- [ ] 6.1.4 確認 Phase 04 Migration + DevOps + Integration SOP 更新完成
- [ ] 6.1.5 確認 Phase 05 Testing + Performance + Security SOP 更新完成
- [ ] 6.1.6 執行 QA-1 ~ QA-5 全部驗證，確認標記 `[x]`
- [ ] 6.1.7 執行 grep 驗證：所有 Agent YAML 包含 `generate_adr` 關鍵字

### 6.2 文件目錄最終確認

- [ ] 6.2.1 確認 `docs/02_architecture/adr/` 目錄存在（含 ADR-TEMPLATE.md）
- [ ] 6.2.2 確認 `docs/02_architecture/api/` 目錄存在（含 CONTRACT-TEMPLATE.yaml）
- [ ] 6.2.3 確認 `docs/02_architecture/migration/` 目錄存在
- [ ] 6.2.4 確認 `docs/03_testing/contracts/` 目錄存在（含 TCS-TEMPLATE.md）
- [ ] 6.2.5 確認 `docs/04_planning/performance/` 目錄存在（含 PBS-TEMPLATE.md）
- [ ] 6.2.6 確認 `docs/06_quality/security/` 目錄存在
- [ ] 6.2.7 確認 `docs/08_deployment/iac/` 目錄存在

### 6.3 SDD 規範持續維護機制

- [ ] 6.3.1 建立 `AISDLC_v0.09/guides/system/sdd/SDD_GUIDE.md`（SDD 核心指引）
- [ ] 6.3.2 更新 `AISDLC_v0.09/AISDLC_INIT.md`：加入 SDD 相關 Workflow 索引
- [ ] 6.3.3 更新 `CLAUDE.md`：加入 SDD 轉型完成後的規範（寫檔規則更新）
- [ ] 6.3.4 更新 `FILE_DIRECTORY_RULES.md`：納入所有新增目錄和文件類型
- [ ] 6.3.5 建立 `build/planning/archive/SDD_ADOPTION_TRACKER.md`（採用追蹤，Phase 01-06 轉型完成後歸檔）

### 6.4 SDD 採用推廣計畫

- [ ] 6.4.1 **試點情境**：選擇 Greenfield（最純粹 SDD 場景）作為首次試點
- [ ] 6.4.2 **試點驗證**：完整執行一個 Greenfield 專案，驗證 SDD 流程
- [ ] 6.4.3 **回饋收集**：記錄 SDD 流程的摩擦點和改進建議
- [ ] 6.4.4 **文件更新**：根據試點回饋更新 Phase 02 計畫
- [ ] 6.4.5 **全面推廣**：逐步推廣至所有情境
