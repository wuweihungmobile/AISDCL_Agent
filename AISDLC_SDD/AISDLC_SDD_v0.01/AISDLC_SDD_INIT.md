...
### Specialized Agents（18 個）

> 含 14 個場景專屬 + 4 個系統級 runtime agent（Phase D~H 自動化閉環產出，由 Runtime 在對應 observation 狀態觸發，非場景載入）。

| Agent 檔案 | 角色 | 適用場景 |
|-----------|------|---------|
| `agent/specialized/code-analyzer-zh.yaml` | 代碼分析師 | Brownfield / Refactoring |
| `agent/specialized/compliance-officer-zh.yaml` | 合規官 | Security |
