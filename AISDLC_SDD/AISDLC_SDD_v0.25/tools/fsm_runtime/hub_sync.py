...
class HubSyncClient:
    ...
    def diff(self, rule_id: str, endpoint_id: Optional[str] = None) -> DiffResult:
        # R44: rule_id is externally-controlled (CLI positional arg / direct
        # API call). Without sanitization a value like "../../secret_dir/leak"
        # survives the f-string path joins below untouched, letting both
        # cache_dir.rglob(...) and local_paths escape their intended
        # directories (path traversal / arbitrary file read). Sanitize before
        # any path assembly; keep the raw rule_id for the returned DiffResult
        # (display only).
