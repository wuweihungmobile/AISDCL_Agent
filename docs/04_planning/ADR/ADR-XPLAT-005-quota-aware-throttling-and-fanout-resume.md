...
> **一句話**：把水位拆成**兩條互不替代的軸**——context（分母＝window，沿用 75/90）與 quota（分母＝方案，新增 80/95）；quota 的值**只從 server 拿**、絕不反推；80% 那道**真的擋掉扇出**、95% 那道**真的收斂並依額度種類分三種動作**；可續跑的單位由 session 降到 **workflow run**。
