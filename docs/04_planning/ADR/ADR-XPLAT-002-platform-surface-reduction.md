...
| 12 | **UEP 棘輪化尚未落地**（R63 新增項，從舊項 4 拆分）：tier/reason 資料已齊備（R63），但無機械鎖阻止未來把某筆 tier3/4 悄悄改回 unpinned 或刪減硬理由關鍵詞——目前只有「當下合法」的驗證，沒有「不得退步」的棘輪 | §4.4 | **未指派**（R64+，非阻塞——tier 分類本身已可用，棘輪是錦上添花而非本輪 Phase 1-C 的必要條件） | 比照 `tools/tests/test_adr_xplat001_c1c2_lock.py::TestShrinkOnlyRatchet` 形狀：`git show HEAD:tools/check_script_parity.py` 取上一版 `_EXEMPT_PAIRS`/`_SINGLE_SIDED_EXEMPT` 的 tier 值機械比對，任何 tier3/4→非 tier3/4 或 tier1/2→unpinned 的降級皆須紅（除非該筆同時被移出登記表，代表已徹底收斂） |
