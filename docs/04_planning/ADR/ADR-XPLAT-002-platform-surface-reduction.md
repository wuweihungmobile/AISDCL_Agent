...
**可達的替代判準（不需 signoff）**：把 Scan-H 的通過條件由「ΔUEP < 0」改為
**「三元組（UEP／AC／GLC）由機械物一次取齊且不退化」**——具體三款見 §4.3.1 R75 裁決。
...
🔴 **R92 掌舵者裁決（`DEF-101-802`② 的明文化，政策而非缺陷）**：末階 `≤ 4` 的唯一路徑
Phase 2-B **必須**取得掌舵者 signoff 才可執行——**空的 §8.1 回執容器＝未通過，不得以
「容器長年為空」為由自動放行或降級該閘**。上段 R74 所述「結構上不可達」自本裁決起改讀為
「**刻意由掌舵者把關**」：這不是待修的結構缺陷，是政策。可達的替代判準（上段三元組）維持
可用，Scan-H 等掃描維度一律走替代判準，不得回頭以 ΔUEP 為唯一通過條件。本段是該政策的
唯一權威落點（護欄層 .py 依 LOC 淨額紀律不再複寫一份）。

🔴 **R75 訂正上一版替代判準的第一款（它自己也不可達，且不可達的形態與它要治的病同型）**：
R74 版第一款要的是「三元組**逐輪登記**完整」，承接者是人——每輪收尾把三個數字手抄進 §4.3.1
的表。而 §4.3.1 的登記自 R69 之後就停了，其後連續數輪零登記 ⇒ 該款在寫下的當輪即不成立。
更關鍵：它與 §9.1 邊界 (d-2) 直接衝突——那條逐字記載 §4.3／§4.3.1 的量測數字**沒有機械承接
者**，而 ARCH-R67R2-01 在 §4.3.1 抓到的正是一個當輪就失真的手抄常數，當時的處置是移除常數、
改指現查指令。**一個要求人逐輪製造「必然過期的手抄常數」的通過判準，等於把本 ADR 在治的病
寫成入場費。** R75 起該款由機械物承接、手抄登記廢除，見 §4.3.1。
...
🔴 **R75 裁決：逐輪「手抄登記」廢除，改由機械物承接。本節此後不再要求任何人抄任何數字。**

**廢除的對象**：R67r2 建立本節時，承接方式是「每輪收尾把 UEP／AC／GLC 抄成一列新表列」；
R74 又把「三元組逐輪登記完整」寫成 `CrossPlatform_Scan_Dimensions.md` Scan-H 通過判準的第一款。

**為何廢除（三條實據，不是嫌麻煩）**：
1. **自然實驗已經判過了**：本節自建立至今只有下面兩組配對量測（R67／R69），其後連續數輪
   零登記，而缺席**不會讓任何東西轉紅**（同 §6 邊界 1 覆蓋表缺列的形態）。R74 在治覆蓋表
   那一半時上了 `§9.1 SC-10`，卻把本節這一半留成散文交棒 ⇒ **同一個病治了一邊，而留下的
   那邊剛好就是新判準本身**。把同一個承接者（人）再指派一次，是拿已被證偽的方案重來。
2. **手抄本身就是本 ADR 在治的病**：§9.1 邊界 (d-2) 逐字載明本節的量測數字沒有機械承接者，
   而 ARCH-R67R2-01 在本節抓到的就是一個「量測 → 寫進文件 → 同輪後續波次讓它失真」的常數，
   當時的處置是**移除常數、改指現查指令**。要求逐輪手抄，等於把那次處置反向執行。
3. **抄的內容本身已被本節自己判為不合格資料**：下面兩組配對量測的段首都逐字寫著「量測面髒
   ⇒ 依 §4.3 唯一規則本組數字不得作為新基線」，而 GLC 行數欄從一開始就不寫值、只寫「見上列
   指令對兩側各跑一次」。⇒ 這個儀式在**最順利**的情況下產出的也是自陳不可用的資料。

**替代的承接者（機械物，R75 落地）**：`tools/tests/test_adr_xplat001_c1c2_lock.py::
TestScanHTripletIsTheLiveCriterion`。它做四件事：
- (a) **三元組一次取齊並印出單行報表**（UEP／AC 走生產碼 `check_script_parity` 的同一份計算，
  GLC 走上列指令的同一個 glob）⇒ 本節建立時要的「三個數字同時出現」由此承接，頻率從
  「每輪一次、靠人記得」變成**每次跑根層閘門一次**；
- (b) **UEP 不得上升**（凍結對 shrink-only；刻意不要求下降——§4.1 的地板是可辯護殘留）；
- (c) **AC 上升時必須同時有 UEP 下降**（§4.2 判定規則 2 的逐字機械化；規則 2 明文允許的
  「類別升級帶動 AC 上升」另有對照組測試護著，避免把「不計為收斂成果」超譯成「禁止」）；
- (d) **凍結對必須等於現況**（多退少補都紅）＝自緊，避免餘裕累積成破口（形狀同
  護欄層棘輪 `guard_line_problems()` 的 `[基準過時]` 那一款；🔴 R78 ARCH-03 訂正：
  原文指名的 `test_frozen_guard_count_matches_the_worktree` 隨 R77 檔數棘輪一併移除，
  已是幽靈引用）。

⚠️ **不變的部分（不要順手一起改掉）**：GLC 仍是**報表、不設上限**（§4.3 已用兩組實測否決兩種
上限設計，該論證不重辯）；要取**基線級** GLC 仍須照 §4.3 唯一規則——同一個 commit 前後各量
一次、且量測面 `tools/tests/` 零未提交變更。機械物只保證「三元組算得出來且不退化」，
**不保證**它算出來的任何一次數字具備基線資格，兩者別混。

**下面兩組配對量測就地保留為史料**（R67／R69）；本節此後**不再新增表列**。要看今天的三元組，
一律跑上列現查指令，或讀上述機械物的報表行。
...
1. ❌ **「已實測」是輪次屬性，不是本 ADR 的常數。** 任何一條「已實測」宣稱都只在**該輪的執行平台**
   ...
   **完全沒有進入任何平台覆蓋判定**：心跳與 log 落在 `AutoClaude/logs/`、被 `.gitignore` 排除、
   ...
   | R77 | **Windows 11 真機**（PowerShell 工具側）。⚠️ **本列為進行中輪次，收輪時必須複驗本列**——比照 R72／R74／R75／R76 的作法。🔴 本輪實測訂正一句沿用多輪的載具宣稱：**PowerShell 工具是 pwsh 7.6.4（Core）**、`powershell.exe -NoProfile` 才是 5.1（Desktop）；上方 R74~R76 三列寫的「PowerShell 工具側」因此**不等於** PS 5.1，凡引擎相依的結論須以顯式 `powershell.exe` 外呼複驗。本輪為 **12 個修復包並行**於同一棵工作樹 ⇒ 任何「全套 rc=0」只對取得它的那個時點有效 | 一律現查（`gh run list`）；🔴 **本輪開輪即查出收輪前的錨落後 4 個 commit 且宣告全綠**，實查 HEAD 的三支 compat／root-infra 皆 `failure`（逐筆查證每個 job 的 steps 數為 0＝Actions 帳務停擺造成的未啟動，不是程式碼紅）⇒ `ONBOARDING.md` §7 表③ 與錨的 `red` 欄已照實回填。**帳務未恢復前，雲端結論不可得，不得以本機全綠代替** | 同 R76（launchd 家族／bash 3.2／zsh／`macos_smoke_local.sh` 的實際執行行為；**排程環境仍是 PowerShell 5.1**）。🔴 本輪另以逐題可重跑矩陣量到一個此前未登記的覆蓋缺口：**mac→Win 方向的靜態注入攔截率為 0/10**（對面平台專屬 API 整類零判準），該缺口**不需 mac 真機**即可補（屬靜態掃描面），與「需 mac 真機的執行期覆蓋」是兩件事。<!-- xplat-rate-history: R77 動工前量測 --> 🔴 **R78 訂正（ARCH-05）：這個數字是 R77 動工前的量測，同一個 commit 落地的第六道判準已把兩個方向都推高**；本 ADR 不登載該量測值，現值一律現跑 `python -m unittest tools.tests.test_platform_neutral_paths.TestXplatInjectionMatrix`（末行印 `[Xplat injection matrix]`），判準本體見 `docs/06_quality/CrossPlatform_Maturity_Criteria.md` M5 列 |
   | R78 | **Windows 11 真機**。工具側＝pwsh 7.6.4（Core）；凡 PS 5.1 語意的標的一律顯式 `powershell.exe -NoProfile` 外呼（沿用 R77 訂正）。⚠️ **本列為進行中輪次，收輪時必須複驗本列**。本輪為**四方複審＋五修復包＋四支獨立 agent 並行**於同一棵工作樹 ⇒ 沿用 R77 的警語：任何「全套 rc=0」只對取得它的那個時點有效。🔴 本輪把這件事由警語升為**已取證的機械事實**：根層 unittest 的 rc 在多 agent 共用樹下是「別人鍵盤的函數」（`DEF-101-878` 族；`MIN_TESTS` 重釘紀律要求的「量測窗口前後工作樹指紋相同」在此作業型態下結構上不成立），故本輪收輪的閘門實測**一律在所有 agent 停工後的窗口內取得** | 一律現查（`gh run list`）。🔴 **本輪未推送前不查、亦不假設**：R77 遺留的月度支出上限在本輪開場已解除（實測依據＝四方複審與九支 agent 全數派工成功），但那是 **API 額度**不是 **Actions 額度**，兩者不同源；雲端結論在本輪 push 之後才可得，**不得以本機全綠代替** | 零覆蓋（無 mac 真機）。🔴 本輪新增一個此前未登記的**跨平台隱形面**：`git` 的行尾正規化只作用於 index、從不回寫工作樹，而 `git status` 兩側套用同一份正規化 ⇒ 工作樹與政策不符時**結構上不可見**（本輪實測並修復 144 支 `.sh` 與 6 支 `.ps1`）。連帶推論：**雲端 fresh clone 全綠不蘊含本機開發環境為綠**，因為 `actions/checkout` 一律重新 smudge。現查配方見 `docs/06_quality/CrossPlatform_R78_Debt_Audit.md` |
   | R79 | **Windows 11 真機**。工具側＝pwsh 7.6.4（Core）；凡 PS 5.1 語意的標的一律顯式 `powershell.exe -NoProfile` 外呼（沿用 R77 訂正）。⚠️ **本列為進行中輪次，收輪時必須複驗本列**。本輪為**十維唯讀掃描＋七個修復包**並行於同一棵工作樹，沿用 R78 的機械事實：根層 unittest 的 rc 在多 agent 共用樹下是「別人鍵盤的函數」⇒ 收輪閘門實測**一律在所有包停工後的單人窗口內取得**（本輪收斂包即在該窗口內重跑十一道閘門） | 一律現查（`gh run list`）。**本輪 push 之前不查、亦不假設**；雲端結論在 push 之後才可得，**不得以本機全綠代替**（R77 的 Actions 帳務停擺與 R76 的 job 層 `continue-on-error` 兩個already-known 盲區都要一併看，配方見 `ONBOARDING.md` §7 表③／表③-b） | 零覆蓋（無 mac 真機）。🔴 本輪新增一個此前未登記的方法論缺口：**護欄層自己的稽核痕跡沒有防改寫**——`_GUARD_LINES_REPIN_LOG` 自稱 append-only 而零機械強制，且逐檔凍結表在乾淨 HEAD 上已有 3 筆與磁碟不符（淨額 0、閘門全綠）⇒ 「這一層長了多少、長在哪」的歷史可被壓平。兩者本輪皆已上鎖，現查配方見 `docs/04_planning/R79_HANDOFF.md` §1 |
   | R80 | **Windows 11 真機**。工具側＝pwsh 7.6.4（Core）；凡 PS 5.1 語意的標的一律顯式 `powershell.exe -NoProfile` 外呼（沿用 R77 訂正）。⚠️ **本列為進行中輪次，收輪時必須複驗本列**。本輪為**六個修復包＋舵手＋兩支收工包**並行於同一棵工作樹，而 🔴 **六個包全部在額度上限被半途打斷**——沿用 R78／R79 的機械事實：多 agent 共用樹時根層 unittest 的 rc 是「別人鍵盤的函數」，故收輪閘門一律在所有包停工後的**單人窗口**內取得（本輪收斂包即在該窗口內重跑八道閘門）；被打斷的包留下的是半套變更，收斂包逐筆歸因後才決定修或還原 | 一律現查（`gh run list`）。**本輪 push 之前不查、亦不假設**；雲端結論在 push 之後才可得，**不得以本機全綠代替**。🔴 本輪新增一支 push 軌 `shellcheck-ci.yml`（DEF-101-937），它**雲端零次執行**，`ONBOARDING.md` §7 表③ 已照實記為「尚未執行過」而不是留白 | 零覆蓋（R80 當輪無 mac 真機）。POSIX 側改以 `bash:3.2` 容器取證（與 macOS 內建 `/bin/bash` 逐字同版）⇒ **bash 語言層可外推、coreutils 與 exec bit 執行期不可**，逐節劃界見 `docs/06_quality/CrossPlatform_R80_PackF_Posix_Evidence.md` |
   | R81 | **Windows 11 真機**。工具側引擎沿用 R77 訂正的結論（PowerShell 工具＝pwsh 7.x／Core，凡 PS 5.1 語意的標的一律顯式 `powershell.exe -NoProfile` 外呼）——🔴 該引擎版本本輪**尚未複驗**，收輪時須現查，不得沿用上一輪的版本號當本輪實測。⚠️ **本列為進行中輪次，收輪時必須複驗本列**：本列於帳本寫入 R81 第一列的同一刻建立（`current_round()` 現查已為 **81**），即 SC-10 要求的那一列——比照 R75 列已記載的作法。本輪為**多包並行**於同一棵工作樹 ⇒ 沿用 R78~R80 的機械事實：多 agent 共用樹時根層 unittest 的 rc 是「別人鍵盤的函數」，故收輪閘門一律在所有包停工後的**單人窗口**內取得 | 一律現查（`gh run list`）。**本輪 push 之前不查、亦不假設**；雲端結論在 push 之後才可得，**不得以本機全綠代替**。兩個已知盲區要一併看：Actions 帳務停擺造成的 `steps: 0`（看起來是紅、其實是未啟動）與 job 層 `continue-on-error` 造成的 run 層假綠，配方見 `ONBOARDING.md` §7 表③／表③-b | 零覆蓋（R81 當輪尚無 mac 真機）。取證路徑於收輪時依實際作法複驗；R80 曾以 `bash:3.2` 容器取證 POSIX 側，該路徑**只能外推 bash 語言層**，coreutils 與 exec bit 的執行期行為不可外推 |

   | R82 | **Windows 11 真機**。工具側引擎沿用 R77 訂正的結論（PowerShell 工具＝pwsh 7.x／Core，凡 PS 5.1 語意的標的一律顯式 `powershell.exe -NoProfile` 外呼）——🔴 該引擎版本 R82 當輪**未複驗**，欄內不登載版本號，現查入口＝`powershell.exe -NoProfile -Command '$PSVersionTable.PSVersion'` 與工具側 `$PSVersionTable.PSVersion` 兩邊各跑一次。⚠️ **本列為進行中輪次，收輪時必須複驗本列**：本列於收輪的單人窗口建立，`current_round()` 現查為 **82**（權威源＝`tools/check_defect_log_crossref.py::current_round`），即 SC-10 要求的那一列。本輪為**多包並行**（13 個 agent 交件：六個修復包＋Q2 減法＋額度政策＋三鏡對抗複驗＋收斂＋接線＋帳本＋對抗稽核）於同一棵工作樹 ⇒ 沿用 R78~R81 的機械事實：多 agent 共用樹時根層 unittest 的 rc 是「別人鍵盤的函數」，故收輪閘門實測一律在所有包停工後的**單人窗口**內取得。🔴 本輪另有一項與前八輪都不同的執行條件：**扇出寬度本身受本 repo 自己的額度節流守衛節制**（ADR-XPLAT-005 §2.4 的 80／95 兩軸），該守衛在本輪實戰中擋下過主控自己的扇出呼叫 ⇒ 「本輪派了幾個包」不再是主控單方面的決定，逐輪覆蓋面因此帶有一個新的、與平台無關的變因 | 一律現查（`gh run list`）。**本輪 push 之前不查、亦不假設**；雲端結論在 push 之後才可得，**不得以本機全綠代替**。三個已知盲區要一併看：Actions 帳務停擺造成的 `steps: 0`（看起來是紅、其實是未啟動）、job 層 `continue-on-error` 造成的 run 層假綠，以及 R80 新增後**雲端零次執行**的 `shellcheck-ci.yml`，配方見 `ONBOARDING.md` §7 表③／表③-b | **mac 真機 R82 當輪零覆蓋**（launchd 家族／`plutil`／`launchctl`／BSD `stat -f`／bash 3.2／zsh／`macos_smoke_local.sh`／`run_local_nightly.sh` 的實際執行行為全部未量）。🔴 本輪的 mac 面全部走**靜態掃描與文件對照**，不得讀成執行期覆蓋；R80 用過的 `bash:3.2` 容器路徑 R82 當輪亦未重跑，且該路徑本來就只外推 bash 語言層、coreutils 與 exec bit 的執行期行為不可外推。**排程環境仍是 PowerShell 5.1** 而工具側為 pwsh 7 ⇒ 兩引擎差異仍是常設覆蓋缺口 |
   | R83 | **macOS 真機**（`sw_vers` 26.5.2 build 25F84／`uname -srm` `Darwin 25.5.0 arm64`；系統 `/bin/bash` **3.2.57(1)-release**、預設 shell zsh）——**自 R71 起連續 12 輪 Windows 真機之後首次換回 mac**，故本輪的 Windows 面與前十二輪的 mac 面**角色互換**（見末欄）。⚠️ **本列為進行中輪次，收輪時必須複驗本列**：本列於帳本寫入 R83 首批列的同一刻建立，`current_round()` 現查為 **83**（權威源＝`tools/check_defect_log_crossref.py::current_round`），即 SC-10 要求的那一列。本輪為**六個修復包並行**於同一棵工作樹 ⇒ 沿用 R78~R82 的機械事實：多 agent 共用樹時根層 unittest 的 rc 是「別人鍵盤的函數」，收輪閘門實測一律在所有包停工後的**單人窗口**內取得。🔴 本輪另有一項與前十二輪都不同的執行條件：**共用工作樹的破壞性 git 原語已由 PreToolUse hook 阻斷**（`DEF-200-007`：一個 subagent 的 `git stash -q -u --keep-index` 當場清空 16 個修改檔＋4 個未追蹤檔，靠 `stash pop` 還原）⇒ 「多包並行」這個執行條件本身在本輪被證明有資料遺失風險，不再只是 rc 可信度問題 | 一律現查（`gh run list`）。**本輪 push 之前不查、亦不假設**；雲端結論在 push 之後才可得，**不得以本機全綠代替**。三個已知盲區沿用 R82 那一列（帳務停擺的 `steps: 0`／job 層 `continue-on-error` 的 run 層假綠／`shellcheck-ci.yml` 雲端零次執行），配方見 `ONBOARDING.md` §7 表③／表③-b | **Windows 真機 R83 當輪零覆蓋**（PowerShell 5.1／pwsh 7.x 兩引擎差異／`schtasks`／Git Bash／`windows_smoke_local.ps1`／NTFS 大小寫與檔案鎖語意的實際執行行為全部未量）。🔴 本輪的 Windows 面全部走**靜態掃描與文件對照**，不得讀成執行期覆蓋——這正是 R68~R70 那三列反過來的形狀。**排程環境**本輪換成 mac 的 launchd，`schtasks` 那一側因此連 smoke 都沒跑到 |
   | R84 | **macOS 真機**（`sw_vers` 26.5.2 build 25F84／`uname -srm` `Darwin 25.5.0 arm64`；系統 `/bin/bash` **3.2.57(1)-release**、預設 shell zsh；開輪 HEAD `bc024e3`）——**連續第二輪 mac**（R83 為自 R71 起連續 12 輪 Windows 真機後的第一輪）。⚠️ **本列為進行中輪次，收輪時必須複驗本列**：本列於帳本寫入 R84 首批列之後建立，`current_round()` 現查為 **84**（權威源＝`tools/check_defect_log_crossref.py::current_round`），即 SC-10 要求的那一列。本輪為**九個修復包並行**於同一棵工作樹、其後轉入**收斂窗口**（九包全部停工後才取閘門實測）⇒ 沿用 R78~R83 的機械事實：多 agent 共用樹時根層 unittest 的 rc 是「別人鍵盤的函數」，收輪閘門實測一律在所有包停工後的**單人窗口**內取得。🔴 本輪與 R83 不同的執行條件：**九包在中途全體停工**，工作樹留下 79 檔未提交改動（71 改＋8 新增、零刪除），保全點＝tag `R84-wip-preserved`（同〈可重啟點四條件〉第 1 條指定的手法）⇒ 本輪的閘門紅有一部分是**跨包干擾**而非單包缺陷，歸因時不得把兩者混為一談 | 一律現查（`gh run list`）。**本輪 push 之前不查、亦不假設**；雲端結論在 push 之後才可得，**不得以本機全綠代替**。三個已知盲區沿用 R82／R83 那兩列（帳務停擺造成的 `steps: 0`＝看起來是紅其實是未啟動／job 層 `continue-on-error` 造成的 run 層假綠／`shellcheck-ci.yml` 雲端零次執行），配方見 `ONBOARDING.md` §7 表③／表③-b | **Windows 真機在 R84 當輪零量測**（PowerShell 5.1 與 pwsh 7.x 兩引擎差異／`schtasks`／Git Bash／`windows_smoke_local.ps1`／NTFS 大小寫與檔案鎖語意的實際執行行為全部未量；本輪連一次 Windows 側 smoke 都沒跑）。🔴 本輪的 Windows 面全部走**靜態掃描與文件對照**，不得讀成執行期覆蓋——同 R83 那一列的形狀。**排程環境**本輪仍是 mac 的 launchd，`schtasks` 那一側因此連 smoke 都沒跑到。R80 用過的 `bash:3.2` 容器路徑 R84 當輪亦未重跑，且該路徑本來就只外推 bash 語言層、coreutils 與 exec bit 的執行期行為不可外推 |
   | R85 | **macOS 真機**（`uname -srm` 現查；本列由 R85 的**帳本洩壓包（P1）**在寫入本輪第一列帳本（`DEF-200-091`）的同一刻建立，`current_round()` 現查即為 **85**，權威源＝`tools/check_defect_log_crossref.py::current_round`，即 SC-10 要求的那一列）。⚠️ **本列為進行中輪次，收輪時必須複驗本列**——本列由**單一並行包**建立，它只知道自己那一格；平台版本字串、包數、以及本輪其餘執行條件**當輪未量**，一律由收輪單人窗口補齊，不得沿用 R84 那一列的值當本輪實測。本輪為**多包並行**於同一棵工作樹 ⇒ 沿用 R78~R84 的機械事實：多 agent 共用樹時根層 unittest 的 rc 是「別人鍵盤的函數」，收輪閘門實測一律在所有包停工後的**單人窗口**內取得 | 一律現查（`gh run list`）。**本輪 push 之前不查、亦不假設**；雲端結論在 push 之後才可得，**不得以本機全綠代替**。已知盲區沿用 R82~R84 三列（帳務停擺造成的 `steps: 0`／job 層 `continue-on-error` 造成的 run 層假綠／`shellcheck-ci.yml` 雲端零次執行） | **Windows 真機在 R85 當輪零量測**（截至本列建立時）——本列建立者在 mac 上，Windows 側的執行期行為一項都沒量；本輪若確有 Windows 真機量測，由收輪窗口就地訂正本欄 |
   | R86 | **macOS 真機**（`uname -srm` 現查為 `Darwin 25.5.0 arm64`；`HEAD=6229b5e`、建立本列時工作樹 dirty 39 筆）。本列由 **R86 收尾單人窗口**在帳本本輪第一列（`DEF-200-105`）落地後建立，`current_round()` 現查即為 **86**，權威源＝`tools/check_defect_log_crossref.py::current_round`，即 SC-10 要求的那一列。**雲端 CI：本輪零 `push` 試探**（掌舵者訴求注⑥逐字要求「先用 act 與 Docker 在本機全面驗證」，且 GitHub CI 額度已用盡）⇒ **雲端本輪零量測**，已知盲區沿用 R82~R85 四列（帳務停擺造成的 `steps: 0`／job 層 `continue-on-error` 造成的 run 層假綠／`shellcheck-ci.yml` 雲端零次執行）。🔴 **不得以本機全綠代替雲端** | **Windows 真機在 R86 當輪零量測**——本列建立者在 mac 上；本輪 Windows 側全部為靜態推論。🔴 R86 特有的一筆：`--pace` 的額度快取 TTL＝180s（`tools/lib/quota_gate.py:338`）**短於任何刷新間隔** ⇒ 手動查詢多半落在 stale 帶而印出 degraded 地板，該現象在兩平台皆成立、非平台差異（立案見 `DEF-200-105` 一族） |
   | R87 | **macOS 真機**（`uname -srm` 現查為 `Darwin 25.5.0 arm64`；`HEAD=746ceac`、建立本列時工作樹 dirty 12 筆）。本列由 **R87 收尾單人窗口**在帳本本輪首列（`DEF-200-107`）落地後建立，`current_round()` 現查即為 **87**，權威源＝`tools/check_defect_log_crossref.py::current_round`，即 SC-10 要求的那一列。**雲端 CI：本輪零 `push` 試探**（掌舵者訴求注⑥逐字要求先用 act 與 Docker 在本機驗證，且 GitHub CI 額度已用盡）⇒ **雲端本輪零量測**，已知盲區沿用 R82~R86 五列。🔴 **不得以本機全綠代替雲端** | **Windows 真機在 R87 當輪零量測**——本列建立者在 mac 上；本輪 Windows 側全部為靜態推論。🔴 R87 特有的一筆：本輪 8 實作包＋四方複審**全數未執行**——月度支出上限撞頂（`used 610 > limit 500`、`can_purchase_credits: false`）使任何 subagent 派不出去，且該軸**沒有 reset 可以等** ⇒ 本輪跨平台量測面（含 M5 注入矩陣）在兩平台皆為零，非平台差異（立案見 `DEF-200-107`／`DEF-200-108`） |
   | R88 | **macOS 真機**（`uname -srm` 現查為 `Darwin 25.5.0 arm64`；`HEAD=a7527b2`）。本列由 **R88 收尾單人窗口**在帳本本輪首列（`DEF-200-109`）落地後建立，`current_round()` 現查即為 **88**，權威源＝`tools/check_defect_log_crossref.py::current_round`，即 SC-10 要求的那一列。**雲端 CI：本輪零 `push` 試探**（GitHub CI 額度仍為用盡）⇒ **雲端本輪零量測**，已知盲區沿用 R82~R87 六列。🔴 **不得以本機全綠代替雲端** | **Windows 真機在 R88 當輪零量測**——本列建立者在 mac 上；本輪 Windows 側全部為靜態推論。🔴 R88 特有的兩筆：①同 R87，月度支出上限仍撞頂（`used 610 / limit 500`、`can_purchase_credits: false`、`disabled_reason: org_level_disabled_until`）⇒ 8 實作包＋四方複審**全數未執行**，跨平台量測面（含 M5 注入矩陣）在兩平台皆為零，非平台差異；②掌舵者本輪告知 **PG17+pgvector staging（≥1k 列真實 BGE-M3 向量）只有 Windows 11 環境有** ⇒ pgvector 那批 `[DEBT]` 只可能在 Windows 輪轉綠，mac 輪跑到它一律 skip 是正確結果（立案見 `DEF-200-112`；通道已於本輪建於 `run_local_nightly.ps1` pg-e2e stage） |
   | R89 | **macOS 真機**（`uname -srm` 現查為 `Darwin 25.5.0 arm64`；`HEAD=3ba5b93`、建立本列時工作樹 dirty 7 筆）。本列由 **R89 收尾單人窗口**在帳本本輪列（`DEF-200-114`／`DEF-200-115`）落地後建立，`current_round()` 現查即為 **89**，權威源＝`tools/check_defect_log_crossref.py::current_round`，即 SC-10 要求的那一列。**雲端 CI：本輪零 `push` 試探**（GitHub CI 額度仍為用盡）⇒ **雲端本輪零量測**，已知盲區沿用 R82~R88 七列。🔴 **不得以本機全綠代替雲端** | **Windows 真機在 R89 當輪零量測**——本列建立者在 mac 上；本輪 Windows 側全部為靜態推論。🔴 R89 特有的兩筆：①同 R87／R88，月度支出上限仍撞頂（`extra_usage`／`spend` 兩軸皆 100%、`resets_at=null`）⇒ 四方複審**結構上派不出去**（`--pace` 實測 `cap=0 band=halt binding=extra_usage`），本輪全部結論皆為單人自證，跨平台量測面（含 M5 注入矩陣）在兩平台皆為零，非平台差異；②本輪治的 `DEF-200-112` 是**平台中立**的訊息面缺陷（halt 兩型在 `--pace` 與閂鎖後訊息上分不出來），兩平台同形，修法零平台分支（詳情＝`docs/06_quality/CrossPlatform_R89_Closure_Evidence.md`） |
   | R90 | **macOS 真機**（`uname -srm` 現查為 `Darwin 25.5.0 arm64`；`HEAD=be53ff0`、建立本列時工作樹 dirty 17 筆）。本列由 **R90 帳本單人窗口**在帳本本輪首批列（`DEF-200-126`~`DEF-200-129`）落地後建立，`current_round()` 現查即為 **90**，權威源＝`tools/check_defect_log_crossref.py::current_round`，即 SC-10 要求的那一列。⚠️ **本列為進行中輪次，收輪時必須複驗本列**——建立者只知道自己那一格，包數與其餘執行條件由收輪窗口補齊。**雲端 CI：本輪零 `push` 試探** ⇒ **雲端本輪零量測**，已知盲區沿用 R82~R89 八列（帳務停擺造成的 `steps: 0`／job 層 `continue-on-error` 造成的 run 層假綠／`shellcheck-ci.yml` 雲端零次執行）。🔴 **不得以本機全綠代替雲端** | **Windows 真機在 R90 當輪零量測**——本列建立者在 mac 上；本輪 Windows 側全部為靜態推論。🔴 R90 特有的兩筆：①**額度由 halt 恢復**（R87~R89 連三輪月度支出上限撞頂使複審派不出去，本輪掌舵者切換帳號後解除）⇒ 本輪起四方複審重新可派，與平台無關；②本輪為**多包並行**於同一棵工作樹——🔴 **包數訂正（R90 收尾窗口）：本列原記「額度／PRD／PlaybookRunner／帳本四線」，實際至少 5 條線**（帳本／A 額度取數／B PRD／C PlaybookRunner／D ADR-交棒書），**再加包 E（平台條件記債）、旁測包、交棒書編纂包，全輪共 8 包**；低估包數會讓「rc 是別人鍵盤的函數」這個結論的強度被低估——沿用 R78~R89 的機械事實：根層 unittest 的 rc 是「別人鍵盤的函數」。🔴 **本輪的紅數有兩組，兩組都必須帶座標才可引用**：**(a) 移動樹讀數**＝帳本包收尾實測 **18 支紅**，逐支歸屬為 3 支護欄層行數棘輪（另一包的兩支 test 檔 +149）／4 支 ONBOARDING §7 live 格失準（另一包拆除死接縫致 AutoClaude total 20425→20417）／11 支即本列缺列所致的 SC-10 及其零串音級聯，歸因法＝mtime ＋ 持有面前後雜湊 ＋ 逐筆命中；**座標補記（R90 收尾窗口依包 D 要求補）＝量測於 2026-08-15 約 14:0x–14:3x (+08:00)、`HEAD=be53ff0e2d524bb34e341e1987690b2ace4c1e18`、當時 dirty 17~19 筆且仍在增長**（同一棵樹於 22 分鐘內由 19 增為 23 筆，逐時刻表見 `docs/04_planning/R90_HANDOFF.md` §0.1）⇒ 該讀數**不具憑證力**，僅可作為歸因線索。**(b) 靜止樹憑證**＝R90 收尾單人窗口在八包全部停工後量得，座標與原始整行見 `R90_HANDOFF.md` §0.0 與〈附錄 Z〉，那一組才是本輪收輪的唯一有效讀數。**下一輪不得引用「18」這個數字，除非同時引用它的座標。** |
   | R91 | **macOS 真機**（`uname -srm` 現查為 `Darwin 25.5.0 arm64`；`HEAD=5d6e3bc`、建立本列時工作樹 dirty 10 筆）。本列由 **R91 單人窗口（Developer）**在帳本本輪首批列（`DEF-200-135`／`DEF-200-136`）落地後建立，`current_round()` 現查即為 **91**，權威源＝`tools/check_defect_log_crossref.py::current_round`，即 SC-10 要求的那一列。⚠️ **本列為進行中輪次，收輪時必須複驗本列**——建立者只知道自己那一格，包數與其餘執行條件由收輪窗口補齊；本輪至此為**單包（零並行）**於靜止樹，故 rc 可歸因，不適用 R78~R90 那條「rc 是別人鍵盤的函數」。**雲端 CI：本輪零 `push` 試探** ⇒ **雲端本輪零量測**，已知盲區沿用 R82~R90 九列（帳務停擺造成的 `steps: 0`／job 層 `continue-on-error` 造成的 run 層假綠／`shellcheck-ci.yml` 雲端零次執行）。🔴 **不得以本機全綠代替雲端** | **Windows 真機在 R91 當輪零量測**——本列建立者在 mac 上；本輪 Windows 側全部為靜態推論。🔴 R91 特有的一筆：本輪唯一動到的跨平台面是 **hook stdout 的 JSON 送達形態**，而 `ensure_ascii=True` 正是為了 Windows 非 UTF-8 codepage（cp1252／cp950）而選的——該選擇在本輪**只在 mac 上驗過**（`.claude/hooks/context_budget_guard.py` 的模組層 `reconfigure` 失敗路徑，在 mac 上構造不出來）⇒ 那一半仍是靜態推論，交棒 Windows 輪複驗 |
   | R92 | **macOS 真機**（`uname -srm` 現查為 `Darwin 25.5.0 arm64`；`HEAD=12246db`、建立本列時工作樹 dirty 20 筆）。本列由 **R92 設計包（機械 autocompact＋84/94 訊號帶）**在帳本本輪首列（Q-1／`autocompact_posture` settings 鏈失明修復）落地後建立，`current_round()` 現查即為 **92**，權威源＝`tools/check_defect_log_crossref.py::current_round`，即 SC-10 要求的那一列。本列已於 **R93 收尾單人窗口複驗**：本輪為**多包並行**於同一棵工作樹（設計包／帳本結案包／E 族複審修復包／R92 guard 修復包），移動樹上的 rc 讀數不具憑證力，收輪靜止樹複量見 R93 該列。**雲端 CI：本輪零 `push` 試探** ⇒ **雲端本輪零量測**，已知盲區沿用 R82~R91 十列。🔴 **不得以本機全綠代替雲端** | **Windows 真機在 R92 當輪零量測**——本列建立者在 mac 上；本輪 Windows 側全部為靜態推論。🔴 R92 特有的兩筆：①本輪把機械 compaction 釘進 repo settings（`autoCompactEnabled` ＋ `env.CLAUDE_AUTOCOMPACT_PCT_OVERRIDE=90`，取捨見 ADR-XPLAT-008）——env 注入已在 mac 實證（posture 印出 `'90'`），**Windows 側同一條注入鏈未驗**（靜態推論：settings `env` 區塊為平台中立契約）；②context 訊號帶 75/90→**84/94**（掌舵者裁決，與額度尺 85/95 錯開）——閾值判定為純函式、平台中立，但 hook stdout 送達形態的 Windows 半邊沿 R91 該列的劃界仍為靜態推論，一併交棒 Windows 輪複驗 |
   | R93 | **macOS 真機**（`uname -srm` 現查為 `Darwin 25.5.0 arm64`）。本列由 **R93 收尾單人窗口**在帳本本輪首列（`DEF-200-139`）落地後建立，`current_round()` 現查即為 **93**，權威源＝`tools/check_defect_log_crossref.py::current_round`，即 SC-10 要求的那一列。本輪為**收尾窗口**：R92 帳本結案（8 項掌舵者裁決＋改派稅 27→29 列）＋容量自適應攤提落地（`DEF-200-122`／`DEF-200-114`／ADR-XPLAT-009）＋護欄層行數棘輪最終重釘（R90/R91/R92 連升 streak 由 R92 淨額 -7 打斷）三線收斂於本輪，逐項證據見 `CrossPlatform_R91_Scan_Findings.md` §F~§I-25。**雲端 CI：本輪零 `push` 試探** ⇒ **雲端本輪零量測**，已知盲區沿用 R82~R92 十一列。🔴 **不得以本機全綠代替雲端** | **Windows 真機在 R93 當輪零量測**——本列建立者在 mac 上；本輪 Windows 側全部為靜態推論。🔴 R93 特有一筆：收尾途中發現「兩條並行工作線都已停工」的前提在動工當下並不成立（容量自適應攤提的 account-key 延伸仍在落地），逐次量測與定案判準（連續三次靜止檢查）見 `CrossPlatform_R91_Scan_Findings.md` §I-25——**這是平台中立的流程教訓，非平台差異** |
   | R94 | **macOS 真機**（`uname -srm` 現查為 `Darwin 25.5.0 arm64`）。本列由 **R94 收尾單人窗口**在帳本本輪首列（`DEF-200-140`）落地後建立，`current_round()` 現查即為 **94**，權威源＝`tools/check_defect_log_crossref.py::current_round`，即 SC-10 要求的那一列。本輪為**全案收尾**：容量自適應攤提機制（`account_key` 訊號）四方複審（Architect／SA／SD／QA，含兩輪 REJECT→修復→複驗）全數 APPROVE，`ADR-XPLAT-009` 狀態由 Proposed 轉 Accepted；`DEF-200-114` 二次訂正落地（索引化＋證據檔 §J-1）；D1 發現（account_key 缺席退化路徑補 `note_degraded()`）以 `DEF-200-140` 承載；改派稅 27 列 R93→R94；護欄層行數棘輪重釘 84367→84406（+39，第 2／2 輪，見 §J）。逐項證據見 `CrossPlatform_R91_Scan_Findings.md` §J。**雲端 CI：本輪零 `push` 試探** ⇒ **雲端本輪零量測**，已知盲區沿用 R82~R93 十二列。🔴 **不得以本機全綠代替雲端** | **Windows 真機在 R94 當輪零量測**——本列建立者在 mac 上；本輪 Windows 側全部為靜態推論 |
   | R95 | **macOS 真機**（`uname -srm` 現查為 `Darwin 25.5.0 arm64`；`HEAD=45ba611`、建立本列時工作樹 dirty 21 筆）。本列由 **R95 收尾單人窗口**在帳本本輪首批列（`DEF-200-141`~`DEF-200-144`）落地後建立，`current_round()` 現查即為 **95**，權威源＝`tools/check_defect_log_crossref.py::current_round`，即 SC-10 要求的那一列。本輪三個並行包（Pkg-B 治理檔禁寫／Pkg-C 配速致動器／Pkg-D 喚醒選路）停工後由收尾窗口收攏：三份 R95 證據檔登記 `_GOVERNANCE_DOCS`、帳本時鐘 R94→R95（29 列改派）、`AUTOSDD_RESUME_MAX_TRANSCRIPT_BYTES` 補進 ENV_SPEC、根 CLAUDE.md 守衛總表 govwrite 列更新；護欄層行數棘輪處置見 `CrossPlatform_R95_Guard_Repin_Evidence.md`。**雲端 CI：本輪零 `push` 試探** ⇒ **雲端本輪零量測**，已知盲區沿用 R82~R94 十三列。🔴 **不得以本機全綠代替雲端** | **Windows 真機在 R95 當輪零量測**——本列建立者在 mac 上；本輪 Windows 側全部為靜態推論 |
   | R96 | **Windows 11 真機**（當回合現查 `python -c "import sys,platform;print(sys.platform, platform.platform())"` → `win32 Windows-10-10.0.26200-SP0`；`HEAD=51d0cf1`、建立本列時工作樹 dirty **15 筆**、修復包 C1 於 2026-08-19 動工時 **16 筆**；🔴 **定案值＝16 筆**——由**最後的收尾單人窗口**在所有包停工後以 `git status --porcelain` 當回合重量填實（14 筆 ` M` ＋ 2 筆 `??`＝本輪新建的兩份 R96 證據檔）。誠實劃界：該讀數取於本輪最後一次**人為**寫檔之後、收輪閘門重跑之前；閘門執行自身若再生出衍生檔（本輪入場時 `AutoClaude/.perf_baseline.toml` 與 pgvector fixture 曾以 dirty 出現、其後被還原），那不屬本格所指的人為改動面）。🔴 **第 ⑩ 波複驗（本格最終狀態）**：所有包停工後當回合重量仍為 **16 筆**（14 筆 ` M` ＋ 2 筆 `??`），**但組成已變**——第 ⑦ 波清單中的 `.perf_baseline.toml` 與 pgvector fixture 已還原，換成 `ONBOARDING.md`／本 ADR 自身／`tools/lib/quota_messages.py`／`tools/tests/test_block_destructive_git_r83.py` 等。**總數相同純屬巧合，不得讀成「工作樹沒變」**；下一輪引用本格時必須連同這句一起看，逐檔清單見 `CrossPlatform_R96_Closure_Evidence.md` §8.9.1——**自 R83 起連續 13 輪 macOS 之後首次換回 Windows**，故本輪的 mac 面與前十三輪的 Windows 面**角色互換**（見末欄），這正是 R83 那一列反過來的形狀。本列由 **R96 四方複審修復包 A（文件／帳本／ADR）**在帳本本輪首批列（`DEF-200-149`~`DEF-200-157`）落地後建立，`current_round()` 現查即為 **96**，權威源＝`tools/check_defect_log_crossref.py::current_round`，即 SC-10 要求的那一列。🔴 **本列的「進行中輪次」警語已於 R96 第二輪四方複審後由修復包 C1 結清**（原文逐字寫「本列由單一並行包建立，它只知道自己那一格；總包數與收輪閘門讀數一律由收輪單人窗口補齊」，而收尾單人窗口寫了 §8 卻沒回頭補這一列 ⇒ 一列自稱草稿卻全綠地留到複審——根因是 **SC-10 是純缺席型判準、只問「當前輪那一列在不在」、完全不判內容**，立案見 `DEF-200-171`；R85／R90／R91 三列同形）。**補齊如下**：**(a) 總包數（逐波可考）** ＝ ①Windows 真機切換輪的第一批修復；②第一輪四方複審（Architect／SA／SD／QA）＋閘門取證專員，全數 REJECT；③修復包 A（文件／帳本／ADR）與包 B（程式／測試）**並行**；④收尾單人窗口（`MIN_TESTS` 3284→3462 重釘＋全套 pass A/B/C）；⑤第二輪四方複審，四方全數 `APPROVE_WITH_CONDITIONS`；⑥修復包 C1（文件／帳本／ADR）與 C2（程式／測試）**並行** ⑦**最終收尾單人窗口**（在包 C1／C2 停工後重跑全套並整節改寫 §8.2）⇒ **工作包 7 波（含第 ⑦ 波最終收尾單人窗口）、其中兩波為雙包並行；複審 2 輪 × 4 方 ＋ 閘門取證專員 1**。🔴 **修復包 D1 補記（第三輪四方複審之後）**：原文寫「6 波」而同格稍後又自陳有第 ⑦ 波，兩者互斥（第三輪複審 SA 的 C-1 實測命中）。其後另有 ⑧ **第三輪四方複審**（Architect／QA 有條件核准、SD `REJECT`、SA 不核准現在 commit）與 ⑨ **修復包 D1（文件／帳本／ADR）＋D2（程式／測試）並行** ⇒ 截至該次補記共 9 波。其後另有 ⑩ **最終收尾單人窗口（第二次）**——在 D1／D2 全部停工後照相工作樹、重量護欄層行數（**零漂移**）、整節重跑並改寫 §8.2、判定 D1／D2 兩份交件報告的矛盾說法，逐項＝同檔 **§8.9**。🔴 **本格由第 ⑩ 波定案，不再是待填值**：**全輪工作波 10 波、其中三波為雙包並行；複審 3 輪 × 4 方 ＋ 閘門取證專員 1**。**(b) 收輪閘門讀數** ＝ `CrossPlatform_R96_Closure_Evidence.md` **§8.2** 八道全 rc=0（原七道 ＋ 第 ⑩ 波新增的第 8 道 `check_pytest_baseline_sites.py`，編號劃界見該節）——🔴 **該節已於 2026-08-19 由第 ⑩ 波「最終收尾單人窗口（第二次）」在所有包（含 D1／D2）停工後重跑並整節改寫**，這已是它的**第三版**（第 ④ 波原文 → 第 ⑦ 波 → 第 ⑩ 波；前兩版為何作廢逐字保留在同檔 **§8.6** 與 §8.2 前言）。🔴 **本格刻意不複寫任何收輪讀數**：`Ran N tests`／`帳本 N 筆`／`未結存量 N 列`／pytest 計數這類數字**只住 §8.2 一個家**——把收輪讀數抄進本 ADR 正是 `DEF-200-163` 的第二個家族成員（第三輪四方複審 SD 實測命中：抄上去的那一刻閘門就轉紅）。**定案讀數一律現查 `CrossPlatform_R96_Closure_Evidence.md` §8.2 那張表**，本格只留不帶數字的定性結論：`run_root_unittests` rc=0／`OK`（🔴 **第三輪四方複審後改判，由修復包 D1／D2 執行**：原文寫「本輪未再重釘 `MIN_TESTS`」，理由是「重釘會連動 `ONBOARDING.md` 的 live 格，而那又要求再跑一次全套」——QA 當回合實查 `tools/sync_onboarding_baselines.py::measure_rootunit()` 證實該因果**機械上不存在**：live 格 `import run_root_unittests` 讀的是 `MIN_TESTS` **常數本身**、刻意不重跑整套，`--check`／`--write` 皆為秒級 ⇒ 成本為零。本輪最終**決定重釘為當回合實測值**，下限與實收的 gap 歸零；定案讀數一律見 §8.2 實測表，本格不複寫數字）、`check_defect_log_crossref` rc=0 且 stdout＋stderr 搜「早退」命中 0 次（帳本有效狀態紀錄筆數與未結存量列數一律見 §8.2，本格不複寫）、`local_ci_gate` rc=0（七道 gate 全 PASS；🔴 **pytest 計數不在 ADR 內開第二個家**——唯一 SSOT ＝ `ONBOARDING.md` §7，本輪讀數見 `CrossPlatform_R96_Closure_Evidence.md` §8.2。原文逐字抄了 `local_ci_gate` 的 pytest 尾巴，當場讓 `test_doc_loc_baseline_freshness_r60.py::TestR69AdrMeasurementTokensAreLive` 轉紅——那正是 `DEF-200-163`「把收輪閘門讀數抄進一份會被閘門掃描的文件，本身就會在跑完閘門之後製造新紅」的第二個家族成員，第三輪複審由 SD 實測命中）、`check_loc_budget --json` rc=0（四類 violations 皆空陣列）、`archive_defect_log --check` rc=0、`sync_onboarding_baselines --check` rc=0、`ruff` `All checks passed!`。遞迴停損的劃界見同檔 **§8.7**（第 ⑦ 波）與 **§8.9.4**（第 ⑩ 波，該節另附「為何那兩道不必重跑」的掃描面實查依據）。本輪為**多波並行**於同一棵工作樹 ⇒ 沿用 R78~R95 的機械事實：多 agent 共用樹時根層 unittest 的 rc 是「別人鍵盤的函數」，收輪閘門實測一律在所有包停工後的**單人窗口**內取得。🔴 本輪另新增一個此前未登記的、**與平台無關**的收輪順序缺陷：`current_round()` 由缺陷帳本現查，而孤兒 backlog／改派出口保鮮／存量棘輪／本 SC-10 四類判準都吃它 ⇒ 「修 code → 跑閘門全綠 → 寫帳本 → commit」這個順序**結構上必然**在寫帳本之後生出新紅（本輪實測 19 筆根層 failure 全數由此而來），立案見 `DEF-200-163`。**雲端 CI：本輪入場實查（`gh run list` ＋ `gh run view --json jobs`，原始讀數見 `CrossPlatform_R96_Closure_Evidence.md` §1）最近 10 筆 run 全 failure 且牆鐘 ≤8 秒、抽驗 `32031136314` 得 `jobcount=1, steps=0`＝runner 從未被配置（帳務／額度平面），不是測試紅** ⇒ 這段期間任何「push 軌全綠」宣稱一律不成立，本機閘門是本輪唯一的活體驗證。🔴 **不得以本機全綠代替雲端**；已知盲區沿用 R82~R95 十四列 | **mac 真機在 R96 當輪零量測**（launchd 家族／`plutil`／`launchctl`／BSD `stat -f`／bash 3.2／zsh／`macos_smoke_local.sh` 的實際執行行為全部未量）——本列建立者在 Windows 上；本輪 mac 側全部為靜態推論。🔴 R96 特有的一筆：本輪修好的八筆缺陷有**五筆**是「mac 上恆綠、Windows 上才現形」，逐筆歸因見 `CrossPlatform_R96_Closure_Evidence.md` §2；其反方向（本輪為過 skip 天花板而顯式重釘的兩個常數，會在下一次 mac 開工時翻紅）**完全同構**，立案見 `DEF-200-155`——這一格因此不是「這一輪剛好沒 mac」，而是**判準形狀本身**每次切換都會付的稅 |
   | R98 | **Windows 11 真機**（當回合現查 `python -c "import sys,platform;print(sys.platform, platform.platform())"` → `win32 Windows-10-10.0.26200-SP0`；`HEAD=ba4599f`、建立本列時工作樹 dirty **13 筆**：9 筆 ` M` ＋ 4 筆 `??`）——**自 R96 起連續第二輪 Windows 真機**（R96 為自 R83 起連續 13 輪 macOS 之後首次換回 Windows；本輪延續同一平台，不是又一次切換）。⚠️ **本列為進行中輪次，收輪時必須複驗本列**——本列由**收尾單人窗口**在多包停工後建立，工作樹是否還有後續動作未知，附帶的閘門讀數與 dirty 筆數僅為建立當回合的快照。🔴 **「R97」輪次名稱撞號訂正**：`R97` 這個輪次名稱已被 commit `295daec`（額度哨兵無人看管耗用 token 修復，非跨平台複審系列）用掉，帳本原「承接輪次：R97」全數（存量 40 筆）已一次性訂正為 R98，`current_round()` 現查即為 **98**，權威源＝`tools/check_defect_log_crossref.py::current_round`，即 SC-10 要求的那一列。本輪工作波（收尾單人窗口盤點）＝①帳本瘦身包×2（改派出口保鮮、承接 R95 到期改派列的後續處置，`DEF-200-128`／`DEF-200-129`／`DEF-200-132`／`DEF-200-159`~`DEF-200-164` 等一族的批次改派承接 R98）；②PRD 差距分析包（對照 `docs/01_requirements/AutoClaude_Token_監控與喚醒機制_PRD_v2.1.md` 全 1833 行逐節核對現有實作，PowerShell 工具實跑三批測試 447+408+62 passed，見 `CrossPlatform_R98_PRD_Gap_Analysis.md`，新立 `DEF-200-176`）；③LOC 拆分包×2（`tools/lib/quota_policy.py`／`schedule_backend.py`／`sentinel_lifecycle.py` 三支頂格檔各拆出 `quota_policy_env.py`／`schedule_backend_calendar.py`／`sentinel_lifecycle_arm.py` 子模組解除 LOC 頂格，`git diff --stat` 淨額 94 insertions(+)／479 deletions(-)），其中一個 LOC 拆分包因 context 撞線中止、零改動落地；④兩筆真修復：`DEF-101-941`（BSD/GNU 掃描 22 類缺口記載已久卻無機械物在守，本輪補 `base64 -w`／`sha256sum` 兩條進 `tools/tests/test_bash32_compat.py` 的 `_PATTERNS`／`_BAN_TOKEN_SAMPLES` 與 `tools/macos_smoke_local.sh` 散文清單）與 `DEF-200-131`（`tools/lib/governance_docs.py` 的治理文件發現面常數（改名前單一樣式 `CrossPlatform_*.md`）看不到 `Quota_*.md` 命名慣例的姊妹檔，改為樣式元組 `_GOVERNANCE_DOC_GLOBS` 並登記 `Quota_R90_CrossAccount_Experiment.md`）。**雲端 CI：本輪零 `push` 試探**（掌舵者訴求逐字要求「GitHub CI 額度已用盡，本輪一律先用 act／Docker 本機驗證」）⇒ **雲端本輪零量測**，已知盲區沿用 R82~R96 十五列（帳務停擺造成的 `steps: 0`／job 層 `continue-on-error` 造成的 run 層假綠／`shellcheck-ci.yml` 雲端零次執行／`windows-compat-ci`與`macos-compat-ci` 近期 `runner_id=0` 帳號層級停擺，見 `DEF-200-174`）。🔴 **不得以本機全綠代替雲端** | **mac 真機在 R98 當輪零量測**（launchd 家族／`plutil`／`launchctl`／BSD `stat -f`／bash 3.2／zsh／`macos_smoke_local.sh`／`run_local_nightly.sh` 的實際執行行為全部未量）——本列建立者在 Windows 上，沿用 R96 該欄登記的 mac 面未覆蓋清單；本輪 mac 側全部為靜態推論（`DEF-101-941` 的 BSD/GNU 22 類補強僅載具自驗 regex 對已知壞樣本命中，非真 bash 3.2 執行期覆蓋） |
   | R100 | **macOS 真機**（當回合現查 `python -c "import sys,platform;print(sys.platform, platform.platform())"` → `darwin macOS-26.5.2-arm64-arm-64bit`；`HEAD=772e28b`、建立本列時工作樹 dirty **65 筆**：53 筆 ` M` ＋ 12 筆 `??`）——**自 R96／R98 兩輪 Windows 真機之後換回 mac**。本列由**收尾單人窗口**在所有並行包停工後建立，故**刻意不自標草稿警語**（R107 訂正：原文此處逐字引了 `DEF-200-171` 那句草稿警語本身——該列的 SC-10 內容判準補上後，逐字引述即命中禁詞，同 R73「訂正註記逐字引述壞形態＝製造新違規」判例，故改為指稱不引文；判準已於 R107 落地，本列建立當時即自律遵守其精神）：`current_round()` 現查為 **100**（權威源＝`tools/check_defect_log_crossref.py::current_round`），即 SC-10 要求的本列。🔴 本窗口的工作面＝解除根層閘門**兩道**早退（靜態標籤掃描的 `AutoClaude/tests` 檔數下限過期；`MIN_TESTS` 取證敘述保鮮自檢）＋重釘四處棘輪（`skip_tag_policy._TREE_FILE_FLOORS`／`run_root_unittests.MIN_TESTS`／`test_subprocess_encoding_hygiene._CHILD_SITE_FLOOR`／基線數字站點納管）＋帳本輪次時鐘由 R98 推進至 R100。🔴 **護欄層行數棘輪與 `prose` 分桶棘輪本輪未重釘**（前者當輪淨額超出 `net_cap_for_round(100)`；後者是 shrink-only 桶、依其判準自陳需四方複審），確切數字已交主控裁決 ⇒ **不得**把本列讀成「本輪閘門已收斂」。逐項讀數與 rc 見收件回報 | **本輪零 `push` 試探 ⇒ 雲端零量測**（沿用 R98 的訴求：GitHub CI 額度優先本機驗證）。已知盲區沿用 R82~R98 各列（帳務停擺造成的 `steps: 0`／job 層 `continue-on-error` 造成的 run 層假綠／`shellcheck-ci.yml` 雲端零次執行／`runner_id=0` 帳號層級停擺，見 `DEF-200-174`）。🔴 **不得以本機全綠代替雲端**——況且本輪本機亦非全綠 | **Windows 真機在 R100 當輪零量測**（`Get-ScheduledTask` 的 `NextRunTime` 憑證／PowerShell 5.1 vs pwsh 7 兩引擎差異／CP950 console／NTFS 大小寫與檔名保留字的執行期行為，全部未量）——本列建立者在 mac 上，Windows 面全為靜態掃描與文件對照，**不得**讀成執行期覆蓋 |
...
| 15 | **護欄層重釘的「淨額只准往下」義務尚未兌現**（R84 新增項，承接 ARCH-01 款(11)）：`_GUARD_LINES_REPIN_LOG` 歷來每一列都是上升、零列下降（逐輪淨額現查 `repin_round_nets()`，本表不登載數值），故 R84 落地的判準採「每輪淨額上限 ＋ 連續上升輪數上限」而非配對制——代價是**延後**而不是**封死**。🔴 本輪（R84）九包並行、每包都在補判準，收尾那次重釘必然是正淨額 ⇒ **R84 是加法輪**，義務因此順延而非兌現；本列的存在就是為了讓那個順延**有一個會到期的時點**，而不是像 item 7／8 那樣靠散文交棒 | §4.3.1／`tools/tests/test_adr_xplat001_c1c2_lock.py` 的 ARCH-01 款(10)(11) | **R86**（依表頭規則 1 的具名輪次；連續上升的輪數上限＝`_REPIN_MAX_CONSECUTIVE_RISING_ROUNDS`，該常數與生效點 `_REPIN_ROUND_CAP_SINCE` 皆現查，本表不寫死其值） | `repin_round_nets()` 對承接輪算出的淨額 ≤ 0，且 `repin_growth_problems(_GUARD_LINES_REPIN_LOG)` 回空 list；現查＝`python -m unittest tools.tests.test_adr_xplat001_c1c2_lock.TestGuardLayerRatchet`（該類的 `test_the_real_repin_log_stays_inside_the_cost_envelope` 是綠側控制組、`test_a_third_consecutive_rising_round_is_red` 是注入紅側）。⚠️ 判準刻意不追溯到生效點之前——歷史列受款(7) 的 append-only 指紋保護，補不回來 |
...
🔴 **R84 現查（本表仍為空，逐字據實記載）**：R83 交棒書把「§8.1 現查是否仍為空」列為 M1 達標的前提之一，
R84 於收斂窗口實查本小節——**回執列數仍為 0**，item 7（護欄層 LOC 預算的成長係數與 WARN 帶）與
item 8（`ci-gate.ps1` fallback 刪除政策）**均未拍板**。⚠️ 這一句是**觀測結果**，不是拍板：
本輪沒有向 PM／使用者取得任何一筆 signoff，任何人不得把本段讀成「已核可」或「已放行」。
本段刻意寫在表外而不新增表列——本小節是回執容器，把「沒有回執」寫成一列回執，
正是本容器上方那句「空表本身即為證據」要防的死信形態。
⇒ 兩項的正確承接不變：依表頭規則 1 維持「未指派」，並優先走 item 7 已載明的**降維選項**
（只量不判的報表，零 signoff 需求），那條路不需要拍板即可解除該列的一半。
...
🔴 **R75 依 `DEF-101-757` 的規則補「為何補不上」與承接輪次（劃界不得只是劃界）**：把 SC-3 的
千分位形態擴掃 §4.3／§4.3.1 會**當場永紅**——那兩節逐字保全著多組帶 provenance 的歷史量測值
（三個不同行數同屬一個檔案集合的那組實證，正是 §1.1 的立案樣本），而 SC-3 沒有同行豁免；
永紅的唯二出路（改寫保全原文／臨時加豁免）本檔在 (d-1) 已判過更糟。⇒ **這一格的正解不是加
掃描器，是讓那兩節不再需要寫下新的量測常數**：§4.3 已明文「此後不再登載任何行數常數」，
§4.3.1 已於 R75 廢除逐輪手抄登記並改由 `TestScanHTripletIsTheLiveCriterion` 承接 ⇒ 新增常數
的**來源**被移除，殘留的是史料而非活躍站點。**承接輪次：未指派**（觸發點＝任何一輪若又要在
§4.3／§4.3.1 寫下新的量測常數，就必須在同一個 commit 內先把 SC-3 的同行豁免做出來）。
