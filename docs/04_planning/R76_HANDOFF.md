...
- **🔴 下一輪應該是 macOS 真機輪**：同檔 §6.1 逐條列出 11 條「本輪全靠推論、零 mac 真機」的結論，
  ...
  以逐題可重跑的注入矩陣在 Windows 真機重量，**mac→Win 是 0/10（0%）**、 <!-- xplat-rate-history: R77 動工前量測 -->
  **Win→mac 是 5/12（41%；扣掉一筆「因為缺 encoding 而順帶命中」後為 4/12＝33%）**。 <!-- xplat-rate-history: R77 動工前量測 -->
  也就是說**攔截率為零的是 mac→Win 那個方向**，不是本列原先寫的 80%。 <!-- xplat-rate-history: 引述當時被訂正掉的舊值 -->
  🔴 **「這一格只有 mac 真機補得了」也是錯的**：mac→Win 的 0/10 全部是**靜態掃描面**缺口 <!-- xplat-rate-history: R77 動工前量測 -->
