...
> **平台**：Windows 11 Pro build 26200 真機（PowerShell 5.1）。
...
- **🔴 下一輪應該是 macOS 真機輪**：同檔 §6.1 逐條列出 11 條「本輪全靠推論、零 mac 真機」的結論，
  並建議 macOS 輪開場第一件事就是把那 11 條逐條驗掉。M5（雙向注入攔截率）現況是
  mac→Win 方向 80%、Win→mac 方向 40%、程式碼語意層 0/3 —— 差距 40pp，這一格只有 mac 真機補得了。
