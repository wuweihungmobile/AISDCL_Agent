...
🔴 **一個未實測的關鍵推論，下一輪務必驗**：守衛把這個池當成「不能派 agent」看起來
**是對的**——R87 那次 13 個 subagent 全撞的錯誤訊息逐字是 `monthly spend limit`，
而當時主 session 照常運作 ⇒ 暗示 **subagent 用量計在這個付費池、不計在訂閱窗**。
若成立，則在池子恢復前**沒有任何辦法派 agent，與守衛怎麼寫無關**，
`DEF-200-112` ② 面（cap）也就不是缺陷。**刻意不試驗**：驗證方法就是真派一個 agent
看它死在哪，而那正是 R87 燒掉 1,319,703 tokens 零產出的那條路。
...
### 未決的實然問題（PRD 管應然，這一條要實測）

R87 事故中 13 個 subagent 全撞 `monthly spend limit`，暗示 **subagent 用量可能計在
付費池**。若屬實，則即使守衛改對了，池子沒恢復時 agent 仍派不出去（那是 Anthropic
的計費行為，不是本 repo 的判準問題）。**驗證方式已與掌舵者議定**：
他重開 session 帶逃生口 `AUTOSDD_QUOTA_GUARD_OFF=1`，由我派**一個**最小 agent，
看它成功還是死在 `monthly spend limit`。成本一個 agent，不是 R87 的 1,319,703 tokens。
