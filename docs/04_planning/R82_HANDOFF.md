...
所以那一群測試**在世界上任何一處都沒被執行過**，mac 真機這一側當時完全沒有覆蓋。

> 🔴 **R83 訂正（本段的到期日已到，照下面那個標記自己的要求改寫）**：R83 在 macOS 真機上跑完
> `tools/run_root_unittests.py`，那群測試**已經被執行過**，且 `_RUNTIME_SKIP_CEILING` 已真的收了
> `tools/tests@darwin` 那一格（值取自 runner 當場印出的 `[skip census]`，六格為 `platform` 獨大、
> 其餘皆 0 ⇒ mac 側的 skip 全部是「在這個平台沒有標的」，零欠債、零未標籤）。
> 該剖面因此由 **advisory 升為阻斷式天花板**。逐項見
> [`CrossPlatform_R83_Scan_Findings.md`](../06_quality/CrossPlatform_R83_Scan_Findings.md) §C。
> **本段的歷史語句刻意保留不刪**（它記錄的是 R82 當時為真的狀態），只加這一段訂正——
> 而下面那個 `absent-if` 標記已隨標的落地而失去證偽對象，故一併移除：留著它會讓這道鎖
> 對一個**已經兌現**的宣稱永久轉紅，而那種紅沒有鑑別力、只會被下一個人整段拔掉。
...
> 🔴 **原本這裡有一個 `absent-if` 證偽標記**（標的是 `_RUNTIME_SKIP_CEILING` 內的 darwin 那一格），
> 附註逐字寫著「一旦真的收了 darwin 那一格，這個字面就會在磁碟上找得到，本段當場轉紅並要求改寫……
> 它逼這句話有一個到期日」。**R83 兌現了它，而且那道鎖真的當場叫了**——R83 收尾第三輪全樹只剩這一支紅，
> 訊息逐字指出宣稱與磁碟矛盾並列出兩個命中座標。這是本 repo 少見的「機械物在設計者預期的那一刻、
> 以設計者預期的方式生效」的實例，值得留下記錄，故標記本體移除、理由留在此處。
