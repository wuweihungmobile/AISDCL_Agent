...
狀態欄已改 Adopted），上一則訂正注尾句「落款生效待合併同審，本檔 Status 不變」時效終止。
② §9 W2.5（淨減法輪，兌現 `_REPIN_NET_CAP_DUE_ROUND=109` 到期義務）＝已由實作超越：
`tools/tests/test_adr_xplat001_c1c2_lock.py` 的 `_REPIN_NET_CAP_SCHEDULE` 已含
`(109, 610)` 一列且旁註標「到期輪兌現」（`:1449-1450` 本日現查），排程並已推進至
`(119, 564)`（`:1462-1464`）＝該到期義務已在常態棘輪節奏兌現完畢；棘輪通則不受影響
（`_REPIN_MAX_CONSECUTIVE_RISING_ROUNDS = 2`，`:1469`），W2.5 一格不再是待排工作。
③ §10 Q1~Q9 已由 `R110` 裁決記錄逐題裁決（`AutoSDD_Adjudication_Record_R110.md:22-30`）；
其中 Q7 指定的「v2.1.9＋v2.1.10 合併同審」路徑由 D2 直接落款補齊程序（PRD 修訂表
v2.1.10 列＝該 PRD `:15`，明載 D2 依 `R110`「未生效修憲不疊層」判例補齊落款程序）。
④ 誠實劃界：除 W2.5 外，本檔全部實作條款＝Adopted 待落地——M197-*／M198-*／L1／L2／
L3／(b)(c)／W0~W6 零落地（機讀錨本日現查零命中：`grep -rn "SYNTHETIC_KINDS" tools/`、
`grep -rn "W_TOTAL_FLOOR_MINUTES" tools/`、`grep -rn "AUTOSDD_QUOTA_BURNDOWN" tools/`），
§1 座標校正表抽驗零漂移（`_pace_of` `:546`／`cap_notice` `:222` 本日現查同值），
均不得標「已由實作超越」；PRD 修訂表 v2.1.10 列自稱「施工圖＝本檔＋增補檔，本表不重抄」
＝修憲文本不合入 PRD 本文是既定落款形態，非缺口。原文保留。】
...
> **(4c)（v2.1.14，`DEF-200-244`）**：通道 C 的獨立裁決已落款——gate 聚合面切換為**設計內例外**
> （R89／R98 取數層裁決，不受 (4) 約束；fail-safe 退回語意保留；實作義務＝`Decision.reason` 帶
> `gate_excluded=<kinds>`）。條文全文與程式面見 `docs/04_planning/PRD_Amendment_R126_GateExclusion.md`；
> 本檔 (4)／(4b) 一字不改。通道 B 的裁決見 `DEF-200-243`（同批落地，帳本列）。
