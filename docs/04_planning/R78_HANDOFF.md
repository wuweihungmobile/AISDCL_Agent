...
> 🔴 **R79 訂正上面這格（兩筆，皆為 R79 當回合實測；原文逐字保留以便辨認版本）**：
> ① **那條指令量的不是這一題**。`audit_session.py` 量的是「指令字串的形態」（管線後讀 rc、裸 `cd`…），
>    而 Q4 問的是「失誤的成因分佈」。以它重跑只會得到另一個量，卻很容易被誤當成歸因已更新。
>    正確的重跑載具是 R79 新增的 `tools/probe/misstep_attribution.py`（來源清單、桶的關鍵詞表、
>    每筆的歸屬理由都在檔內，輸出可 diff 的 `.jsonl`）。**R77 那組百分比至今沒有任何可重跑的產物**
>    ——該輪 commit 只新增 hook 與 probe 兩支檔，全庫零分群腳本。
> ② **`--latest N` 這個窗本身會被量測動作污染**：它以 mtime 排序，而每一支掃描 subagent 都會在
>    同一個目錄開一支新逐字稿 ⇒ 派愈多 agent，窗裡就愈全是 agent。R79 同一小時內以同一條指令
>    量到三組不同數字、rc 由 0 翻成 1。要引用數字請改用內容定義的窗（`--since <開工 ISO>`）。
> ③ 下一句「修之後 0 例」在真實流量上為假（R79 實測：同窗 12 例、全史 15 例，含 1 例「攔得下卻量不到」）
>    ——守它的 parity 鎖只餵 11 條寫死指令，結構上看不到窗口差異。
