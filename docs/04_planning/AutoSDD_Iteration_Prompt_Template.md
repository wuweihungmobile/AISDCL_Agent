...
1. **帳本**：`docs/06_quality/AutoSDD_Defect_Log.md`（monorepo 層、跨輪累積、只增不刪）。
   **首輪（或帳本不存在時）：於階段一自動建立含表頭的空帳本再開始記錄。**
   ...
   > 🔴 **開輪機械第一動作（非首輪；帳本已存在時）**：帳本當前輪號由「發現情境」欄
   > 最新一列推得。本輪第一列**尚未**落地前，帳本時鐘仍停在上一輪——此時若有並行
   > 派工在程式碼散文／測試裡先寫下本輪輪號字面，會被「程式碼輪號不得超前帳本當前
   > 輪」這類機械鎖判紅（同型復發見帳本 `DEF-101-978`／`DEF-200-091`）。故**開輪的
   > 第一個動作，先於任何並行派工，是先落地本輪帳本第一列**（哪怕只是一則開場記事
   > 列，內容可稍後補完）——不得靠人記得，寫入本 SOP 成為機械第一步。
...
- **防線預算制（R14 DEF-101-209 治理級凍結，R15 落地）**：防線塔遞迴深度**凍結於現行四階**
  （守門→守門之測試→數量下限釘選→清單交叉鎖）；**禁止**建立第五階（互鎖之互鎖）。
  新增任何守門／掃描器／互鎖，須同時滿足：(1) 引用**實證缺陷編號**（帳本 DEF-* 或
  本輪四方審查發現），禁「理論上可能漂移」開防線；(2) **搭載優先序**：擴充既有掃描器
  樹清單/判準 ＞ 新增 `tools/tests/` unittest 掃描器（搭 `run_root_unittests.py` 載具，
  自動獲得 root-infra-ci 第 8 道＋pre-push＋nightly 三處執行，零接線）＞ 新建獨立
  `check_*.py` 工具位（僅限需被非 Python 載具單獨呼叫者；新工具位須同步 CI 道、
  pre-push 清單、`test_root_infra_parity.py` 三站點）；(3) 每道新防線在帳本或計畫書
  載明**退場條件**（守的回歸面消失時何時拆）。審查者（Architect/QA）對違反本條的
  新防線一律 REJECT。**R15 ARCH-R15-REV-6 澄清**：要件 (3) 適用對象＝**掃描器／
  守門／互鎖**（會產生阻斷或紅綠訊號、需人工判斷「回歸面是否已消失」才能拆除的
  機制）；**不適用**於 dev_start 平台健檢中的**永久性 advisory sentinel**（如既有
  心跳新鮮度檢查、`core.longpaths` 檢查，以及 R15 新增之 venv Python 版本比對／
  launchctl 精確查核／CI 活性哨兵）——此類 sentinel 隨其依附機制（venv／launchd／
  GitHub CI）存在而存在，機制本身若廢除，健檢自然隨之移除，無需獨立退場條件；
  牽強編造退場條件反而混淆兩種性質不同的防線。

## 🧾 派工紀律（任務書門檻，先於任何並行派工，每輪不變）
1. **驗收指令派工前親跑（DEF-200-215）**：任務書內的每一條驗收指令，派工前必須由主控
   本人親跑一次（零成本、當場可證），或只准引用 repo 內具名腳本（如
   `python tools/run_root_unittests.py`）；**未親跑過的驗收指令不得寫進任務書**——曾有
   一條在本 repo 結構上跑不起來的 `python -m unittest discover -s tools/tests` 被寫進
   任務書，由兩個獨立的包各自撞到才被發現。
2. **外部端點輪詢間隔下限 ≥ 快取 TTL（DEF-200-216）**：凡派工涉及輪詢外部端點，任務書
   必須明定輪詢間隔下限，且不得低於該端點的快取 TTL——低於 TTL 的輪詢每次只是重讀同一
   份快取、不產生任何新觀測，卻照樣消耗請求額度（曾因任務書未限制間隔，15 秒輪詢打出
   一次真 429，使閘門進 `cap=0` 約 3 分鐘，並在燃燒帳本永久留下一列合成軸）。
3. **自造事件不得計為活體驗證（DEF-200-216）**：自家派工／探針觸發的訊號（如自造 429
   引出的 halt）只能證明「線路接通、機制會動」，**不得**寫成「生產首次生效」「活體驗證」
   ——活體驗證的判準是**非自家流量**引發的真實外部事件；把自傷事故描述成驗證成果，
   本身就是帳本已立案的錯誤描述形態（方向同 DEF-200-197）。
...
> 重現法與判準，導致各專家發現的項目狀態**未知**（既不能讀成已處置，也不能讀成已否
> 決）。故每輪四方複審的**第一動作**＝各專家 agent 的逐條 findings（含證據、重現步驟、
> 判準）必須落地為 repo 內的具名檔案（如 `docs/06_quality/CrossPlatform_R{{N}}_Scan_
> Findings.md` 或本輪審查報告檔），才可進入下方步驟 2 的修復；**派工書轉述的標題清單
> 不算落地**。收斂窗口的第一動作＝檢查上一輪複審產出是否已落檔，未落檔者一律視為
> 「狀態未知」，不得逕自讀成已處置或已否決。
