...
| **6b** cap＝f(水位%, 距 reset) | **未做（結構性，不是參數沒調對）** | `tools/lib/quota_policy.py` 的 `_pace_of()` 在 production **恆夾 1.0**：live 快取現查有數軸 `resets_at` 恆為 `null` ⇒ horizon 恆有 `none` 軸，而三檔乘數表（R84 已收斂為 `_mult()`）的 `near=2.0` 結構上到不了。另：`tools/session_resume_planner.py --help` 的旗標裡沒有任何「我現在能派幾個」的出口 | ① 讓 `resets_at=null` 的軸**不參與** `min()`（或明文降級為 advisory）；② 加一個 CLI 出口印出「當前 band／cap」——**這是舵手每次派工前唯一需要的那個數字** |
