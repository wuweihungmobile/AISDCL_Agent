...
| **T5（不建議預設啟用）** | 未公開的 OAuth usage HTTP 端點 | 低（隨時可能失效，且可能違反使用條款） | v1 將此列為主要方案，**v2 降為選用**，預設 `false`，並要求使用者明確承擔風險 |
...
```
壓縮觸發條件（AND）：
  K_ctx ≥ CONTEXT_COMPACT_PERCENT (75)        # 單一 session 的上下文佔用
...
# ------------------------------------------------------------------------------
CONTEXT_COMPACT_PERCENT=75                  # 【新增】單一 session 上下文佔用率
...
- 若啟用了未公開端點，額外驗證：關閉該端點後系統能自動降級且不中斷。
...
| 面向 | 要求 |
| :---- | :---- |
| **憑證** | Daemon **不得**讀取、複製或記錄 OAuth token／API key 明文。帳號識別一律使用不可逆雜湊。共享遙測快取只存用量數字，不存憑證 |
...
- 任何形式的限流／計費繞過、請求偽裝。
- 對未公開介面的高頻探測（選用的 T5 來源必須遵守輪詢間隔且失敗即降級）。
...
1. **不要碰未公開的 HTTP 端點。** statusLine 已提供你需要的一切，而且是官方支援的路徑。原 PRD 的 T5 方案現在既無必要也有風險。
