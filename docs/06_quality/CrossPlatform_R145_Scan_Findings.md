...
<!-- guard-total:R172 --> R172 護欄層累積淨額＝ 105860 → 106209（+349）——console_qa 事故輪收尾單人窗口重釘（2026-09-25，Windows）：`test_context_budget_guard.py`（`ConsoleFreeSpawnTest` 掃描面擴大到 pythonw 可達模組＋`spec_from_file_location_problems()` 新規則＋`PlannerCheckIsConsoleFreeTest`／`NoWindowBehaviourTest` 行為鎖與 STARTUPINFO(SW_HIDE) 負對照，DEF-200-389／390）+252＋`test_run_root_unittests.py`（`console_orphan_census.wrap()` 接線與三支自證測試，DEF-200-391）+65＝317，全額計入主軌＋本表自身收斂、`_REGRESSION_LANE_LOG` 同輪列、`_FROZEN_PREFIX_REWRITE_LEDGER` 三次接鏈（凍結前綴 275→281，sha 前後共四次重釘）造成的漂移 +32。回歸鎖軌申報 309（＝軌上限，同 R152／R169／R171 先例）、主軌 40（R171 經掌舵者核准一次性例外使 streak 歸零，本輪為新 streak 第一輪）。逐檔清單見缺陷帳本 DEF-200-389～391；證據見 `CrossPlatform_DEF200274_Parallel_Tests_Evidence_2.md`〈Windows console 洩漏事故（2026-09-25）〉；本行為 doc-total 對帳第二站點，另一站點寄居 `AutoSDD_improving_112.md`。 round-label-ok
