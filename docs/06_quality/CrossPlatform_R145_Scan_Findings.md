...
<!-- guard-total:R170 --> R170 護欄層累積淨額＝ 104746 → 105179（+433）——多 CPU 第二十二輪（cpu_budget worker 公式改 logical−ceil(logical/physical)＋CAP 由既有值上修為 18＋pytest 站點普查鎖）與 pre-push 三 leg 並行鎖收尾窗口：`test_ci_gate_xdist_allowlist.py` +149、`test_cpu_budget.py` +29、`test_windowsapps_guard_bash_parity.py`（TMPDIR flake 修法）+15、`test_pre_push_dispatcher.py`（AutoClaude／SDD leg 背景並行、root-infra leg 前景吃滿 W）+126＝319，全額計入主軌（不申報回歸鎖軌）＋本表自身收斂與 `_FROZEN_PREFIX_REWRITE_LEDGER` 接鏈漂移。QA-2 全套與對抗複審修復批（同輪追加）：`test_pre_push_dispatcher.py`（INT/TERM trap 補 `rm -rf` 並行 log 暫存目錄回歸鎖）+22＋本表自身補齊既有稽核列 round-label-ok 註解與收斂 +33＋本表自身收斂再延伸 +6（凍結前綴 246→264，sha 前後共八次重釘）。本輪非收尾單人窗口，實際 DEF-ID 待帳本補登時回填，暫以 DEF-200-372 當接續錨點。逐檔清單見本節；本行為 doc-total 對帳第二站點，另一站點寄居 `AutoSDD_improving_112.md`。 round-label-ok

## 附記（R171；系列收斂後四方重驗收尾單人窗口）

<!-- guard-total:R171 --> R171 護欄層累積淨額＝ 105179 → 105860（+681）——系列收斂後四方重驗收尾單人窗口（2026-09-25，Windows）：`test_workflow_permission_concurrency_lock.py`（`TestFsmChaosNightlyStreakReadsJobLayer`，chaos-latest job 層連敗隔離回歸鎖，DEF-200-379）＋`test_archive_defect_log.py`（穩定快照自證與正樣本，DEF-200-382）＋`test_dev_start.py`／`test_platform_neutral_paths.py`／`test_doc_loc_baseline_freshness_r60.py`（效能熱點回歸鎖，DEF-200-383）＋`test_ci_gate_xdist_allowlist.py`（普查掃描面補站，DEF-200-384）＋`test_block_destructive_git_r83.py`（cwd 密封回歸鎖 `TestResultDoesNotDriftWithCallerCwd`，DEF-200-385，另補兩支既有 prose 分類類別的受測模組指名訂正）＝625，全額計入主軌＋本表自身收斂、`_REGRESSION_LANE_LOG`／`_REPIN_APPROVED_ROUND_OVERAGE`／`_REPIN_NET_CAP_SCHEDULE` 到期兌現（171/531，重新武裝 173/530）／`_FROZEN_PREFIX_REWRITE_LEDGER` 接鏈／`_PHASE2_REVIEW_LOG` 新列（`[維持觀察]`）／E501 存量債折行造成的漂移 +56（凍結前綴 270→275，sha 前後共四次重釘）。回歸鎖軌申報 309（＝軌上限，同 R152／R169 先例）、主軌 372（R169／R170 已連續兩輪為正，本輪為第三輪，經掌舵者核准一次性例外，款(11) 判準邏輯與門檻數字一個字未動）。逐檔清單見缺陷帳本 DEF-200-379～385；量測與四方複審見 `CrossPlatform_DEF200274_Parallel_Tests_Evidence_2.md`〈系列收斂後四方重驗〉。 round-label-ok

## 附記（R172；console_qa 事故輪收尾單人窗口重釘）

<!-- guard-total:R172 --> R172 護欄層累積淨額＝ 105860 → 106240（+380）——console_qa 事故輪收尾單人窗口重釘（2026-09-25，Windows）：`test_context_budget_guard.py`（`ConsoleFreeSpawnTest` 掃描面擴大到 pythonw 可達模組＋`spec_from_file_location_problems()` 新規則＋`PlannerCheckIsConsoleFreeTest`／`NoWindowBehaviourTest` 行為鎖與 STARTUPINFO(SW_HIDE) 負對照，DEF-200-389／390）+252＋`test_run_root_unittests.py`（`console_orphan_census.wrap()` 接線與三支自證測試，DEF-200-391）+65＝317，全額計入主軌＋本表自身收斂、`_REGRESSION_LANE_LOG` 同輪列、`_FROZEN_PREFIX_REWRITE_LEDGER` 三次接鏈（凍結前綴 275→281，sha 前後共四次重釘）造成的漂移 +32。同輪追加（收尾單人窗口修復棒 H，DEF-200-393）：`test_context_budget_guard.py` -1（`PlannerCheckIsConsoleFreeTest` 改合成逐字稿＋顯式 `--transcript`，移除對真逐字稿存在與否的依賴）、`test_skip_ceiling_ratchet_direction.py` +6（修復棒 G 三方 CI 復紅修復＋skip ceiling 天花板同步，DEF-200-392）＝+5，＋本表自身收斂（含 `_FROZEN_PREFIX_REWRITE_LEDGER` 兩次接鏈列造成的漂移，凍結前綴 281→289，sha 前後共三次重釘）+26。回歸鎖軌申報 309（＝軌上限，同 R152／R169／R171 先例）、主軌 40（R171 經掌舵者核准一次性例外使 streak 歸零，本輪為新 streak 第一輪）。逐檔清單見缺陷帳本 DEF-200-389～393；證據見 `CrossPlatform_DEF200274_Parallel_Tests_Evidence_2.md`〈Windows console 洩漏事故（2026-09-25）〉〈f901721 雲端驗收與後續修補〉；本行為 doc-total 對帳第二站點，另一站點寄居 `AutoSDD_improving_112.md`。 round-label-ok

## 附記（R173；DEF-200-386 種子過期警告根治＋v0.30 console 洩漏 audit 擴面收尾單人窗口）

<!-- guard-total:R173 --> R173 護欄層累積淨額＝ 106240 → 106664（+424）——DEF-200-386（根層 runner「計時種子可能過期」警告與 `refresh_parallel_timing_seed.py` 判定矛盾）根治收尾單人窗口（2026-09-26，Windows）：`tools/lib/parallel_timing_cache.py` 新增 `current_staleness_report()`（讀磁碟 SEED vs 磁碟 LIVE 單一比較面）＋`tools/lib/parallel_shard.py`（`run_parallel()` 改走該 SSOT）＋`tools/refresh_parallel_timing_seed.py`（同步收斂）＝三處單一真相源化。同輪 DEF-200-389 家族擴面（Windows console 洩漏 audit）：`.claude/hooks/closure_evidence_verify.py` 可達的 AISDLC_SDD_v0.30 `tools/fsm_runtime/closure_evidence.py`／`hub_sync.py`／`sandbox_runner.py`／`tlc_runner.py` 七處子行程（四處 git、兩處 docker、一處 TLC java）補 `CREATE_NO_WINDOW`（`test_check_hooks_liveness.py` 掃描面擴大為 hook 可達 import 模組深度 3 BFS，同批新增兩支測試；DEF-200-394）；`tools/lib/console_orphan_census.py` 零增長時改印確認行而非全靜默；`test_context_budget_guard.py` 負對照擴充為武裝 WMI 事件監看並斷言零筆 OpenConsole／WT 建立事件；`tools/install_windows_nightly.ps1` 呼叫端 `test_install_windows_nightly.py` 四處 `subprocess.run` 補 timeout 與 WhatIf 斷言補強。同輪修復棒覆審收尾（DEF-200-394／395／396）：BFS 補解相對 import＋UnicodeDecodeError 兜底、WMI 監看 deadline 與量測解耦。結案回歸鎖：`test_run_root_unittests.py` +124（`ParallelTimingCacheStalenessReportTest`＋`RunParallelStalenessAdvisoryReadsMergedLiveCacheTest` 突變自證）、`test_check_hooks_liveness.py` +187、`test_context_budget_guard.py` +57、`test_install_windows_nightly.py` +11＝379＋本表自身漂移 45（新列、回歸鎖軌列、cap 到期兌現列、`_FROZEN_PREFIX_REWRITE_LEDGER` 接鏈列與反覆覆核收斂，凍結前綴 289→291，sha 前後共七次重釘）＝424，母項超出軌上限 ⇒ 部分申報 309（＝軌上限，同 R152／R169／R171／R172 先例），餘額 115 歸主軌（R172 起新一輪連續上升，本輪為 streak 第二輪，未觸 `_REPIN_MAX_CONSECUTIVE_RISING_ROUNDS`）。同輪兌現 `_REPIN_NET_CAP_SCHEDULE` 到期義務 (173, 530) 並重新武裝 175／529；`_ROOT_TOOLS_OLD_SCALE_DEBT_DUE_ROUND` 具名展延 173→178（鐵律七，非 root-tools 重構持有面）。逐檔清單見本節；本行為 doc-total 對帳第二站點，另一站點寄居 `AutoSDD_improving_112.md`。 round-label-ok
...
## 附記（R176；DEF-200-316 方案 B D1 落地與 C8 healed_fail 收尾單人窗口）

<!-- guard-total:R176 --> R176 護欄層累積淨額＝ 106985 → 107216（+231）——DEF-200-316 方案 B 收尾單人窗口（2026-09-26，mac，Apple M1 Max）：新檔 `tools/tests/test_hook_carrier_symlink.py`（P3-2 `hook_carrier_symlink.ensure()` dangling symlink fail-loud 修復的回歸鎖，六支測試含目標缺席情境）全額歸本軌＋主控裁決 C8（掌舵者直接指令：徹底解決 Stop hook 每輪重複回報同一批舊 ENOENT 的問題）帶來的 `test_check_hooks_liveness.py` 回歸鎖（`runtime_carrier_verdict()` 新增 `healed_fail` 治癒語意：同一顆載具的失敗只要之後出現過它自己的 `hook_success` 就不算活著；新增覆蓋治癒／未治癒仍活著／alien 不被治癒／問題清單只留最新八筆四情境＋ Stop guard 端到端安靜測試；同輪把本檔命名沿革與方案書誤植訂正等純歷史散文壓縮成指向本節與 `CrossPlatform_DEF200274_Parallel_Tests_Evidence_2.md`〈C8 復原判準〉節的 pointer，原文逐字保全於該節）＋既有 D2b/D2c 鎖檔逐檔漂移（`test_block_destructive_git_r83.py`／`test_mac_readiness_r82.py`／`test_platform_neutral_paths.py`／`test_single_venv_identity.py`）＋既 C8 之後主控親跑 `tools/run_root_unittests.py` 抓到的 C9（`test_mac_endurance_r83.py::HookWiringReachesThisPlatformTest` 讀真實 settings.json 斷言 POSIX 半邊條目存在，方案 B 下前提消失，設計書 D2c 漏盤第三支；改為單一載具種類斷言＋symlink 健康合成注入判準，新增負向格保留鑑別力，原斷言原文逐字保全於證據檔）＋本表自身漂移（新列、回歸鎖軌同輪列、反覆覆核收斂）。未超軌上限（231 ≤ 309）⇒ 全額申報回歸鎖軌，主軌 0。逐檔清單見本節；量測與兩鏡複審見 `CrossPlatform_DEF200274_Parallel_Tests_Evidence_2.md`〈D1 落地與兩鏡複審（主控收尾）〉〈C8 復原判準〉與〈C9 續航鏈載具鑑別力〉；此附記為 doc-total 對帳（≥2 站點）另一站點寄居 `AutoSDD_improving_112.md`，同 R129～R175 寄居體例。round-label-ok
