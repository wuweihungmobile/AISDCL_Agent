...
## 帳本瘦身批（承接 R89 交棒書 §5b）— 四列結案的逐字原文
> 走 R82（13 列）／R85（17 列）判例的同一條合法出口：**結案 ＋ 瘦身成索引，原文逐字進具名證據檔**。下方每一節的 fenced 區塊是該列**瘦身前的整列原文**（含全部七欄與前後 `|`），逐位元組取自帳本、未經任何改寫或略段；搬遷後以 `原文 in 本檔文字` 的子字串比對複驗（比對指令與輸出見本輪回報）。

### DEF-101-377 — 瘦身前整列原文（4247 bytes，逐字）
...
### DEF-101-388 — 瘦身前整列原文（2967 bytes，逐字）
...
### DEF-101-746 — 瘦身前整列原文（974 bytes，逐字）
...
### DEF-200-111 — 結案憑證與**未解殘留**的誠實劃界

瘦身前整列原文（581 bytes，逐字）：

```text
| DEF-200-111 | 2026-08-14 | R88 建 pgvector 自動通道時就地發現 | `_NO_AUTOMATED_CHANNEL` 逐字寫「本 repo 目前沒有任何自動通道會跑這兩支」，而 R88 就是建通道那一輪 ⇒ 該句在同一個 commit 內變成假話；既有鎖只讀「承接輪次 R<n>」字面、讀不到這段散文 ⇒ **零機械訊號**（同 `Find-GitBash`／`FROZEN_SETTINGS_PREFIX` 判例） | P2 | 就地改寫為「通道已存在＋接線憑證＋平台綁定」；🔴 仍**無鎖**守這類散文宣稱，承接輪次：**R89** | open（承接輪次：**R90**） |
```

**已解的那一半（本輪實查）**：`AutoClaude/tests/integration/test_pgvector_hnsw_recall.py:181` 的 `_NO_AUTOMATED_CHANNEL` 現文逐字含分流欄要求的三要件——「自動通道**已存在**（R88 建）」（通道）、「不帶 marker ⇒ `5 passed, 2 skipped`；照舊併入 `-m pg_real` ⇒ `7 deselected`」（接線憑證）、「PG17+pgvector staging **只有 Windows 11 那台環境有**」（平台綁定）。

**未解的那一半（不得讀成已處置）**：分流欄第二句「🔴 仍**無鎖**守這類散文宣稱」在本輪**沒有**任何進展，且本次結案**沒有**把它併給任何一列。🔴 **本段第一版在這裡寫了一句假話並當回合自查抓到，逐字訂正於此**：原文寫「它現在只由『已結列殘留待辦』那道 warning 看著」——當回合實測**不成立**，`check_defect_log_crossref.py` 印出的殘留待辦仍是 13 筆且**不含** `DEF-200-111`（該判準只認 `下輪`／`未指派`／`擇機`／`backlog`／`留待`／`待辦` 等字面，本列新狀態欄一個都沒有）。⇒ 誠實說法是：**這個殘留今天零觀測者**，只由本節這段散文記著。這正是 `DEF-101-757`「已知射程缺口不得只以劃界結案」的射程內；要真的關掉它需要另立一列或補一道鎖，兩者皆非本包持有面，故列為本包的 `not_done`。

### 本批**沒有**結掉哪些列，以及為什麼（誠實劃界）

R89 交棒書 §5b 的目標是把未結列壓到 warn 線（86）以下，本批只降到下方回報所載的數字，**沒有達標**。逐筆實查後判定「結不掉」的主要理由三類：
1. **卡在掌舵者拍板**（agent 不得代決）：`DEF-101-795`(b)／`DEF-101-798`／`DEF-101-802`②／`DEF-101-936`／`DEF-101-559`／`DEF-101-392`。
2. **實查確認未修**（逐項跑過，不是猜的）：`DEF-200-012`（`quota_meter.cache_path()` 仍為 `Path(tempfile.gettempdir()) / CACHE_NAME`）、`DEF-200-015`（`_REF_RE` 仍寫死 `DEF-101-\d+`）、`DEF-200-097`（`AUTOSDD_QUOTA_FANOUT_CAP` 仍渲染在「既有逃生口」區）、`DEF-101-941`（`test_bash32_compat.py` 內 `base64`／`sha256sum` 皆 0 命中）、`DEF-101-399`（全 workflow `workflow_call` 0 命中）、`DEF-101-235`①（`dev_start.py:973` 的 `OpenProcess` 仍未設 `restype`）、`DEF-200-067`①（`tools/tests/test_ps_engine_ssot.py` 仍是 tracked 檔，`git rm` 從未執行）。
3. **只修了一半**：`DEF-200-070`② 已由 `AutoClaude/tests/execution/test_shell_portability_contract_r85.py`（32 支測試、`portability_note()` 執行期診斷）落地，但①「該掛的沒掛」仍是該族**唯一**載體（`DEF-200-045` 狀態欄逐字把①指給本列）⇒ 整列結掉會讓①失去承接者，故不結。`DEF-200-065`③（`quota_ledger.py:6` 散文已帶訂正）與 `DEF-200-075`③（`[DEBT]` 承接輪次已推進）同理，①②仍在。
4. **`DEF-200-053` 刻意不結**：其分流欄寫的治本＝「天花板由寫死常數改為『現查值 ≤ 史料末元素』」，那是把一條**零成長容忍**的相等棘輪改成不等式＝放寬判準本身。本包的授權面明文禁止「修改判準來讓自己過關」，故本輪只走它既有的合法出口（結案＋瘦身＋兩條天花板往下重釘），**不動判準形狀**；「餘裕結構上恆為 0」這個立案事實在本輪之後仍然成立。


### 護欄層史料搬遷（帳本瘦身批）— `_UNPINNED_HANDOVER_CEILING` 的下修沿革

**為什麼搬**：`tools/check_defect_log_crossref.py` 受 `check_loc_budget.SPECIAL_FILES` 的
**相等**棘輪管（門檻＝納管當日行數，只准往下），而 `TestActionableMessagesHaveLocHeadroom`
另要求它保留 ≥5 行餘裕給「訊息教人做的事」。本包為記錄 5 → 4 的下修寫了 4 行註解，
當回合實測餘裕由 5 掉到 **1** ⇒ 該鎖轉紅（`AssertionError: 1 not greater than or equal to 5`）。
正解是**瘦身**而非調高上限（那是砸溫度計）⇒ 註解收成 3 行指針，原文逐字落在這裡。
...
#: 🔴 帳本瘦身包再下修 5 → 4：`DEF-101-377` 依其分流欄自述「不修復（非專案缺陷）」訂正
#: 首詞為 `wontfix`（判例同 `DEF-101-359`／`382`）並瘦身成索引，原文逐字保全於
#: `CrossPlatform_R89_Closure_Evidence.md`〈帳本瘦身批〉節 ⇒ 已結案者不再需要豁免。
#: 同樣是判準第②向訊息**自己指名**的動作（逐字要求刪除並下修為 4），不是放寬。
```

🔴 上方最後四行提到的首詞 `wontfix` 為**本包的錯誤動作**，已依 `R89_HANDOFF.md` §6d 的
舵手裁決訂正為 `closed-by-decision`（見下一節）。此處逐字保留原文是為了讓那個錯誤動作
與它造成的紅可被追溯，不是現行說法。


### `R89_HANDOFF.md` §6d 舵手裁決的執行 — 三列轉 `closed-by-decision` 前的逐字原文

**本包先犯過一次錯，逐字記在這裡**：第一版把 `DEF-101-388` 的首詞寫成 `wontfix`（依其分流欄
自述推得），而 §6d 的舵手裁決明文是 `closed-by-decision`。兩個首詞的**射程後果不同**：
`wontfix` 會讓該列落入 ADR-XPLAT-001 §4.3.1 的射程，觸發 `hard_block_offenders()` 的 C1
（DEF-ID 須出現在 `ONBOARDING.md` §9）與 C2（須寫出重新評估觸發條件）硬阻塞。當回合實測：

```text
FAIL: test_no_row_in_scope_violates_c1_or_c2
AssertionError: {'DEF-101-388': Finding(...unmet=frozenset({'C1', 'C2'}))} != {}
```

⇒ 訂正為裁決指定的 `closed-by-decision` 後，該鎖 `Ran 138 tests … OK`。
**教訓：狀態首詞不是措辭選擇，是射程選擇；已有明文裁決時不得自行替換。**

三列改寫前的整列原文（逐字）：


**DEF-101-388（改寫前 697 bytes）** — 刻意**不用** `####` 標題：同一個 ID 在本檔有兩個節（瘦身前／改寫前），做成標題會讓「`§<ID>`」這種指標產生歧義（解析得到、但解析到錯的那一節）。

```text
|DEF-101-388|2026-07-26|R47 修復 DEF-101-386 時撞見|`v0.05`～`v0.29` 各版凍結的 FF-17 斷言（`arch_fitness.py`）仍認舊版 `ci-gate.sh` glob 慣用語（R10 已改走 SSOT `sdd_version.py`）⇒ 手動指定歷史版本時 pytest 先失敗；原文逐字保全於 CrossPlatform_R89_Closure_Evidence.md §DEF-101-388|P3（僅影響手動指定歷史版本的 debug 路徑）|不修復程式碼（凍結版依紀律不回改；比照 DEF-101-359／382，known-gap 記入本列）|wontfix+凍結版紀律（known-gap；首詞依分流欄自述與兩具名判例訂正）：徹底解法要逐版回改 25 個凍結版，屬 Copy-on-Evolve 例外授權（未取得）。全文見同節|
```

**DEF-101-917（改寫前 680 bytes）** — 刻意**不用** `####` 標題：同一個 ID 在本檔有兩個節（瘦身前／改寫前），做成標題會讓「`§<ID>`」這種指標產生歧義（解析得到、但解析到錯的那一節）。
...
**DEF-101-936（改寫前 691 bytes）** — 刻意**不用** `####` 標題：同一個 ID 在本檔有兩個節（瘦身前／改寫前），做成標題會讓「`§<ID>`」這種指標產生歧義（解析得到、但解析到錯的那一節）。
...
## 帳本新立四列的實測詳情面（本節為傘狀說明；逐列各有自己的 `##` 節）

> 🔴 **本標題刻意不寫出那四個 ID 的完整字面**：第一版寫成「（DEF-200-126 ~ 129）」，
> 而帳本列的指標是「`<檔>.md §DEF-200-126`」⇒ 任何以「標題含該 ID」解析的讀者或工具都會
> **停在這個傘狀標題**，而不是那一列自己的節。指標解析得到、卻解析到錯的地方，表徵與正確
> 完全相同——正是 SA/N-3 點名的那個缺口的實例。本包當回合自查抓到並訂正。

> 帳本列受 `ROW_MAX_BYTES=700` 管，四列各只留 ≤700 B 的索引；**不可壓縮的實測數字全文在這裡**。
> 🔴 **出處標示紀律**：`DEF-200-126`／`127` 的數字**全部是 [包 C 回報]**——本包沒有獨立複驗
> 任何一個 AST 計數、探針讀數或 PASS 數，逐句照轉且不加工。`DEF-200-128`／`129` 是**本包
> 自己量的**，每一項都附當回合可重跑的指令。兩者刻意分開標，免得下一輪把轉述讀成自證。

## DEF-200-126

**[包 C 回報]** `autoclaude/execution/playbook_runner.py:113-116` 的註解逐字宣告
「IExecutor / IEvaluator / IBrain ports（hex 架構，目前為可選注入）」。實測：

| 量 | 值 |
|---|---|
| `_port_executor`／`_port_evaluator`／`_port_brain` 的 AST Attribute 節點 | 三筆**全為 `ctx=Store`、零 `Load`** |
| 掃描面 | 502 檔、**0 parse failure** |
| 字串常量含 `_port_` | **0** ⇒ 排除動態名稱組裝（`getattr` 繞道） |
| 建構點 | **24** 個（AST 精確計；`grep` 的 28 含 4 筆 docstring）——**零個**傳這三個 kwarg、**零** `**kwargs` 轉發 |
| 運行時 `__getattribute__` 探針 | 注入**被儲存**（True/True/True），但 `run()` 期間讀取 **0** 次、sentinel 觸碰 **0** 次 |
| A/B 對照（`dry_run=False`） | 注入 fake executor 與完全不注入，輸出**逐字相同** |

**真實接縫**在別處：`_pr().PtyWrapper` 的模組全域查詢（`prompt_dispatcher.py:44,56`）。

**危害＝誤導修法**：任何人讀了那段註解，第一個會試的就是「傳一個 fake executor 進去」——
而那個動作什麼都不會發生，且**沒有任何訊號**告訴他為什麼。`lint-imports` 對這一型
**結構上失明**：未被讀取的屬性不是 import 違規，九條契約全綠。

**同址第 4 個死接縫**：`:111 self._evaluator = Evaluator(...)`，全庫零讀取。

**更大的事實（本列真正的份量所在）**：那 24 個建構點**全在 `tests/`，production 零建構點**
（repo 已把 production 入口載於 `main.py:145`／`kernel.py:419`，從未與本死接縫連結）；
連帶 `execution/steps_orchestrator/`、`execution/mutation_applier/` 僅被 `playbook_runner.py`
import ⇒ **整個子樹 production 不可達**。

**處置＝拆除**（非改註解）。LOC 實測：該檔 341 → 333（**−8**），全庫 20425 → 20417，
餘裕 13 → 21 ⇒ **方向正確、無鎖阻擋**。

## DEF-200-127

**[包 C 回報]** `tests/test_gap014_020.py:103` 與 `tests/test_gap039_049.py:59` 的 skip reason
逐字寫著「治本方向＝SD_10 P3-R56-2 改用 fake-executor 重寫」。

**為什麼那個「治本方向」是錯的**：`make_service`（`tests/helpers/kernel_fixtures.py:65`）
組出來的是 `AutoResumeService` + `PlaybookKernel`＝**另一條程式路徑**。而這 11 支測試的斷言
**全部落在 Runner 路徑專屬的語意**上：

- `patch.object(runner, "_get_correction")`
- `patch.object(runner, "_evaluate")`
- `runner._minimax_evolver`
- `result.step_log` 的 `[SKIPPED]` 標記
- batch 截斷至 3

**Kernel 路徑一項都沒有** ⇒ 照登記的正解做，等於**換掉受測單元**：測試會變綠，但它守的
已經不是原本那件事。這比「沒有正解」更糟——它是一個**看起來可執行、做完還會變綠**的錯誤指路。

**真正的正解**＝補一層 `patch("autoclaude.execution.playbook_runner.PtyWrapper", …)`。
這是 repo 內**既有慣例**（**26 個站點**在用），不是新發明。包 C 已實證
**11/11 PASS、斷言零修改**，工作量約 **0.5 人日**。

🔴 **誠實劃界（包 C 自陳，本包照轉）**：此修法**繞過**而非歸因 skip reason 記載的原始成因
——「macOS `env -u CLAUDECODE pytest` 逾 600s 未完成」。那個逾時的根因仍未查明。

## DEF-200-128

**本包自量。** `R89_HANDOFF.md` §5a 逐字寫：

> 🔴 **另有 4 筆同樣寫 R89、卻不會轉紅（別漏了它們）**：`DEF-200-012`／`053`／`059`／`070`。

**當回合實測為 10 筆**（把 `current_round()` monkeypatch 成 90，跑真判準的同一組出口條件；
唯讀，不改磁碟）：

```text
named_R89：10 筆
  :136 DEF-101-796   :158 DEF-101-917   :159 DEF-101-918   :161 DEF-101-926
  :173 DEF-101-981   :176 DEF-101-998   :178 DEF-200-012   :198 DEF-200-053
  :204 DEF-200-059   :217 DEF-200-070
交棒書 4 筆 - 實測 = []
實測 - 交棒書 4 筆 = ['DEF-101-796','DEF-101-917','DEF-101-918','DEF-101-926','DEF-101-981','DEF-101-998']
```

漏掉的那 6 筆體例是 `**改派**承接為 **R89**`，與交棒書已列的 4 筆**同型**——所以不是判準
抓不到，是清單漏列。⇒ 照交棒書做的人只會修 4 筆，留 6 筆在 R90 靜默過期。

**立即損害本輪已修**：27 列（17 筆會轉紅 ＋ 10 筆靜默）一併改派，逐列**位元組中性**
（`R89`／`R90` 同為 3 bytes，逐列 assert 前後相等）。改派後複驗：釘 90 → 孤兒 17→**0**、
R89 靜默桶 10→**0**。

**未修的是結構面**：§5a 那類「待驗清單」只列 ID、**沒有可重跑的載體**，所以「清單對不對」
這件事沒有人能便宜地複驗——與本檔對 R77 失誤分群下過的判決（「沒有留下任何可重跑的產物，
所以『每輪重跑』結構上做不到」）**逐字同型**。治本＝待驗清單一律附腳本。

## DEF-200-129

**本包自量。** 交棒書 §5a 末尾把「這條關鍵字出口本身是否該收緊」列為另案，但沒有給量級。
**當回合普查**（未結列中，狀態欄命中 `_reassign_hit()` 的那些）：

| 桶 | 筆數 | 說明 |
|---|---|---|
| 承接輪號 ≥ 90 | 9 | 本輪改派的（原 10 筆，`DEF-101-917` 已於本輪結案故離開分母） |
| 字面「未指派」 | 12 | 走硬規則② 後半句的**合法**出口 |
| **承接輪號 < 89、早已靜默過期** | **28** | 分佈：**R79:9／R82:11／R83:7／R85:1** |
| 合計走該出口的未結列 | **49** | |

⇒ 那 28 筆**不是**時鐘推進到 R90 造成的，它們在 R79~R85 那幾輪就已經過期，只是「改派」
這個關鍵字讓判準一律放行 ⇒ **零訊號地掛了 5~11 輪**。這個量級改變了「另案」的優先序：
它不是個位數的收尾雜項，是帳本裡每 4 個走該出口的未結列就有超過 2 個是死的。

**為什麼判準看不到**：出口條件只問「狀態欄有沒有『改派』二字」，**不問那次改派指向哪一輪**。
跨列回執那一半 R84 已經補上輪號比較（`DEF-200-088`），**自列這一半沒有**——同一個洞的兩半，
只補了一半。收緊的方向就是把自列那一半補齊；上線前必須先量假紅（母體＝現行 49 筆）。
