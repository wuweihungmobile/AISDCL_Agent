...
## 維度定義（A~E 為 R56 定案的五維；F／G 為 R59 新增，理由見表後）
...
**Scan-D 的相關限制也一併記下**（同輪 SA 指出）：crossref 閘門做的是「文件 vs 文件」一致性，**沒有任何機械物拿文件裡的數字去跟機器重量一次**。這是 ONBOARDING §7 能長期單邊（DEF-101-515）、且 R59 新表初版又寫入中途快照（SA-R59-01）的結構原因。目前對策是紀律（收尾時由主控在最終工作樹一次性重測並回填），尚無機械鎖。
...
- 🔴 **兩條硬規則（R59 新增，源於上表 Scan-F／Scan-G 的成立理由）**：
