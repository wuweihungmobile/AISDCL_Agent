...
4. **護欄層規模趨勢須逐輪量測並記入帳本；通過判準採 `docs/04_planning/ADR/ADR-XPLAT-002-platform-surface-reduction.md` §4.1 的 UEP（未受檢等價平面）**
   ...
   - 🔴 **R74 兩項訂正（兩者都是「本項自己在空轉」，不是標的難做）**：
     1. **量測那一半自 R69 後停擺四輪**（R70~R73 零登記）。本項逐字寫著「須逐輪量測並記入帳本」，
        而缺席型的漏做**不會讓任何東西轉紅**（同 ADR-XPLAT-002 §6 邊界 1 覆蓋表缺列的形態，
        該筆已於 R74 由 `§9.1 SC-10` 承接）。⇒ 承接者：**每輪收尾包**，三元組登記位置＝
        ADR-XPLAT-002 §4.3.1（該節已定義 UEP／AC／GLC 三元組的欄位與取值指令），
        本檔不另開第二個登記處。**不寫「下一輪某人」**——無主詞的交棒正是它蒸發四輪的原因。
     2. 🔴 **「通過判準（唯一）」在當前結構下不可達，本輪明說**：UEP 自 R65 起停在 5、
        連續 8 輪 ΔUEP＝0，而 §4.1 階梯的最後一階（`≤ 4`）唯一路徑是 Phase 2-B，
        它**需使用者／PM signoff**，而回執容器 §8.1 自 R67 建立至今仍是空表 ⇒ 通過條件
        不在工程側。把「維度未通過」歸因給執行者，會逼出本項自己警告過的壞行為
        （把可辯護的列硬刪）。**現行判準改為三條可達條件的合集**：
        (i) 三元組逐輪登記完整；(ii) 反位移未發生（ΔAC ≤ 0）；(iii) 護欄層規模趨勢有量測。
        要重開 `≤ 4` 那一階，**先取得 §8.1 的回執，不是先改工程**（ADR §4.1 已同步此裁決）。
