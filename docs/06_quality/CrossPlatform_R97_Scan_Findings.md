...
<!-- guard-total:R97 --> **本輪護欄層累積淨額＝ 84806 → 85829（+1023）** —— 逐檔漂移 3 支
...
<!-- guard-total:R97 --> 護欄層累積總量現值 **84806 → 85829（+1023）**；原始逐檔清單即上一節，
「同輪追加」節見下方新增段落（含「同輪追加④」四方最終複審收斂修復），款(12) 到期義務
同輪兌現（上限表追加 `(97, 950)`，重新武裝下一段到期輪 99、目標 850）。
