...
> 🔴 **R78 訂正（ARCH-05）：本節已不是 M1〜M6 的家。現行判準與現況一律以
> [`docs/06_quality/CrossPlatform_Maturity_Criteria.md`](CrossPlatform_Maturity_Criteria.md) 為準。**
> 本節保留為 R76 當輪的原始記載（可追溯），**其中的數字不得引用為現況**。
> 為何搬走：治理層的成熟度判準寄生在一份「輪次專屬的掃描發現文件」裡，而輪次文件按定義
> 不會有人回頭維護；後果在 R78 現形——M5 的攔截率同時住三個地方，三處全部停在**修復前**的
> 值（低報自己的成果，但下一輪跑載具會看到「一輪暴衝」而去找一個不存在的原因）。
> 下方 M5 兩處的量測值已就地標記為歷史值，現值請跑該列具名的載具。
...
| M3 | ❌ | 第三方抽樣面尚未含既有鎖庫 |
| M4 | ❌ | 本輪 4 筆 |
| M5 | ❌ | mac→Win **0%**（整類無判準）、Win→mac 33〜41%、程式碼語意層 0 <!-- xplat-rate-history: R77 動工前的量測，現值見 CrossPlatform_Maturity_Criteria.md M5 列具名的載具 --> |
