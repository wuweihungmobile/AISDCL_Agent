...
|---|---|---|---|---|---|
| **M1** | UEP 抵達可辯護地板，且該狀態被**正式承認**而非每輪重新解釋 | `python -m unittest tools.tests.test_adr_xplat001_c1c2_lock`（末行印三元組）＋ `ls` ADR-XPLAT-002 §8.1 回執表 | `UEP=5 AC=48 GLC_FILES=56`；自 R65 起連 9 輪 ΔUEP=0；§8.1 自 R67 建立至今**空表** | 二擇一：§8.1 出現回執且 UEP=4；**或** ADR 正式宣告 5 為終態並把 `_EXEMPT_PAIRS` 凍成 shrink-only、不再列為「目標」 | Architect |
| **M2** | 假宣稱密度單調下降 | 每輪複審抓到的「失實宣稱」筆數 ÷ 該輪新帳本列數 × 100 | 本輪 12 筆存活 blocking／25 筆 R76 新列 ⇒ **48/100**（其中純失實宣稱 6 筆 ⇒ 24/100） | **連續三輪 ≤ 3/100 且無任何一筆 P1** | SA |
...
| **M5** | 雙向注入攔截率：mac→Win 與 Win→mac 兩個方向都 ≥80%，且差距 ≤10pp | 每輪跑固定形制 N=10 注入矩陣（每輪強制抽換 ≥2 題防過擬合），逐題記「哪一道閘攔下」 | 甲 4/5＝80%／乙 2/5＝40%／差距 **40pp**；且乙攔下的兩筆全在檔名／路徑層，**程式碼語意層 0/3** | 兩方向皆 ≥80% 且差距 ≤10pp，**連續三輪不倒退** | QA |
...
| M2 | ❌ | 24〜48/100 vs 門檻 3/100，差一個數量級 |
| M3 | ❌ | 第三方抽樣面尚未含既有鎖庫 |
| M4 | ❌ | 本輪 4 筆 |
| M5 | ❌ | 反方向 40%、程式碼語意層 0/3 |
