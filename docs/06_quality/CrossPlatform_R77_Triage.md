...
| N-03（交棒書方向對調＋誤診） | P0 | **P1** | 同上。其量測本體（mac→Win 攔截率 0/10）是 N-04 的內容，N-04 保留為 P1 |
...
| **Q3** 雙向落差 | ✅ 中強 | R77-33（mac→Win 攔截率實測 0/10）、R77-45（交棒書方向寫反）、R77-52（150 站點 108 判不出方向）、R77-28（launchd 零對照組／兩 smoke 零對帳）、R77-48 | 雙向注入的「Win→mac」半邊有量測，「mac→Win」半邊的**執行期驗證**仍缺 mac 真機 |
