...
# ⚠️ 時態性敘述不會被誤收：家族內兩則**當時決策紀錄**都沒有 `見` 緊接 `主檔`，本樣式對
# 它們零命中（放寬成「主檔 … DEF-x」則歷史紀錄全部翻紅，而歷史檔逐字保全 ⇒ 閘門永紅）。
# 🔴 本輪把兩則樣本的逐字內容與**所在檔**交給 `test_archive_defect_log.py::
# TestTemporalNarrativeSamplesStayUncaught._SAMPLES` 看守（原文缺座標，曾被誤讀成 stale）。
