...
_TREE_FILE_FLOORS: dict[str, int] = {
    ...
    # 🔴 本輪（承接帳本列 `DEF-200-103`）重釘 48 → 49，機制與上三段逐字相同：並行包新增鎖檔
    # 讓 `tools/tests` 由 61 支長到 **62**，48 只剩實測的 77%（低於 `TREE_FLOOR_RATIO` 的
    # 80%），第三向「下限已過期」當場判紅並**逐字**指示「重釘為 49」⇒ 本行照填、零加減推算。
    # 方向是**上修＝判準更嚴**（下限愈高，對「掃描面靜默縮小」的鑑別力愈強），不是放寬門檻。
    # 🔴 與 R83 那一段不同，**49 只在實測恰為 62 時成立**（`int(62×0.8)=49`、`int(63×0.8)=50`）
    # ——上一段刻意挑「對兩個檔數皆成立」的值，本輪沒有這種值可挑（公式在 62↔63 之間就跨格），
    ...
    # 🔴 R85 P2：209 → 210（**方向是收緊**：下限拉高＝要求更大的掃描面）。上一段預告的
    # 那件事真的發生了——`AutoClaude/tests` 在 P3 持續交付期間由 262 一路長到 264（非本包），
    # 第三向逐字指示「請重釘為 211」，本行照填、不做加減推算；P3 再新增鎖檔時本向會再度
    # 逐字給出新值，收尾單人窗口請在所有包停工後重量一次。
    # 🔴 R85 收尾單人窗口：211 → 212（**方向仍是收緊**）。上一段那句「收尾單人窗口請在
    # 所有包停工後重量一次」正是本行——**十二包全部停工後**實測 `AutoClaude/tests` ＝ 265
    # （成長來源是新目錄 `AutoClaude/tests/execution/`，P7 三支 ＋ P11 一支），第三向逐字
    # 指示「請重釘為 212」，本行照填、零加減推算。
    # 🔴 本輪多一筆值得記的成本：這一紅會讓 `tools/run_root_unittests.py` **在靜態標籤掃描
    # 階段就短路**——連一支測試都不跑，而畫面上沒有任何 FAIL 行。⇒「沒有 FAIL 行」不等於
    # 測試通過（R85 P11 實測發現，改用 `pytest tools/tests` 才拿到真實面）。判 rc 時要知道
    # 有這個早退分支存在。
    # 🔴 R86 Dev 包：212 → 213（**方向仍是收緊**）。與上面幾段不同的是**歸因**：這一次
    # 是**本包造成的**，不是「非本包」——新增 `AutoClaude/tests/test_r86_pace_contract.py`
    # 讓該樹由 266 長到 **267**，而公式在 266↔267 之間正好跨格
    # （`int(266×0.8)=212` ⇒ 綠；`int(267×0.8)=213` ⇒ 紅），第三向逐字指示「請重釘為 213」，
    # 本行照填、零加減推算（同上四段紀律）。
    # 🔴 上一段那個早退分支的代價**本輪真的付了一次**：Dev 包只跑了 AutoClaude 子專案閘門
...
_SITE_CLASS_CENSUS: dict[str, dict[str, int]] = {
    ...
    "tools/tests": {
        ...
        # 🔴 R115 重釘 `runtime-skipTest` 22→24  # round-label-ok：早於帳本歸檔輪
        # （**非放寬**，同上：本表判準是「相等」）。
