...
#: 🔴 帳本結案輪（`DEF-101-752` 結案）追加 44：該列結案並各欄瘦身成 ≤700 bytes 索引
#: （原文逐字保全於 `CrossPlatform_R82_DEF101752_Untracked_Scan_Closure.md` §3.1）
#: ⇒ 豁免過期移除（`--repin-oversize` 機械收緊，當回合實測值直接填入）。
#: 🔴 結構性長債分軌輪（掌舵者 2026-08-30 裁決）追加 43：`DEF-101-018` 遷入
#: `AutoSDD_Structural_Debt_Log.md` 並瘦身至 ≤700 bytes ⇒ 豁免過期移除
#: （`--repin-oversize` 機械收緊，當回合實測值直接填入）。
#: 🔴 R121（round-label-ok）純結案輪追加 42：`DEF-101-867` closed-by-decision 後瘦身至
#: 588 bytes ⇒ 豁免過期移除（`--repin-oversize` 機械收緊，當回合實測值直接填入）。
#: 🔴 帳本可寫性修復輪（round-label-ok）追加 36：`DEF-101-675`／`736`／`796`／`803`／
#: `856`／`887` 六列瘦身跌破 700 bytes（史料逐字搬進 `CrossPlatform_R124_Row_Slimming.md`）
#: ⇒ 豁免過期移除（`--repin-oversize` 機械收緊，當回合實測值直接填入）。
OVERSIZE_ROW_CEILING_HISTORY: tuple[int, ...] = (
    105, 101, 98, 85, 83, 66, 63, 62, 45, 44, 43, 42, 36)
...
#: 追加 36796：帳本結案輪（結案包 #1）逐筆結案，多列超標豁免列的狀態欄瘦身成
#: 「短句＋指針」（長證據移入 CrossPlatform_R107_Ledger_Closure.md），membership
#: 不變（45 列皆仍 >700），僅超標量淨下降 262（37058−36796，當回合實測值直接填入）。
#: 追加 36440：帳本結案輪（收尾書記落列，2026-08-28）13 筆結案列狀態欄改寫為
#: 「首詞＋指針」（敘事移入 CrossPlatform_R107_Ledger_Closure.md 對照節），
#: `DEF-101-338` −381／`DEF-101-559` +25，membership 不變（45 列皆仍 >700），
#: 淨下降 356（36796−36440，當回合實測值直接填入）。
#: 🔴 帳本結案輪（`DEF-101-752` 結案）追加 34712：該列 2428 bytes 瘦身至 ≤700
#: （−1728 恰為其原超標量），豁免過期移除（`--repin-oversize` 機械收緊，方向只有降，
#: 當回合實測值直接填入、零推算）。
#: 🔴 結構性長債分軌輪（掌舵者 2026-08-30 裁決）追加 27682：7 列遷入
#: `AutoSDD_Structural_Debt_Log.md` 並把狀態欄改寫為 closed-by-decision 索引句，
#: 其中 5 列原在豁免清單（`DEF-101-018` 跌破 700 過期移除；398／701／702／886 仍 >700、
#: 僅超標量下降），方向只有降（`--repin-oversize` 機械收緊，當回合實測值直接填入、零推算）。
#: 🔴 同輪追加 27657：已結列殘留待辦清理（DEF-101-338／559 分流去向欄改寫，−25 bytes，
#: 兩列仍 >700 僅超標量下降），`--repin-oversize` 機械收緊、當回合實測值直接填入。
#: 🔴 R121（round-label-ok）純結案輪追加 25520：`DEF-101-867` 瘦身跌破 700 過期移除、
#: 多列 closed-by-decision 索引化使既有超標列略瘦（`--repin-oversize` 機械收緊，實測值）。
#: 🔴 帳本可寫性修復輪（round-label-ok）追加 20027：同上六列瘦身，超標總量隨之下降
#: （`--repin-oversize` 機械收緊，當回合實測值直接填入、零推算）。
OVERSIZE_ROW_EXCESS_CEILING_HISTORY: tuple[int, ...] = (
    ...
    37081, 37058, 36796, 36440, 34712, 27682, 27657, 25520, 20027,
)
...
_SEALED_HISTORY_PREFIXES: dict[str, tuple[int, ...]] = {
    ...
    # 帳本結案輪（`DEF-101-752` 結案）：兩條史料各追加一格（44／34712），故把 45 與
    # 36440 各一併封入（同上，只在尾端延長、零改寫）。
    # 結構性長債分軌輪（2026-08-30）：兩條史料各追加一格（43／27682），故把 44 與
    # 34712 各一併封入（同上，只在尾端延長、零改寫）。同輪殘留待辦清理再追加 27657，
    # 故把 27682 一併封入（同上）。
    # R121（round-label-ok）純結案輪：兩條史料各追加一格（42／25520），故把 43 與 27657
    # 各一併封入（同上）。帳本可寫性修復輪（round-label-ok）：兩條史料再各追加一格
    # （36／20027），故把 42 與 25520 各一併封入（同上，只在尾端延長、零改寫）。
    "OVERSIZE_ROW_CEILING": (105, 101, 98, 85, 83, 66, 63, 62, 45, 44, 43, 42),
    "OVERSIZE_ROW_EXCESS_CEILING": (
        162282, 147944, 147455, 146210, 143303, 140957, 138938, 138936, 123867,
    121758, 82896, 77186, 75047, 69122, 48864, 40889, 40884, 40759, 37082,
    37081, 37058, 36796, 36440, 34712, 27682, 27657, 25520,
    ),
...
#: 🔴 帳本結案輪（結案包 #1）重釘 **35 → 36**：`OVERSIZE_ROW_EXCESS_CEILING` 封印再
#: 延長一格納入 37058（8+21+7=36），當回合實測值直接填入。
#: 🔴 帳本結案輪（`DEF-101-752` 結案）重釘 **37 → 39**：兩條封印各延長一格納入
#: 45 與 36440（9+23+7=39），當回合實測值直接填入。
#: 🔴 結構性長債分軌輪（2026-08-30）重釘 **39 → 42**：CEILING 封印延長一格納入 44、
#: EXCESS 封印延長兩格納入 34712 與 27682（同輪兩段收緊：遷軌＋殘留清理，依
#: `_SEAL_TAIL_MAX=1` 只留最新 27657 當尾巴）＝10+25+7=42，當回合實測值直接填入。
#: 🔴 R121（round-label-ok）純結案輪重釘 **42 → 44**：CEILING 封印延長一格納入 43、
#: EXCESS 封印延長一格納入 27657（`DEF-101-867` 過期移除觸發兩條史料各追加一格，
#: `_SEAL_TAIL_MAX=1` 只留最新 42／25520 當尾巴）＝11+26+7=44，當回合實測值直接填入。
#: 🔴 帳本可寫性修復輪（round-label-ok）重釘 **44 → 46**：CEILING 封印延長一格納入 42、
#: EXCESS 封印延長一格納入 25520（六列瘦身跌破 700 觸發兩條史料各追加一格，
#: `_SEAL_TAIL_MAX=1` 只留最新 36／20027 當尾巴）＝12+27+7=46，當回合實測值直接填入。
_SEAL_TOTAL_MIN_LEN = 46
...
#: 🔴 帳本結案輪（結案包 #1）重釘（封印延長納入 37058，當回合 `seal_table_digest()` 實測值）。
#: 🔴 收尾書記落列（2026-08-28）重釘（封印延長納入 36796，當回合實測值，量法同上）。
#: 🔴 帳本結案輪（`DEF-101-752` 結案，2026-08-29）重釘（兩條封印各延長一格納入
#: 45／36440，當回合 `seal_table_digest()` 實測值）。
#: 🔴 結構性長債分軌輪（2026-08-30）重釘（CEILING 封印延長一格納入 44、EXCESS 封印
#: 延長兩格納入 34712／27682，當回合 `seal_table_digest()` 實測值）。
#: 🔴 R121（round-label-ok）純結案輪重釘（兩條封印各延長一格納入 43／27657，
#: 當回合 `seal_table_digest()` 實測值）。
#: 🔴 帳本可寫性修復輪（round-label-ok）重釘（兩條封印各延長一格納入 42／25520，
#: 當回合 `seal_table_digest()` 實測值）。
_SEAL_TABLE_SHA256 = "6d2270c25ce55b88"
