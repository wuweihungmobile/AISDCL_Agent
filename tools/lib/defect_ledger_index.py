...
DEF-101-649 DEF-101-675 DEF-101-701 DEF-101-702 DEF-101-736
DEF-101-739 DEF-101-740 DEF-101-747 DEF-101-748 DEF-101-769
DEF-101-790 DEF-101-794 DEF-101-796 DEF-101-801 DEF-101-802
DEF-101-803 DEF-101-810 DEF-101-856 DEF-101-867 DEF-101-871
DEF-101-872 DEF-101-876 DEF-101-878 DEF-101-880 DEF-101-886
DEF-101-887 DEF-101-888 DEF-200-190
""".split())
...
#: 🔴 帳本瘦身包重釘 **66 → 63**（往下改）：`DEF-101-377`／`388`（依各自分流欄自述
#: 「不修復」訂正首詞為 `wontfix`，判例同 `DEF-101-359`／`382`）與 `DEF-101-746`
#: （併列結案，單一載體收斂至 `DEF-101-701`）三列**結案並瘦身成索引**，原文逐字保全於
#: `CrossPlatform_R89_Closure_Evidence.md`〈帳本瘦身批〉節，瘦身後皆 ≤700 bytes ⇒ 依②向
#: 訊息**自己指名**的動作移除，不是放寬。本值＝當回合實測超標列數，零加減推算。
#: 🔴 帳本減半波（單一書記收斂）重釘 **63 → 62**：`DEF-101-596`（測試改用可注入
#: `outbox_root`＋`mkdtemp()`）狀態欄瘦身結案，跌破 700 bytes ⇒ 豁免過期，自上方清單
#: 移除。本輪另有 20 餘筆存量列狀態欄改寫瘦身（結案／移入外部阻塞軌／B 類複驗附記），
...
#: 因為本次下修而改變，只是分母變小了。
#: 🔴 帳本瘦身包重釘 **82896 → 77186**（−5710，當回合實測值直接填入、零推算）。
#: 淨額**同時有升有降**，兩邊都寫出來而不是只報淨值：
#:   · **降**：`DEF-101-377`（4247→636）／`388`（2967→697）／`746`（974→655）三列結案並
#:     瘦身成索引，原文逐字搬進 `CrossPlatform_R89_Closure_Evidence.md`〈帳本瘦身批〉節；
#:     三列連同各自的超標量一起離開分子。
#:   · **升**：`DEF-101-701` 因吸收 `DEF-101-746`（兩列自 R81 起即載明「合併承接」，
#:     同一個「重釘取當下實測 × 收輪是多波次」結構衝突）而就地追加一句「我吸收了哪一列」＋
#:     併入的解鎖條件，長 **378 bytes**——那是併列結案的**資訊零損失義務**，方向與本棘輪
#:     相反，所以它必須被上面那三列吐出來的位元組付掉（`DEF-101-957` 形態，R80／R82 已各
#:     實證一次）。誠實記在這裡：−5710 是付完這筆成長之後的數。
#: 🔴 帳本減半波（單一書記收斂）重釘 **69122 → 48864**（−20258，當回合實測值直接
#: 填入、零推算）。淨額**只有降沒有升**：本輪逐筆重寫 20 餘筆存量超標列的狀態欄
#: （四包回報結案／B 類複驗維持未結但附短註／E 類移入外部阻塞軌），全數改寫為「一句
...
OVERSIZE_ROW_EXCESS_CEILING = 27657
