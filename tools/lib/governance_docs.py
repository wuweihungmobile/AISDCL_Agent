...
_GOVERNANCE_DOCS = (
    ...
    # DEF-101-587 拆分慣例：入口檔逼近 Read 工具讀取上限時的姊妹檔（承載
    # MIN_TESTS 逐輪重釘沿革，第五輪四方獨立複審自源碼搬遷至此）。即刻登記。
    _REPO_ROOT / "docs" / "06_quality" / "CrossPlatform_Guard_Line_History_MinTests.md",
    ...
    # 喚醒鏈 PRD 四方複審輪證據檔：DEF-200-063/147 結案憑證與 DEF-200-238 立案探針逐字
    # 唯一居所在本檔（⇒ 體積守門），且逐節寫出「哪筆結案/立案的憑證在哪一節」（⇒ 指針稽核）。
    ...
    # 本檔（⇒ 體積守門），`_GUARD_LINES_REPIN_LOG`／`_FROZEN_PREFIX_REWRITE_LEDGER`
    # R119 round-label-ok 列皆指名本檔（⇒ 指針稽核）。即刻登記。
    ...
    # SessionGate／status line 六包收尾證據（DEF-200-333～342 的詳情面）：同一資格，即刻登記。
