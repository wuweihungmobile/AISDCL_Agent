...
# `env-disabled` 的應然值**不是**一律 0：它混住兩種成分——(a) 缺件型（沒人記得設環境
# 變數；R79 證明：這台機器一件缺件都沒有，設三個環境變數就消 92 支）該清到 0；
# (b) **設計上永久 opt-in** 型清不掉也不該清。`DEF-200-255` 落款的判例：pgvector recall
# 的 p95 延遲 SLA 三次獨立實測（50.59／51.32／51.703ms）都與 50ms 門檻幾乎重合，R82 一度
# 把它接進 conftest 自動打開、當回合就撞 51.703ms 而主動退回 opt-in ⇒ 自動化它等於製造
# flaky 閘門。把 (b) 也算進「該清到 0」，下一個讀這段的人會照著把它打開——那正是 R82
# 已經踩過一次的坑，而政策自述若與帳本落款相反，機械層擋不住這種重演（群天花板只看
# 群總數、不看群內成分）。
