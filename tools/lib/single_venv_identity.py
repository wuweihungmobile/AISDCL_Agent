...
def single_venv_identity_problems(
    ...
    """宣告的 Windows venv 載具，用**這份 settings 自己的專案根**展開＋normpath
    後，必須等於 `<repo_root>/.venv/Scripts/pythonw.exe`——repo 唯一那一顆。
    ...
    for carrier in sorted(hook_wiring.declared_win_carriers(settings)):
        ...
        if resolved != canonical:
            problems.append(
                ...
                f"normpath 後為 {resolved}，非 repo 唯一的根層 venv {canonical}——"
