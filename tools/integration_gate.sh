...
# tools/check_hooks_liveness.py（單一真相源，供本檔與 AutoClaude/tools/local_ci_gate.sh 呼叫）；
# PYTHONUTF8=1（見上）第二層防護非 UTF-8 終端下的中文 print() 崩潰（對齊
# AutoClaude/tools/local_ci_gate.sh 既有做法）。
if [ -z "${CI:-}" ]; then
  _hl_script="$ROOT/tools/check_hooks_liveness.py"
