...
class TestHandoffProseDetectionCoversAlternatePhrasing(unittest.TestCase):
    ...
    正樣本史料（`archive_35` 的 `DEF-101-614`／`DEF-101-615`，R63 前舊版
    `HANDOFF_PROSE_RE` 的用詞盲區）搬遷，原文＝Guard_Repin 證據檔 §E-11。
...
class TestConservationGuardsAreExplicitNotAssert(unittest.TestCase):
    ...
    def test_apply_writes_lf_only_and_conserves_bytes_on_a_faithful_copy(self):
        ...
        沒有這一條，上一條測試無法證明保全不是「恆擋」（恆擋一樣會讓上一條綠）。三筆修訂
        史料（不依賴 live 帳本當下可搬列數／round 2 必須用多列坐實／round 3 守恆算式含
        `added`）搬遷，原文＝Guard_Repin 證據檔 §E-13。
        """
        with _ledger_sandbox():
...
# ============================================================================
# 併檔理由（DEF-101-561③ 棘輪沿革）、R68 動工前實測背景與四條裁決逐條（①採納改根因解
# ②③駁回見 TestOpenBacklogArchiveIsRejected／TestHardLineIsToolFact ④判準②收窄）全文
# 搬至 docs/06_quality/CrossPlatform_Guard_Line_History.md〈R68 帳本容量政策裁決史〉節。
