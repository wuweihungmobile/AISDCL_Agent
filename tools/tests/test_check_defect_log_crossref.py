...
class TestRetainedRangeClaims(unittest.TestCase):
    ...
    **這支測試自己的歷史是本輪最貴的一課，五輪都在同一個地方犯同一種錯**：
      ...
      * round 9：拆掉終點、縮為 4 支。round 10 四方又全數 REJECT——①述詞不認**粗體**，
        ...
        判準被打壞也永遠綠。
      * round 10：述詞收粗體、補正向半邊（但 `ledger_text` **仍是死參數**，round 11 才刪掉）、
        每支 fixture 都給 archive 鍵、
    ...
    **設計紀律（從上面四輪蒸餾出來的）**：
    ...
    def test_all_rows_retained_is_green(self) -> None:
        ...
        self.assertEqual(
            m.retained_range_problems({"main": led + self._CLAIM, "archive_29.md": ""}), []
        )
    ...
    def test_a_row_moved_into_archive_is_red(self) -> None:
        ...
        problems = m.retained_range_problems(
            {"main": led + self._CLAIM, "archive_29.md": arch}
        )
    ...
    def test_a_row_deleted_outright_is_red(self) -> None:
        ...
        problems = m.retained_range_problems(
            {"main": led + self._CLAIM, "archive_29.md": ""}
        )
        self.assertTrue(
    ...
    def test_bold_claim_form_is_matched(self) -> None:
        ...
        arch = self._HDR + self._row(508)
        problems = m.retained_range_problems(
            {"main": led + self._CLAIM_BOLD, "archive_29.md": arch}
        )
    ...
    def test_rows_below_the_start_may_be_archived(self) -> None:
        ...
        self.assertEqual(
            m.retained_range_problems({"main": led + self._CLAIM, "archive_29.md": arch}), []
        )
    ...
    def test_quoted_historical_wording_is_not_matched(self) -> None:
        ...
        self.assertEqual(
            m.retained_range_problems({"main": led + quotes, "archive_29.md": arch}), []
        )
    ...
    def test_predicate_covers_every_real_claim_site(self) -> None:
        ...
        本鎖把那個計數變成機械斷言：**凡「自身全部條目」後 40 字內同時出現「三位數編號」與
        「起」者即為活宣稱**，全部必須被 `_RETAINED_START_RE` 認出。

        🔴 **round 11 訂正（四方全數 REJECT、其中 3 項 P1）**：初版判別式只要求窗內有「起」，
        理由寫「形狀說明用 `NNN~MMM`、引述歷史原文用較窄的『自身條目』，兩者都不含『起』」
        ——**而那句話本身就是一句含「起」的形狀說明**。家規要求每輪複審紀錄列寫清楚修了什麼，
        於是 round 10 那一列描述本鎖判準的散文同時滿足「含『自身全部條目』」與「40 字內有『起』」
        ⇒ 被算成活宣稱、`_RETAINED_START_RE` 自然抓不到 ⇒ **假陽性，整個根層全套 FAILED**。
        更嚴重的是主控**在寫那一列之前**量到「846 OK」並把它當收輪定案值寫進 ONBOARDING §7
        ⇒ 基線 SSOT 記著一個綠燈而實況是紅燈（SA 判 P1，理由與他 round 1 立案的同一缺陷類相同）。
        **這是 round 9 紀律「同一列內不得記載會被本列自身寫入動作改變的量測值」的加強版形態**：
        本次寫入改變的不是位元數，而是**測試結果本身**；故本輪另訂收尾順序紀律
        （見 `ONBOARDING.md` §7）——**先寫完所有文件（含帳本列），才跑全套取證、才回填 §7**。
        判別式改為「須同時有三位數編號」：真宣稱必帶編號（粗體／空白／全半角括號變體皆然），
        純散文不帶（本鎖的規則敘述窗內只有「40」這種兩位數）。
        """
        ...
        for path in targets:
            ...
            for lineno, line in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
                for mm in re.finditer("自身全部條目", line):
                    ...
                    # 活宣稱＝窗內**同時**有三位數編號與「起」。少了前者，本鎖會把「描述本鎖
                    # 判準的散文」算成宣稱而假陽性（round 11 的 P1 就是這樣讓全套翻紅的）。
                    if "起" not in seg or not re.search(r"\d{3}", seg):
                        continue
                    total += 1
    ...
    def test_meta_prose_about_this_lock_is_not_counted_as_a_claim(self) -> None:
        """**描述本鎖判準的散文**不得被算成活宣稱（round 11 的 P1 就是這個缺的 fixture）。

        家規要求每輪複審紀錄列寫清楚修了什麼 ⇒ 帳本必然會出現描述本鎖的 meta 散文。
        初版判別式（只要求窗內有「起」）把它算成宣稱 ⇒ 整個根層全套 FAILED，而且
        **只要下一輪再描述這條鎖一次就會再紅一次**（自我觸發）。本 fixture 把那個類別釘住。
        """
        prose = (
            "> 凡『自身全部條目』後 40 字內同時出現三位數編號與『起』者即為活宣稱，"
            "全部必須被述詞認出\n"
        )
        seg_hits = [
            mm for mm in re.finditer("自身全部條目", prose)
            if "起" in prose[mm.start():mm.start() + 40]
            and re.search(r"\d{3}", prose[mm.start():mm.start() + 40])
        ]
        self.assertEqual(
            seg_hits, [],
            "描述本鎖的 meta 散文被算成活宣稱 ⇒ 平價鎖會自我觸發（round 11 P1 的形態）",
        )
