...
_GUARD_LINES_REPIN_LOG: tuple[tuple[str, int, int, int, str], ...] = (
    ...
    ("R143", 96506, 96671, 165,
     "[非淨減法輪][回歸鎖軌全額申報，見 _REGRESSION_LANE_LOG 同輪列] DEF-200-275 第四輪：新增 "
     "tools/tests/test_context_window_parity.py（根層 context_budget_guard ↔ SDD LATEST "
     "context_window 的 parity 鎖，+145）＋本檔自身逐檔漂移（主表本列＋本軌新列＋"
     "_REPIN_NET_CAP_SCHEDULE 到期兌現列與重新武裝）＋test_platform_neutral_paths.py 的 "
     "_DIRENT_UNGUARDED_DEBT 42→40 下修註解（+3），全額歸回歸鎖軌。逐檔清單見 "
     "docs/06_quality/CrossPlatform_R143_Scan_Findings.md；證據見 "
     "docs/06_quality/CrossPlatform_DEF200275_Context_Metering_Evidence.md〈第四輪〉。"),
    ("R143", 96671, 96744, 73,
     "[非淨減法輪][同輪追加][回歸鎖軌全額申報，見 _REGRESSION_LANE_LOG 同輪列] DEF-200-275 "
     "第四輪第 1 輪審查修復（SA-R4-03）：test_find_git_bash_parity.py::"
     "TestIntegrationGateShellDelegation 補三案候選鏈鑑別（python3 退路／.venv/bin/python "
     "退路／全無 ⇒ rc=1 fail-loud；+60；bug-injection 自證：拔掉兩個退路即 2 失敗）＋本檔"
     "自身逐檔漂移（本列＋回歸鎖軌同輪列＋前綴鏈列，+13），全額歸回歸鎖軌。逐檔清單見 "
     "docs/06_quality/CrossPlatform_R143_Scan_Findings.md §2；證據見同目錄 DEF200275 證據檔。"),
    ("R143", 96744, 97056, 312,
     "[非淨減法輪] R143 同輪三度追加：DEF-200-278 喚醒鏈缺口收尾——新增"
     "test_wake_chain_halt_r278.py（quota_gate halt 交棒 fallback／halt 標記讀寫／"
     "halt_verdict／sentinel_decide halt 分支／check_claim_provenance 詞表擴充的"
     "回歸鎖，+299）＋本檔自身逐檔漂移（新增本列本身，+12）。全額歸本軌"
     "（記帳誠實度分類，非必要湊額）。逐檔清單＝"
    ...
    ("R146", 98236, 98196, -40,
     "[淨減法輪] DEF-200-275 第七輪收尾單人窗口重釘：package A（D27 根層⑥階查表收斂＋"
     "sdd_latest 快取）／package B（D28 telemetry writeback 守衛）對 "
     "test_context_window_parity／test_context_budget_guard／test_root_guard_known_"
     "model_r145 的合法新增，加上 Dev-Trim7 十二支測試檔散文搬遷抵銷，造成本表多檔逐檔漂移"
     "（淨額為負，不受款(9)[未附刪除清單]約束；逐檔清單以下表為準）。逐檔清單＝"
     "CrossPlatform_R145_Scan_Findings.md〈第七輪附記〉節。"),
    ("R146", 98196, 98211, 15,
    ...
    ("R146", 98181, 98195, 14,
     ...
     "逐檔清單沿用 CrossPlatform_R145_Scan_Findings.md〈第七輪附記（R146）〉節。"),
    ("R147", 98195, 98823, 628,
     "[非淨減法輪] DEF-200-274 第九輪收尾單人窗口重釘：test_run_root_unittests.py +610"
     "（P1/P0 zshrc 假紅根治＝ShouldRunParallelDecisionTest／RunWithFloorNeverParallelize"
     "SyntheticTreeTest／`_zero_dep_child_env()`＋ZeroDepProbeForcesSequentialChildEnvTest"
     " 119 行；既有缺陷修復＝ParallelTimingCacheLoadHintsTest（快取壞檔容錯＋bool誤判計"
     "時回歸）／ParallelShardSigtermCleanupTest／ParallelShardSigtermIgnoredOffMainThread"
     "Test 151 行，兩者全額申報回歸鎖軌；LPT 排序／保存／過期回報／大批持久化＋自動細分＋"
     "不均偵測v2＋worker 數印出＋既有 cap 8→9 調整 340 行全額歸功能軌）＋test_pre_push_"
     "dispatcher.py +18（compileall 遷移語法偵測回歸鎖，功能軌）。分軌詳見 "
     "_REGRESSION_LANE_LOG 本輪列。逐檔清單沿用 CrossPlatform_R145_Scan_Findings.md"
     "〈第七輪附記〉節之後續，詳細版見 docs/06_quality/CrossPlatform_DEF200274_Parallel_"
     "Tests_Evidence.md〈第九輪〉。本列、到期兌現列、重新武裝與軌表列的自身行數已計入淨"
     "額。"),
    ("R147", 98823, 98835, 12,
     "[非淨減法輪][同輪追加] 本檔自身逐檔漂移：上一列稽核列本身造成的十二行漂移，同 "
     "R95~R146 既有體例。逐檔清單沿用 CrossPlatform_R145_Scan_Findings.md〈R147 沿用〉。"),
    ("R147", 98835, 98838, 3,
     "[非淨減法輪][同輪追加] 收斂列：上一列本身造成的三行漂移。逐檔清單沿用 "
     "CrossPlatform_R145_Scan_Findings.md〈R147 沿用〉。"),
    ("R147", 98838, 98840, 2,
     "[非淨減法輪][同輪追加] 收斂列：兩行漂移。逐檔清單沿用 "
     "CrossPlatform_R145_Scan_Findings.md〈R147 沿用〉。"),
    ("R147", 98840, 98841, 1,
     "[非淨減法輪][同輪追加] 收斂列：一行漂移。逐檔清單沿用 "
     "CrossPlatform_R145_Scan_Findings.md〈R147 沿用〉。"),
    ("R147", 98841, 98861, 20,
     "[非淨減法輪][同輪追加] 收斂列：_REGRESSION_LANE_LOG 新列＋"
     "_REPIN_NET_CAP_SCHEDULE 到期兌現列與重新武裝造成的二十行漂移。逐檔清單沿用 "
     "CrossPlatform_R145_Scan_Findings.md〈R147 沿用〉。"),
    ("R147", 98861, 98865, 4,
     "[非淨減法輪][同輪追加] 收斂列：上一列本身造成的四行漂移。逐檔清單沿用 "
     "CrossPlatform_R145_Scan_Findings.md〈R147 沿用〉。"),
    ("R147", 98865, 98868, 3,
     "[非淨減法輪][同輪追加] 收斂列：上一列本身造成的三行漂移。逐檔清單沿用 "
     "CrossPlatform_R145_Scan_Findings.md〈R147 沿用〉。"),
    ("R147", 98868, 98871, 3,
     "[非淨減法輪][同輪追加] 收斂列，見 "
     "CrossPlatform_R145_Scan_Findings.md〈R147〉。"),
    ("R147", 98871, 98872, 1,
     "[非淨減法輪][同輪追加] 收斂列，"
    ...
    ("R147", 98886, 98890, 4,
     "[非淨減法輪][同輪追加] 收斂列，"
     "CrossPlatform_R145_Scan_Findings.md〈續3〉。"),
    ("R147", 98890, 98891, 1,
     "[非淨減法輪][同輪追加] 收斂列，"
     "CrossPlatform_R145_Scan_Findings.md〈續4〉。"),
    ("R147", 98891, 98892, 1,
     "[非淨減法輪][同輪追加] 收斂列，"
     "CrossPlatform_R145_Scan_Findings.md〈續5〉。"),
    ("R147", 98892, 98893, 1,
     "[非淨減法輪][同輪追加] 收斂列，"
     "CrossPlatform_R145_Scan_Findings.md〈續6〉。"),
    ("R147", 98893, 98895, 2,
     "[非淨減法輪][同輪追加] 收斂列（含本列自身），"
     "CrossPlatform_R145_Scan_Findings.md〈續7〉。"),
    ("R147", 98895, 98911, 16,
     "[非淨減法輪][同輪追加] 收斂列：E501 存量債棘輪折行（把過長單行列拆成雙行，"
     "折行本身造成的十六行漂移）。逐檔清單沿用 "
     "CrossPlatform_R145_Scan_Findings.md〈R147 沿用〉。"),
    ("R147", 98911, 98915, 4,
     "[非淨減法輪][同輪追加] 收斂列，"
     "CrossPlatform_R145_Scan_Findings.md〈續8〉。"),
    ("R147", 98915, 98918, 3,
     "[非淨減法輪][同輪追加] 收斂列，"
     "CrossPlatform_R145_Scan_Findings.md〈續9〉。"),
    ("R147", 98918, 98921, 3,
     "[非淨減法輪][同輪追加] 收斂列，"
     "CrossPlatform_R145_Scan_Findings.md〈續10〉。"),
    ("R147", 98921, 98927, 6,
    ...
    ("R150", 99825, 99845, 20,
     ...
     "CrossPlatform_R145_Scan_Findings.md〈R150 沿用〉。"),
    ("R151", 99845, 99812, -33,  # round-label-ok：DEF-200-301～306 單一 .venv 收斂收尾，淨減法輪
     "[淨減法輪] DEF-200-301（hook 載具單一 .venv 身分鎖，新檔 test_single_venv_identity.py）／"
     "DEF-200-302（nightly 釘根層 .venv：test_nightly_interpreter_determinism.py 刪 A／D 兩類、"
     "增 E 類）／DEF-200-303（剖面 re-key：test_skip_ceiling_ratchet_direction.py）／"
    ...
    ("R155", 100691, 101503, 812,  # round-label-ok：DEF-200-315／321／322／323 收尾
     "[非淨減法輪] DEF-200-315／321／322／323 收尾（互動式入口 SSOT 化／root-infra-ci "
     "jq 判準／雜散 venv 守衛補洞／uv 第二顆 venv）的新增測試涵蓋："
     "test_nightly_interpreter_determinism.py（H 項新判準能力鎖：本機缺席 fail-loud／"
     "CI 容許 PATH／逃生口三情境）為最大宗，其餘散落 windowsapps_guard 兩支、"
     "test_dev_start.py、test_pre_commit_dispatcher_sigpipe.py、"
     "test_workflow_permission_concurrency_lock.py 等檔。分軌見 _REGRESSION_LANE_LOG "
     "同輪列；逐檔清單見 CrossPlatform_R145_Scan_Findings.md〈第十七輪附記（R155）〉。"),
    ("R155", 101503, 101532, 29,  # round-label-ok：本表自身重釘漂移（單一收斂列，含本列自身）
     "[非淨減法輪][同輪追加] 收斂列：本表（test_adr_xplat001_c1c2_lock.py 自身）新增"
     "上一列、`_REGRESSION_LANE_LOG` 同輪新列、`_REPIN_NET_CAP_SCHEDULE` 到期義務"
     "兌現列 (155, 538)＋重新武裝 157／537、`_FROZEN_PREFIX_REWRITE_LEDGER` 接鏈列與"
    ...
    ("R159", 102515, 102842, 327,  # round-label-ok：多 CPU 第十五輪 DEF-200-345～348 收尾單人窗口
     "[非淨減法輪] 四方獨立審查覆核前輪宣稱後三包修復：test_cpu_budget +153（DEF-200-347 免 "
     "psutil 偵測：DetectPhysicalCountTest +35 結案鎖、平台解析／分派鎖 +118 功能軌）、"
     "test_context_budget_guard +78＋劃界 docstring +3（DEF-200-346 pid 歸因）、"
     "test_check_defect_log_crossref +42（DEF-200-345 整班隔離）、test_ci_gate_xdist_allowlist "
     "+25（DEF-200-348 廣播文字鎖）＋本表自身漂移（主表五格、回歸鎖軌新列、cap 到期兌現 "
     "(159, 536)＋重新武裝 161／535、接鏈列、凍結前綴 238→239）。180 行轉入回歸鎖軌"
     "（A 120＋C 25＋B 結案鎖 35），主軌＝本列淨額−180；連續上升第二輪（上限 2），"
     "下一輪主軌必須 ≤0。逐檔清單見 CrossPlatform_R145_Scan_Findings.md〈第二十輪附記（R159）〉。"),
    ("R163", 102842, 102914, 72,  # round-label-ok：多 CPU 第十九輪 DEF-200-354 收尾單人窗口
     "[非淨減法輪][回歸鎖軌全額申報，見 _REGRESSION_LANE_LOG 同輪列] 四方獨立審查覆核前輪宣稱後 "
     "QA 全洩漏矩陣命中 DEF-200-354：test_wake_chain_halt_r278 +53（setUpModule ENV_SPEC pin "
     "＋EnvSpecLeakIsolationTest 兩支巢狀鎖；純史料一段搬 R86 §G）＋本表自身漂移 +19（本列＋回歸"
     "鎖軌列＋cap 到期兌現 (163, 535) 並重新武裝 165／534＋ROOT-TOOLS 展延 162→168＋接鏈列＋凍結"
    ...
    ("R170", 105176, 105179, 3,  # round-label-ok
     "[非淨減法輪][同輪追加] 詳 CrossPlatform_R145_Scan_Findings.md。"),
    ("R171", 105179, 105804, 625,  # round-label-ok：系列收斂後四方重驗收尾單人窗口
     "[非淨減法輪] 系列收斂後四方重驗收尾：test_workflow_permission_concurrency_lock.py 新增 "
     "TestFsmChaosNightlyStreakReadsJobLayer（chaos-latest job 層連敗隔離回歸鎖，DEF-200-379）、"
     "test_archive_defect_log.py 新增穩定快照自證與正樣本（DEF-200-382）、test_dev_start.py／"
     "test_platform_neutral_paths.py／test_doc_loc_baseline_freshness_r60.py 效能熱點修復回歸鎖"
     "（DEF-200-383）、test_ci_gate_xdist_allowlist.py 普查掃描面補站（DEF-200-384）、"
     "test_block_destructive_git_r83.py cwd 密封回歸鎖（DEF-200-385）。逐項見 "
     "CrossPlatform_R145_Scan_Findings.md〈附記（R171）〉。"),
    ("R171", 105804, 105836, 32,  # round-label-ok
     "[非淨減法輪][同輪追加] 收斂列：本表自身漂移（_FROZEN_GUARD_LINES 逐檔數字更新＋"
     "_REGRESSION_LANE_LOG 新列＋_REPIN_APPROVED_ROUND_OVERAGE 新列與名冊上限註解＋"
     "_REPIN_NET_CAP_SCHEDULE 到期兌現列與重新武裝）。詳 "
     "CrossPlatform_R145_Scan_Findings.md〈附記（R171）〉。"),
    ("R171", 105836, 105841, 5,  # round-label-ok
     "[非淨減法輪][同輪追加] 收斂列：上一列自身漂移。詳 "
     "CrossPlatform_R145_Scan_Findings.md〈附記（R171）〉。"),
    ("R171", 105841, 105844, 3,  # round-label-ok
     "[非淨減法輪][同輪追加] 收斂列：上一列自身漂移。詳 CrossPlatform_R145_Scan_Findings.md。"),
    ("R171", 105844, 105860, 16,  # round-label-ok
...
_REPIN_APPROVED_ROUND_OVERAGE: dict[str, tuple[int, str]] = {
    ...
    "R171": (372, (  # round-label-ok：掌舵者核准的一次性例外，非帳本追蹤輪號
        "掌舵者（2026-09-25，系列收斂後四方重驗）核准本輪一次性例外：本筆熄滅的是款(11)"
        "[只升不降]（R169／R170 主軌淨額已連續兩輪為正，本輪為第三輪）——主軌淨額全數為四方複"
        "審要求的結案回歸鎖與自證測試＋測速修補（已扣除回歸鎖軌部分申報 309），未刪任何既有測"
        "試以保鑑別力，同輪亦無等量舊鎖檔可供合併抵銷。net_cap_for_round() 與 "
        "_REPIN_MAX_CONSECUTIVE_RISING_ROUNDS 的判準邏輯與門檻數字本輪一個字未動，本表只涵蓋"
        "本輪這一個精確主軌淨額，往後任何一輪照原判準阻擋。紀錄見 "
        "CrossPlatform_DEF200274_Parallel_Tests_Evidence_2.md〈系列收斂後四方重驗〉與 "
        "docs/06_quality/CrossPlatform_R145_Scan_Findings.md〈附記（R171）〉。"
    )),
}
...
_REGRESSION_LANE_LOG: tuple[tuple[str, int, str], ...] = (
    ...
    ("R143", 165, "DEF-200-275 第四輪：test_context_window_parity.py（+145）＋本檔自身逐檔漂移"
     "（主表本列＋本軌新列＋_REPIN_NET_CAP_SCHEDULE 到期兌現列與重新武裝）＋"
     "test_platform_neutral_paths.py 欠債表下修註解（+3），全額歸本軌（記帳誠實度分類）。"
     "逐檔清單見 docs/06_quality/CrossPlatform_R143_Scan_Findings.md。"),
    ("R143", 73, "DEF-200-275 第四輪第 1 輪審查修復（SA-R4-03）：test_find_git_bash_parity.py "
    ...
    ("R147", 270,
     "DEF-200-274 第九輪四方獨立複審收斂：P1/P0 zshrc 假紅根治回歸鎖＝"
     "ShouldRunParallelDecisionTest／RunWithFloorNeverParallelizesSyntheticTreeTest／"
     "`_zero_dep_child_env()`＋ZeroDepProbeForcesSequentialChildEnvTest（119 行）＋"
     "既有缺陷修復回歸鎖＝ParallelTimingCacheLoadHintsTest（快取壞檔容錯＋bool 誤判"
     "計時，55 行）／ParallelShardSigtermCleanupTest（SIGTERM 清理競態，55 行）／"
    ...
    ("R153", 257,
     ...
     "CrossPlatform_R145_Scan_Findings.md〈第十五輪附記（R153）〉。"),
    ("R155", 309,
     "貼齊上限（同 R131 體例）：DEF-200-322／323 缺陷回歸鎖優先納入——"
     "test_dev_start.py／test_pre_commit_dispatcher_sigpipe.py／test_clean_venv_carrier.py "
     "段；DEF-200-321 缺陷回歸鎖：test_workflow_permission_concurrency_lock.py 段；"
     "DEF-200-315 缺陷回歸鎖：test_windowsapps_guard_bash_parity.py／"
     "test_windowsapps_guard_cross_consistency.py 本機缺席／CI／逃生口三情境行為測試段"
    ...
    ("R159", 180,
     "DEF-200-345／346／347／348 結案回歸鎖（記帳誠實度分類，非湊額）：test_check_defect_log_"
     "crossref.py 整班隔離＋巢狀鎖（+42）、test_context_budget_guard.py "
     "foreign_trace_growth_problems＋五支鎖（+78）、test_ci_gate_xdist_allowlist.py 廣播文字鎖"
     "（+25）、test_cpu_budget.py DetectPhysicalCountTest 分派順序＋live smoke（+35）＝180"
     "（子集不得大於母項；平台解析／分派鎖 +118 依既有 cpu_budget 判例歸功能軌）。逐項見 "
     "CrossPlatform_R145_Scan_Findings.md〈第二十輪附記（R159）〉。"),
    ("R163", 72,
     "DEF-200-354 結案回歸鎖（記帳誠實度分類）：test_wake_chain_halt_r278.py 模組級 ENV_SPEC "
    ...
    ("R171", 309,
     "DEF-200-379／382／383／384／385 結案回歸鎖（記帳誠實度分類）：test_workflow_permission_"
     "concurrency_lock.py（chaos-latest job 層連敗隔離回歸鎖）、test_archive_defect_log.py"
     "（穩定快照自證與正樣本）、test_dev_start.py／test_platform_neutral_paths.py／"
     "test_doc_loc_baseline_freshness_r60.py（效能熱點回歸鎖）、test_ci_gate_xdist_allowlist.py"
...
_FROZEN_PREFIX_REWRITE_LEDGER: tuple[tuple[str, str, str, str], ...] = (
    ...
    # R143：DEF-200-275 第四輪——分子改讀逐字稿 API usage＋parity 鎖新增，凍結 round-label-ok
    # 前綴延伸涵蓋新增列本身（prefix_len 由 --print-guard-lines 印出）。
    ("R143", "b97e86a790f0", "7bd3af08e2e2", "DEF-200-275"),
    # R143 同輪追加：第 1 輪審查修復（SA-R4-03 候選鏈測試）再延伸凍結前綴一列。 round-label-ok
    ("R143", "7bd3af08e2e2", "3d5c464c7535", "DEF-200-275"),
    ...
    # R146：DEF-200-275 第七輪收尾單人窗口——package A／B 合法新增＋Dev-Trim7 散文搬遷 round-label-ok
...
_PHASE2_REVIEW_LOG: tuple[tuple[int, str, str], ...] = (
    ...
    (141, "[提案]",
     ...
     "下一個視窗（連續『維持觀察』計數歸零）。"),
    (147, "[維持觀察]",
     "本輪是 DEF-200-274 第九輪收尾單人窗口（護欄層 guard-line 記帳＋分軌申報），"
     "未觸碰 ADR-XPLAT-013 方向 (c) 觀測→阻斷轉換提案本身——R129 提出的既存提案"
    ...
    (153, "[提案]",
     ...
     "下一個視窗（連續『維持觀察』計數歸零）。"),
    (159, "[維持觀察]",
     "本輪是多 CPU 第十五輪收尾單人窗口（DEF-200-345～348；護欄層 guard-line 記帳＋分軌"
     "申報），未觸碰 ADR-XPLAT-013 方向 (c) 觀測→阻斷轉換提案本身——R129 提出的既存提案"
     "（送四方複審一事）迄今仍待主控排定、尚未有結果，亦未提出新 Phase 2 提案。"
    ...
    (165, "[提案]",
     ...
     "（連續『維持觀察』計數歸零）。"),
    (171, "[維持觀察]",
     "本輪是系列收斂後四方重驗收尾單人窗口（護欄層 guard-line 記帳＋分軌申報），未觸碰 "
     "ADR-XPLAT-013 方向 (c) 觀測→阻斷轉換提案本身——R129 提出的既存提案（送四方複審一事）"
