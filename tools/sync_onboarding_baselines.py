...
R60 round 3／R67 累積修正（四方複審逐條命中，詳細論證與 DEF ID 已搬至
`docs/06_quality/SD_sync_onboarding_baselines_R60_R67_history.md`，本處只列現行行為
落點，不重複散文——本輪為釋出 SPECIAL_FILES raw-line 棘輪餘裕而搬遷，內容零刪減）：
  - 受鎖行的散文同步受管，見 `prose_problems()`／`historical_problems()`（DEF-101-562）。
  - 表② 平台維度＋逐平台指紋記帳，見 `_PLATFORM_COLUMN_LABELS`／`platform_cell_index()`
    （R67-D1／D6）。
  - `main()` 全面 argparse 化，未知旗標 fail-loud（R67-D20）。
  - 指紋量測窗口收斂於 `measure_slow_on_stable_tree()`（DEF-101-677）。
...
🔴 判準邊界（誠實劃界）：
  ...
  - **指紋只覆蓋測試樹**（四棵皆遞迴 `**/*.py` 的**行尾正規化後**內容）。生產碼變動改變
    `parametrize` 來源、docker daemon 可用性、平台差異都能改變計數而指紋不動 ⇒ 它是
    stale 的**充分觸發器、非必要條件**（會漏、不會冤）。要它變成必要條件就得付整套重測
    的代價，那正是表② 沒有 live 鎖的原因。
...
用法（argparse；未知旗標一律 rc=2 fail-loud，`--help` 印完整用法）：
  python tools/sync_onboarding_baselines.py            # 稽核（＝--check，表①）；stale 即 exit 1
  python tools/sync_onboarding_baselines.py --check    # 同上顯式寫法（文件與閘門引用的那條）
...
def _run_cigate() -> dict[str, int]:
    ...
    # DEF-101-889：裸 `python`（ci-gate.sh 內呼叫）先前繼承呼叫端 PATH，可能與
    # `_run_autoclaude_pytest()` 明確指定的 `sys.executable` 不同支卻共用同句 provenance。
    cigate_env = dict(os.environ)
    py_dir = Path(sys.executable).resolve().parent
    cigate_env["PATH"] = f"{py_dir}{os.pathsep}{cigate_env.get('PATH', '')}"
    proc = subprocess.run(
