...
MIN_TESTS = 4543  # 🔴 重釘 4371 → 4543（方向＝收緊；2026-09-22，DEF-200-360 收尾單人窗口）：discovery 探針實測直接填入、零加減推算；成長來源＝DEF-200-358～360 回歸鎖（test_dev_platform_provenance.py）與 DEF-200-359 兩支鎖，runner 自檢「餘裕只剩 64／219」提示重釘。歷來完整逐輪沿革（含每次重釘理由）已搬至 docs/06_quality/CrossPlatform_Guard_Line_History_MinTests.md，本行不再原地累加。
