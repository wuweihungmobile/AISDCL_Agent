...
def ratchet_drift_message(
    ...
    runner 拿它當**外層後備**（第一層見 `min_tests_margin`），保鮮期斷言傳 `RATCHET_STALE_RATIO`。"""
...
def warn_ratchet_drift(count: int, min_tests: int, module_counts: dict[str, int] | None = None) -> str | None:
    msg = min_tests_margin.headroom_message(count, min_tests, module_counts or {}) or ratchet_drift_message(count, min_tests)
    if msg is not None:
        print(msg)
    return msg
