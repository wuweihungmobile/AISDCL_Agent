...
**最新狀態（improving_100 校正，2026-06-30）**：nightly 仍運行（`.mutation_history.jsonl` 末筆 06-29、`mutation_backlog_token_guard.md` 06-30 產出）。觀察期 #1 mutation：kill_rate 因 improving_79/80/84 改 `token_guard/` 源碼自 76.5% 降至 **69.56%**（survived 35→42）惟仍 ≥ **0.68 effective threshold**（餘 1.56pp）；tail7 unique sha 已 **3/7**（ADR-SD09-009 §11.6 PM v1.2：源碼演進閘門、**無日曆到期日**）→ `should_lock=False` 懸停（kill_rate 過、sha 3/7 卡）；`G0 最遲 2026-06-26` 已逾期，待 PM 決策（TG 退出降 SD_10 或續待源碼演進）。**improving_100（C 軌）補 token_guard boundary 測試殺 3 真缺口 survived**（thresholds #129-131 之 M_b/M_e + watcher #71，受控突變驗紅）、誠實標記 L49 #122-124 死分支等價變異（DEF-100-002 routed）；full pytest 3607→**3618**/122、lint 8 kept、零碰源碼（sha 不變）。R51/06-01 史料見 sprint_history.md §1.7.3。
