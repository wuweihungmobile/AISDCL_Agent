...
**完整版**：[docs/06_quality/Nightly_Forensic_Discipline.md](docs/06_quality/Nightly_Forensic_Discipline.md) v1.14（19 條，#14 增列 1b）。SD_09 W0~W3 40 輪 audit 累積；違反任一條 → P0 audit。**任何紀律新增 / 修訂必須先改完整版，再同步本摘要**。
...
14. **schtasks vs 互動 PATH 等價 + StrictMode $null.Property 保護** — ps1 開頭釘死根層 .venv\Scripts；禁 `(Get-Command X -EA SilentlyContinue).<Prop>` 鏈式；改兩步式（R19 P0-AUDIT-R18-1）。**1b 直譯器等價+可取證**：Windows 亦釘死根層 .venv\Scripts\python.exe 絕對路徑，不靠 PATH；log 必印解析後路徑+版本+venv 狀態，只印字面 token 等同沒印（DEF-200-302，原 DEF-101-506）
