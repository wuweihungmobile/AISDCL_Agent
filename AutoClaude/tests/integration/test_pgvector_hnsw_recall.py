...
def test_pgvector_recall_at_10_ge_095():
    ...
    pytest.skip(
        ...
        f"承接輪次 R90（R89 推遲，理由是**平台綁定不是拖延**：R89 在 mac 真機上跑，"
        f"而 staging 只有 Windows 11 那台有 ⇒ 該輪結構上不可能轉綠；"
        f"R88 已履行 PM 拍板『保留』附帶的自動通道義務，此處只推檢查點）。"
...
def test_pgvector_p95_latency_under_50ms():
    ...
    pytest.skip(
        ...
        f"[DEBT] 需 W3 G3 staging 資料集（同 T1）。承接輪次 R90"
        f"（R89 推遲：同 T1，mac 輪結構上跑不到 Windows 11 才有的 staging）"
