...
def write_baseline(
    ...
    *,
    ...
    # DEF-200-304：檔頭 provenance 行必須從資料本身動態推導（distinct environment
    # 值），不得手寫一句會過期的斷言（例如寫死某個環境名或筆數）——資料變了這行
    # 就自動跟著變，不需要人記得回來訂正（ADR-SD08-003 §2.7）。
    environments = sorted(
        {sec.get("environment") for sec in existing.values() if sec.get("environment")}
    )
    if environments:
        lines.append(
            "# Environment provenance（DEF-200-298／ADR-SD08-003 §2.7）："
            f"本檔各 section 之 environment 欄位＝{','.join(environments)}；"
            "跨環境比對之 advisory 分流見 tools/perf_regression_check.py"
        )
    lines.append("")
