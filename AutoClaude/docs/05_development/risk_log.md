...
| **R-SD09-A-5** | `alembic_version=0012_yaml_import_staging`，但 `0013_drift_log` + `0014_config_audit_log` migration 已落地未跑 → 觀察期 #3 取證 SQL（依賴 `drift_log.detected_at`/`severity` 欄位）恆失敗 → 30 天零事件條件永遠 false → R-SD09-A-4 雙條件 (2) 阻塞 | 🔴 → 🟡 **部分緩解 2026-05-20**（zero-trust audit fix）| A. PG production / 議題 A | (a) `.alembic_offline_head.txt = 0014_config_audit_log` 標記檔已落地；(b) `tests/contract/fixtures/drift_log_30day_zero.json`（30 筆 severity=info 場景 A fall-back fixture）已備；(c) `alembic/__init__.py` shadow 已刪除；(d) **PG 真實 alembic upgrade head 待 W0 G0 預檢執行**（無本地 PG 容器時透過 docker-compose 起 postgres:17 + alembic upgrade） | 🟡 部分緩解（場景 A fixture 齊備；場景 B PG 真實 upgrade 待 W0 G0 預檢）|
...
| ID | 移交說明 | 狀態 |
|----|---------|------|
| **R-SD08-D-1** | mutation pilot TokenGuardPlugin observing → SD_09 §1.2 議題 B 接續 | 🟡 **監控管線未就緒（2026-05-20 W0 G0 audit 再確認）**：mutmut 3.x 不支援 Windows 原生（issue #397）→ 等 Docker `python:3.11-slim` 流程穩定 + `.mutation_history.jsonl` 開始累積後始能進入「觀察期 #1 監控中」；W0 G0 audit QA 實測 `.mutation_history.jsonl` 仍不存在；**W0 啟動後 task breakdown 必含「mutation pilot 重定錨」task**：(a) Docker 流程 SOP 文件化；(b) 觀察期 14 天起算點以「第一筆 `.mutation_history.jsonl` 寫入日」為錨；(c) CI `mutation-pilot-nightly` job 加實作 check |
| **R-SD08-PM-#3** | mutation pilot 單模組 fall-back（< 60%）→ SD_09 §1.2 議題 B 接續 | 🟡 **監控管線未就緒（2026-05-20 W0 G0 audit 再確認）**：同 R-SD08-D-1 — Docker 流程 + `.mutation_history.jsonl` 累積前無資料可判 fall-back；觀察期 #1 起算日延後 |
...
- 最後更新：2026-05-20（**v1.5 第五輪 W0 G0 zero-trust audit 同步**；新增 R-SD09-A-6 alembic multi-head merge revision 待 W2 處置；同步五方審查 W0 G0 FULLY APPROVED-WITH-CONDITIONS）
- 下次審查觸發：SD_09 W0 啟動（**最早 2026-06-18~22 提前窗口 / 最遲 2026-06-26**）+ 每 Wave 末複查
